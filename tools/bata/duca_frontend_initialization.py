...
RESETTABLE_MODEL_STATE_KEYS = frozenset({"frame_selector._loss_weight_schedule_step"})
...
def _normalized_state_dict(state_dict: Mapping[str, Any]) -> dict[str, Any]:
    """Drop the optional DDP prefix while preserving the complete model state."""
...
def initialize_model_from_checkpoint(
    ...
    *,
    ...
    """Initialize a new curriculum phase from an EMA model without resuming state.

    Unlike ``--resume``, this deliberately does not carry the optimizer,
    scheduler, AMP scaler, or curriculum clock forward.  It is therefore the
    correct boundary between uniform full-model warmup and learned sampling.
    """
    ...
    unknown = sorted(set(cfg) - allowed)
    if unknown:
        raise ValueError(f"unknown model_initialization keys: {unknown}")
    ...
    if not checkpoint_path.is_file():
        raise FileNotFoundError(
            f"model initialization checkpoint is missing: {checkpoint_path}"
        )
    expected_sha256 = str(cfg.get("checkpoint_sha256", "")).lower()
    if len(expected_sha256) != 64:
        raise ValueError("model_initialization.checkpoint_sha256 is required")
    observed_sha256 = sha256_file(checkpoint_path)
    if observed_sha256 != expected_sha256:
        raise RuntimeError("model initialization checkpoint SHA256 mismatch")

    state_key = str(cfg.get("state_key", "state_dict_ema"))
    if state_key not in {"state_dict", "state_dict_ema"}:
        raise ValueError("model_initialization.state_key must be state_dict or state_dict_ema")
    checkpoint = torch.load(str(checkpoint_path), map_location="cpu")
    if not isinstance(checkpoint, Mapping):
        raise ValueError("model initialization checkpoint must be a mapping")
    source = checkpoint.get(state_key)
    if not isinstance(source, Mapping):
        raise ValueError(f"model initialization checkpoint lacks {state_key}")
    expected_epoch = cfg.get("expected_checkpoint_epoch")
    observed_epoch = checkpoint.get("epoch")
    if expected_epoch is not None and int(observed_epoch) != int(expected_epoch):
        raise RuntimeError(
            "model initialization checkpoint epoch mismatch: "
            f"expected {int(expected_epoch)}, got {observed_epoch!r}"
        )

    source_state = _normalized_state_dict(source)
    target_state = model.state_dict()
    reset_keys = tuple(
        str(key)
        for key in cfg.get(
            "reset_state_keys", sorted(RESETTABLE_MODEL_STATE_KEYS)
        )
    )
    unsupported_reset = sorted(set(reset_keys) - RESETTABLE_MODEL_STATE_KEYS)
    if unsupported_reset:
        raise ValueError(
            f"model initialization cannot reset unapproved state: {unsupported_reset}"
        )
    for key in reset_keys:
        if key not in target_state or key not in source_state:
            raise RuntimeError(f"model initialization reset state is missing: {key}")
        source_state[key] = target_state[key].detach().clone()

    missing = sorted(set(target_state) - set(source_state))
    unexpected = sorted(set(source_state) - set(target_state))
    parameter_keys = set(dict(model.named_parameters()))
    buffer_keys = set(dict(model.named_buffers()))
    missing_parameters = sorted(set(missing) & parameter_keys)
    missing_buffers = sorted(set(missing) & buffer_keys)
    missing_unknown = sorted(set(missing) - parameter_keys - buffer_keys)
    if missing_parameters or missing_unknown or unexpected:
        raise RuntimeError(
            "model initialization state mismatch: "
            f"missing_parameters={missing_parameters}, "
            f"missing_unknown={missing_unknown}, unexpected={unexpected}"
        )
    for key in missing_buffers:
    ...
    if incompatible.missing_keys or incompatible.unexpected_keys:
        raise RuntimeError(
            "model initialization strict load failed: "
            f"missing={incompatible.missing_keys}, unexpected={incompatible.unexpected_keys}"
        )

    receipt = {
        "schema": "duca_model_initialization_v1",
        "checkpoint_path": str(checkpoint_path),
        "checkpoint_sha256": observed_sha256,
        "checkpoint_epoch": None if observed_epoch is None else int(observed_epoch),
        "checkpoint_state_key": state_key,
        "loaded_model_state_count": len(source_state),
        "reset_state_keys": list(reset_keys),
        "retained_target_buffer_state_keys": missing_buffers,
        "retained_target_buffer_state_count": len(missing_buffers),
        "optimizer_state_loaded": False,
        "scheduler_state_loaded": False,
        "amp_scaler_state_loaded": False,
    }
    receipt["receipt_sha256"] = canonical_sha256(receipt)
    if logger is not None:
        logger.info(
            "Initialized full DUCA model from %s (%s, epoch=%s, reset=%s)",
            checkpoint_path,
            state_key,
            observed_epoch,
            list(reset_keys),
