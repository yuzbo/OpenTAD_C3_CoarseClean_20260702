#!/usr/bin/env python3
"""Same-GPU final-EMA full-stack cost replay for four frozen ZoomToken R1 arms.

This entry point never trains or resumes a model.  It replays the canonical
THUMOS14 validation population with four completed epoch-59 EMA checkpoints in
one Slurm allocation, using a 16-pass Williams counterbalancing order.
"""
...
MAX_POWER_GAP_MS = 3000.0
MAX_UNCOVERED_FRACTION = 0.005
...
EXPECTED_METRICS_PERCENT = {
    ...
    "B": {"average_mAP": 67.38, "mAP@0.6": 59.34, "mAP@0.7": 46.01},
    "C": {"average_mAP": 66.50, "mAP@0.6": 59.24, "mAP@0.7": 45.21},
    "D": {"average_mAP": 66.11, "mAP@0.6": 57.83, "mAP@0.7": 44.88},
}
ACCURACY_PARITY_METRIC_KEYS = tuple(EXPECTED_METRICS_PERCENT["A"])
ACCURACY_PARITY_TOLERANCE_PP = 0.05
ACCURACY_PARITY_REFERENCE = {
    "precision": "reported_2dp",
    "source_revision": "project_wiki_terminal_epoch59_ema_records_2026-08-31",
    "source_path": "research-wiki/query_pack.md",
...
ARM_SPECS = {
    "A": {
        ...
        "checkpoint_arg": "a_checkpoint",
        "checkpoint_sha256": "3977e890b6f3990225d7d6818670aef1243fd024f4eb0f0f7e82344a48807599",
        "official_support": "strict_rect8x8",
        "refresh_carry_mode": "full64",
        "selected_tokens_per_tubelet": 64,
        "refresh_query_tokens_per_tubelet": 64,
        "refresh_support_tokens_per_tubelet": 64,
        "executed_tokens_per_window": 24576,
    },
    "B": {
        "label": "DSR6-KV",
        "job_id": "1252527",
        "training_revision": "c6327a891809aa30370b3b2d9bedab0dcfe0d326",
        "config": "configs/adatad/thumos/georoute_official_r1_dsr6_kv_prebackbone_seed42_v001.py",
        "checkpoint_arg": "b_checkpoint",
        "checkpoint_sha256": "2299b1229fa0dff8e266707b895a05186b570bcffc9b2ca6cd6da712d79599ad",
        "official_support": "strict_rect8x8",
        "refresh_carry_mode": "dsr6_kv",
        "selected_tokens_per_tubelet": 64,
        "refresh_query_tokens_per_tubelet": 32,
        "refresh_support_tokens_per_tubelet": 64,
        "executed_tokens_per_window": 24576,
    },
    "C": {
        "label": "MOD32-KV",
        "job_id": "1252180",
        "training_revision": "813012620dca991ff90121d0d9faf688f303d1ef",
        "config": "configs/adatad/thumos/georoute_official_r1_mod32_kv_prebackbone_seed42_v001.py",
        "checkpoint_arg": "c_checkpoint",
        "checkpoint_sha256": "f1a0d57310fb07a101da970a4f195c016640f0558314ba1a6abcd8d317923ac5",
        "official_support": "strict_rect8x8",
        "refresh_carry_mode": "mod32_kv",
        "selected_tokens_per_tubelet": 64,
        "refresh_query_tokens_per_tubelet": 32,
        "refresh_support_tokens_per_tubelet": 64,
        "executed_tokens_per_window": 24576,
    ...
    "D": {
        ...
        "job_id": "1252179",
        "training_revision": "813012620dca991ff90121d0d9faf688f303d1ef",
        "config": "configs/adatad/thumos/georoute_official_r1_drop32_prebackbone_seed42_v001.py",
        "checkpoint_arg": "d_checkpoint",
        "checkpoint_sha256": "36c3d3cc597b754bb8cc39175b3faf964b2c170a4b21fec4df04b3ae49494f47",
        "official_support": "strict_rect8x8",
        "refresh_carry_mode": "drop32",
        "selected_tokens_per_tubelet": 32,
        "refresh_query_tokens_per_tubelet": 32,
        "refresh_support_tokens_per_tubelet": 64,
        "executed_tokens_per_window": 12288,
    },
}


def _checkpoint_path(args: argparse.Namespace, arm: str) -> Path:
    return Path(getattr(args, ARM_SPECS[arm]["checkpoint_arg"]))
...
def _git_identity(root: Path, expected_commit: str) -> None:
    ...
    if head != expected_commit or dirty:
        raise RuntimeError("R1 depth Pareto replay requires its exact clean execution commit")
    ...
    if not remote_contains:
        raise RuntimeError("R1 depth Pareto replay requires an execution commit present on a remote ref")
...
def _checkpoint_preflight(torch: Any, path: Path, arm: str) -> dict[str, Any]:
    ...
    expected_sha256 = ARM_SPECS[arm]["checkpoint_sha256"]
    if observed_sha256 != expected_sha256:
        raise ValueError(f"{arm} checkpoint SHA256 differs from the frozen artifact")
    return {
        "path": str(path),
...
def _raw_writer_known_answer(scratch: Path) -> dict[str, Any]:
    ...
    _write_jsonl(cost_path, [{"arm": "A", "pass_index": 0, "dataset_item_id": "0:v:0"}])
...
def precheck(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.result_root.exists():
        raise FileExistsError(f"formal R1 depth Pareto result root already exists: {args.result_root}")
    required = (
        args.annotation,
        args.class_map,
        args.video_root,
        *(_checkpoint_path(args, arm) for arm in ARM_KEYS),
    )
    for path in required:
        if not path.exists():
            raise FileNotFoundError(path)
    if not str(args.result_root.resolve()).startswith(str(REMOTE_BOUNDARY.resolve()) + os.sep):
        raise ValueError("R1 depth Pareto result root leaves the shared remote write boundary")
    ...
    with tempfile.TemporaryDirectory(prefix="zoomtoken-r1-depth-pareto-precheck-") as temporary:
        ...
        for arm in ARM_KEYS:
            ...
            checkpoint_receipts[arm] = _checkpoint_preflight(
                torch, _checkpoint_path(args, arm), arm
            )
    ...
    if any(manifest != manifests[0] for manifest in manifests[1:]):
        raise ValueError("the four arms do not share an ordered validation population")
    return {
...
def _sample_identity(cpu_batch: Mapping[str, Any], ordinal: int) -> dict[str, Any]:
    ...
    if not isinstance(metas, list) or len(metas) != 1:
        raise ValueError("R1 depth Pareto profile requires batch-one metadata")
    meta = metas[0]
...
def _runtime_audit_projection(audit: Mapping[str, Any]) -> dict[str, Any]:
    packed = dict(audit.get("packed") or {})
    strict_rectangle = dict(audit.get("strict_rectangle") or {})
    return {
        "official_support": audit.get("official_support"),
        "selection_application": audit.get("selection_application"),
        "native_materialization_before_heavy": audit.get(
            "native_materialization_before_heavy"
        ),
        "selected_tokens_per_tubelet": audit.get("selected_tokens_per_tubelet"),
        "refresh_carry_mode": audit.get("refresh_carry_mode"),
        "refresh_query_tokens_per_tubelet": audit.get(
            "refresh_query_tokens_per_tubelet"
        ),
        "refresh_support_tokens_per_tubelet": audit.get(
            "refresh_support_tokens_per_tubelet"
        ),
        "temporal_carry_enabled": audit.get("temporal_carry_enabled"),
        ...
        "packed_execution_mode": packed.get("execution_mode"),
        "packed_refresh_execution_mode": packed.get("refresh_execution_mode"),
        "executed_patch_tokens_per_window": packed.get(
            "executed_patch_tokens_per_window"
        ),
        "padded_heavy_tokens_per_window": packed.get(
            "padded_heavy_tokens_per_window"
        ),
        "dense_adapter_forward_count": packed.get("dense_adapter_forward_count"),
        "strict_support_tokens_per_tubelet": strict_rectangle.get(
            "tokens_per_tubelet"
        ),
        "strict_executed_patch_tokens_per_tubelet": strict_rectangle.get(
            "executed_patch_tokens_per_tubelet"
        ),
        "strict_refresh_query_tokens_per_tubelet": strict_rectangle.get(
            "refresh_query_tokens_per_tubelet"
        ),
        "strict_kv_context_tokens_per_tubelet": strict_rectangle.get(
            "kv_context_tokens_per_tubelet"
        ),
        "full_update_block_count": packed.get("full_update_block_count"),
        "refresh_update_block_count": packed.get("refresh_update_block_count"),
    }


def _expected_runtime_audit(arm: str) -> dict[str, Any]:
    spec = ARM_SPECS[arm]
    mode = spec["refresh_carry_mode"]
    selected = int(spec["selected_tokens_per_tubelet"])
    expected = {
        "official_support": "strict_rect8x8",
        "selection_application": "pre_heavy_videomae",
        "native_materialization_before_heavy": True,
        "selected_tokens_per_tubelet": selected,
        "refresh_carry_mode": mode,
        "refresh_query_tokens_per_tubelet": int(
            spec["refresh_query_tokens_per_tubelet"]
        ),
        "refresh_support_tokens_per_tubelet": int(
            spec["refresh_support_tokens_per_tubelet"]
        ),
        "temporal_carry_enabled": False,
        "physical_tokens_per_window": int(spec["executed_tokens_per_window"]),
        "heavy_backbone_forward_count": 1,
        "packed_execution_mode": "true_clip_ragged_no_padding",
        "packed_refresh_execution_mode": mode,
        "executed_patch_tokens_per_window": int(spec["executed_tokens_per_window"]),
        "padded_heavy_tokens_per_window": 0,
        "dense_adapter_forward_count": 0,
        "strict_support_tokens_per_tubelet": 64,
        "strict_executed_patch_tokens_per_tubelet": selected,
        "strict_refresh_query_tokens_per_tubelet": int(
            spec["refresh_query_tokens_per_tubelet"]
        ),
        "strict_kv_context_tokens_per_tubelet": (
            64 if mode in {"mod32_kv", "dsr6_kv"} else selected
        ),
        "full_update_block_count": 6 if mode == "dsr6_kv" else None,
        "refresh_update_block_count": 6 if mode == "dsr6_kv" else None,
    }
    return expected
...
def _profile_one_pass(
    *,
    ...
    if len(loader) != EXPECTED_WINDOW_COUNT:
        raise ValueError("R1 depth Pareto loader changed the frozen population")
    ...
    try:
        for ordinal in range(EXPECTED_WINDOW_COUNT):
            ...
            observed_audit = _runtime_audit_projection(audit)
            expected_audit = _expected_runtime_audit(arm)
    ...
    if final_window is None or audit_receipt is None:
        raise RuntimeError("R1 depth Pareto pass did not reach final NMS")
...
def _gpu_identity(torch: Any) -> dict[str, Any]:
    ...
    if not selector or "," in selector or torch.cuda.device_count() != 1:
        raise RuntimeError("R1 depth Pareto profile requires exactly one Slurm-visible GPU")
...
def _power_coverage_summary(
    ...
    max_gap_ms = 1000.0 * max(
        right[0] - left[0] for left, right in zip(checked[:-1], checked[1:])
    )
    uncovered_fraction = 1.0 - coverage_ratio
    if uncovered_fraction > MAX_UNCOVERED_FRACTION:
        raise RuntimeError("power trace uncovered fraction exceeds 0.5%")
    if max_gap_ms > MAX_POWER_GAP_MS:
        raise RuntimeError("power trace maximum gap exceeds 3.0 seconds")
    return {
        "status": "COMPLETE",
        ...
        "measurement_coverage_ratio": coverage_ratio,
        "uncovered_fraction": uncovered_fraction,
        "max_trace_gap_ms": max_gap_ms,
        "maximum_allowed_uncovered_fraction": MAX_UNCOVERED_FRACTION,
        "maximum_allowed_trace_gap_ms": MAX_POWER_GAP_MS,
...
def _persist_pass_artifact(
    ...
    artifact = {
        ...
        "prediction_identity_recorded": True,
...
def _measurement_completeness(pass_receipts: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    if len(pass_receipts) != len(PROFILE_ORDER):
        raise ValueError("measurement completeness requires all sixteen pass receipts")
    prediction_hashes: dict[str, list[str]] = defaultdict(list)
    for pass_index, (receipt, arm) in enumerate(zip(pass_receipts, PROFILE_ORDER)):
        ...
        prediction_sha256 = str(artifact.get("prediction_sha256", ""))
        if len(prediction_sha256) != 64:
            raise RuntimeError("pass prediction SHA is missing")
        prediction_hashes[arm].append(prediction_sha256)
        coverage = receipt["power_coverage"]
        if (
            coverage.get("status") != "COMPLETE"
            or int(coverage.get("invalid_sample_count", -1)) != 0
            or float(coverage["max_trace_gap_ms"]) > MAX_POWER_GAP_MS
            or float(coverage.get("uncovered_fraction", 1.0))
    ...
    for arm in ARM_KEYS:
        hashes = prediction_hashes[arm]
        if len(hashes) != 4 or len(set(hashes)) != 1:
            raise RuntimeError(
                f"{arm} repeated full-population predictions are not identical"
            )
    return {
        "status": "COMPLETE",
        ...
        "within_arm_prediction_sha_stability": {
            arm: {"prediction_sha256": prediction_hashes[arm][0], "pass_count": 4}
            for arm in ARM_KEYS
        },
...
def _persist_acquisition_checkpoint(
    *,
    ...
    state = {
        "schema_version": "zoomtoken_r1_depth_pareto_cost_v001",
        ...
        and all(
            bool(row["artifacts"].get("prediction_identity_recorded"))
            for row in pass_receipts
        ),
...
def _classify_complete_replay(
    ...
    del pass_summaries, pass_quality
    if set(arm_summaries) != set(ARM_KEYS):
        raise ValueError("Pareto classification requires all four frozen arms")
    if any(
        not bool(prediction_stability[arm]["all_four_hashes_identical"])
        for arm in ARM_KEYS
    ):
        raise RuntimeError("within-arm prediction identity is incomplete")
    control = arm_summaries["A"]
    ratios = {}
    survivors = []
    for arm in ("B", "C", "D"):
        candidate = arm_summaries[arm]
        row = {
            "arm": arm,
            "label": ARM_SPECS[arm]["label"],
            "relative_to": "A/R1-FULL64",
            "end_to_end_p50_ratio": float(
                candidate["latency_ms"]["end_to_end_serial_ms"]["p50"]
            )
            / float(control["latency_ms"]["end_to_end_serial_ms"]["p50"]),
            "complete_pass_gross_energy_ratio": float(
                candidate["gross_gpu_energy_j"]
            )
            / float(control["gross_gpu_energy_j"]),
            "peak_allocated_memory_ratio": float(
                candidate["peak_gpu_allocated_mb"]
            )
            / float(control["peak_gpu_allocated_mb"]),
            "peak_reserved_memory_ratio": float(
                candidate["peak_gpu_reserved_mb"]
            )
            / float(control["peak_gpu_reserved_mb"]),
        }
        row["passes_all_gates"] = (
            row["end_to_end_p50_ratio"] <= 0.95
            and row["complete_pass_gross_energy_ratio"] <= 0.95
            and row["peak_allocated_memory_ratio"] <= 1.05
            and row["peak_reserved_memory_ratio"] <= 1.05
        )
        ratios[arm] = row
        if row["passes_all_gates"]:
            survivors.append(arm)
    decision = (
        "PARETO_SYSTEMS_SURVIVOR_PENDING_FRESH_PRO"
        if survivors
        else "STOP_R1_FIXED_DEPTH_SPARSITY_FOURPOINT_AS_CURRENT_EFFICIENCY_ROUTE"
    )
    return {
        "decision": decision,
        "control": "A/R1-FULL64",
        "candidate_ratios": ratios,
        "survivor_arms": survivors,
        "accuracy_role": "recorded_diagnostic_only_not_a_cost_gate",
        "rule": {
            "survivor": (
                "relative to A: median4 complete-pass p50<=0.95, gross energy<=0.95, "
                "allocated memory<=1.05, and reserved memory<=1.05"
            ),
            "aggregation": "ratio of arm medians over four complete pass-local estimates",
        },
    }
...
def _write_failure_terminal_receipt(args: argparse.Namespace, error: BaseException) -> Path:
    ...
    _atomic_json(
        path,
        {
            "status": "BLOCKER_NO_SCIENTIFIC_COST_DECISION",
            "blocker_phase": (
                "post_acquisition_diagnostic"
...
def profile(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not os.environ.get("SLURM_JOB_ID") or int(os.environ.get("SLURM_CPUS_PER_TASK", -1)) != 5:
        raise RuntimeError("formal R1 depth Pareto profile requires one Slurm GPU and five CPUs")
    if (int(os.environ.get("LOCAL_RANK", -1)), int(os.environ.get("RANK", -1)), int(os.environ.get("WORLD_SIZE", -1))) != (0, 0, 1):
        raise RuntimeError("formal R1 depth Pareto profile requires torchrun world-size one")
    ...
    if len(allocated) != 5:
        raise RuntimeError("formal R1 depth Pareto profile requires exactly five allocated CPUs")
    sidecar_cpu = allocated[-1]
    detector_cpus = allocated[:-1]
    args.result_root.mkdir(parents=True, exist_ok=False)
    scratch = Path("/tmp") / f"zoomtoken_r1_depth_pareto_job{os.environ['SLURM_JOB_ID']}"
    ...
    try:
        for pass_index, arm in enumerate(PROFILE_ORDER):
            checkpoint = _checkpoint_path(args, arm)
    ...
    for arm in ARM_KEYS:
        ...
        descriptive_pooled_arm_summaries[arm] = {
            ...
            "selected_tokens_per_tubelet": ARM_SPECS[arm]["selected_tokens_per_tubelet"],
    ...
    profile_payload = {
        "schema_version": "zoomtoken_r1_depth_pareto_fullstack_v001",
        "status": "COMPLETED_READONLY_FOUR_ARM_FULLSTACK_PARETO",
        "execution_commit": args.expected_commit,
        "seed": 42,
        "profile_order": list(PROFILE_ORDER),
        "counterbalancing": "Williams_ABDC_BCAD_CDBA_DACB",
    ...
    _atomic_json(
        args.result_root / "terminal_receipt.json",
        {
            ...
            "produced_evidence": [
                ...
                "sixteen pass prediction/evaluator artifacts",
                "profile.json",
            ],
            "reconstructible_evidence": [
                "per-pass p50/p95/throughput/peak memory/gross energy",
                "median-of-four arm estimates and B/C/D-to-A ratios",
...
def _add_common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument("--d-checkpoint", type=Path, required=True)
