...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Materialize one Continuous-RoI S2 real-batch runtime Gate"
    )
    ...
    try:
        if args.output.exists():
            raise FileExistsError("refusing to overwrite a runtime Gate config")
        cfg = bind_runtime_gate_config(
