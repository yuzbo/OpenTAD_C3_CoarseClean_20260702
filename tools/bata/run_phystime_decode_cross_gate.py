...
def main():
    ...
    require(
        ...
        and os.environ.get("PHYSTIME_SBATCH_PATH"),
        "gate Slurm identity is incomplete",
    )
    ...
    report = {
        ...
        "runtime": {
            ...
            "slurm": {
                ...
                "job_variant": os.environ.get("PHYSTIME_JOB_VARIANT"),
                "dag_token": os.environ.get("PHYSTIME_DAG_TOKEN"),
                "comment": os.environ.get(
                    "PHYSTIME_EXPECTED_JOB_COMMENT"
                ),
                "expected_dependency": os.environ.get(
                    "PHYSTIME_EXPECTED_DEPENDENCY"
                ),
                "sbatch_path": str(
                    Path(os.environ["PHYSTIME_SBATCH_PATH"]).resolve()
                ),
                "sbatch_sha256": sha256_file(
                    os.environ["PHYSTIME_SBATCH_PATH"]
                ),
            },
        },
        "preflight": {
