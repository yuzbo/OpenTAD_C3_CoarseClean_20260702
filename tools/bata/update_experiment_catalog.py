...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "result_status": "D768、U384、MNV 三个参考臂及 F01-F06 均有有效官方终态性能；这是 seed 3407 的六臂正式结果，不代表尚未运行的完整 24 臂或多种子结论",
            ...
            "next_action": "封存 F01-F06 的六份 official receipt；不重复运行已完成的 seed 3407。只有论文主张需要完整 24 臂或多种子统计时，才按同一 exact-SHA 合同扩展，并与当前六臂严格分栏",
        },
        {
            "category": "correction_route",
            "name": "CT-DP 当前正式四臂：基础嵌入、CT-Tubelet、B-AMoD 及二者组合",
            "internal_id": "CT_DP_BAMOD_ACTIVE",
            "current_cycle": "本轮只读核查四臂 log.json：均记录 Training Over，epoch59 最后 loss 有限，未记录 AMP skip/NaN/Traceback；已结束作业 stdout/stderr 未在已知目录找到。缺少 skip 日志不推翻 checkpoint 的 5997/5996/5996/5997 更新证据。训练协议修复仍未部署，下一轮优先接通成功更新 replay 和审计，不把本次日志核查误报为修复完成",
            ...
            "result_status": "PROTOCOL_ERROR：不是单纯缺评测收据，四臂均未达到严格 6000 successful updates，不能作为本合同的最终性能",
            ...
            "next_action": "独立修复分支接通 CT-DP 正式 AMP replay/失败即停与成功更新 checkpoint 记录；本地 focused tests、远端 exact-SHA admission 和逐臂 PRECHECK 后，用新命名空间重训 G0-G3。禁止给旧 checkpoint 补造 6000 更新 receipt",
        },
        {
            "category": "correction_route",
        ...
        },
        {
            ...
            "current_cycle": "独立评测代码 67d7079d 接住官方 evaluator 返回的 metrics，写出包含原训练 SHA、评测 SHA、epoch59 EMA、实际 optimizer/scheduler 6000、config/annotation/prediction 和自哈希的收据；不改模型和数据配置。本地及双臂远端均 28 tests PASS，exact-SHA clean tests/实际 checkpoint 核验/2-GPU 真推理 PRECHECK 1275569_0/1 均 COMPLETED(0:0)。完整官方评测 1275624_0/1 已提交，尚未形成最终 receipt。标注中的 validation 对应 211 个 video_test_*，不能因名称而换数据划分",
            "deployment_status": "真实 VideoMAE 覆盖与 2-GPU/global-batch=2 admission 已通过；OFF 1267218、ON 1267219 均 COMPLETED(0:0)。2026-09-07 直接读取两份 epoch_59.pth，EMA 存在、所有已记录 optimizer state step 均为 6000、scheduler 也为 6000；尚无独立官方 evaluator receipt",
            ...
            "next_action": "监督 1275624_0/1 的完整独立官方评测，完成后核验新 metrics.json/self-hash 再报告最终 OFF/ON mAP。训练权重仍为 74473c27，不重复训练；不得把预检或旧训练 telemetry 当成最终 receipt",
        ...
        },
        {
            ...
            "current_cycle": "A1/A6 的 1274924_2/7 均健康，13:11 读取的训练审计完成 epoch7：各 800 successful optimizer/scheduler/EMA updates；A1 3 次、A6 8 次 AMP skipped attempts 均已 replay，nonfinite_loss_attempts=0、replay_exhaustions=0。73bdd34a 只新增另外四臂真实预检与接线，正式模型沿用 1570a725 修复；本地 28 passed/4 Windows-Torch skipped，远端 admission 1275417 的 68 tests 和 CUDA FP32 gate PASS。F 完整方案、A3 关闭侦察监督、A4 关闭时序合并、A5 关闭密集恢复的两轮真实 PRECHECK 1275466_1/4/5/6 均 COMPLETED(0:0)，正式重训 1275675_1/4/5/6 全部 RUNNING。F 官方评测 1275691_1 已 afterok 依赖 1275675_1；追加 A3 评测时 sbatch 返回 AssocMaxSubmitJobLimit，无 job id，A4/A5 评测随即停止尝试。这三项评测标记 BLOCKED_RESOURCE，等待账户提交额度释放，不改模型代码、不重复提交已登记任务",
            ...
            "result_status": "C0 和 A2 有效官方结果；旧 F/A3/A4/A5 因遗漏时间参数优化而不合格，四臂修复版现已正式重训。A1/A6 修复版也在正式训练；共六个修复臂 RUNNING，但只有 A1/A6/F 已登记逐臂 afterok 评测，A3-A5 评测等待提交额度。完整 Evidence 机制结论尚未形成",
            ...
            "next_action": "监督 A1/A6 1274924_2/7、F/A3/A4/A5 1275675_1/4/5/6 到 epoch59 EMA/6000 updates。账户额度释放后只补 A3/A4/A5 的任务索引 4/5/6 评测，各自 afterok 依赖 1275675_4/5/6，使用训练/评测 SHA 73bdd34a、remaining_73bdd34a/formal 与 evaluation 新目录。保留已登记评测 1274926_2、1274927_7、1275691_1，不重复提交。旧受影响 checkpoint 不纳入结果",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-07 13:18 CST",
            "public_gpu_available": 28,
            "public_gpu_total": 240,
            "user_jobs_in_queue": 16,
            "account_constraint": "公共口径 28/240 可用；本用户 11 RUNNING、5 PENDING。六个 Evidence 修复臂及双 GPU ET-TRC OFF/ON 评测均获得资源；新增 Evidence A3 评测提交被 AssocMaxSubmitJobLimit 拒绝，A3/A4/A5 后续评测等待提交额度，不是缺少公共 GPU。历史 BAFDR 1267921 仍只读",
        },
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。当前 H65 三个参考臂及 F01-F06、Evidence C0/A2 具有已核验终态官方结果；BAFDR 五臂原始官方指标已通过 84f1f035 独立事后自哈希封存，保留原训练和评测身份。CT-DP 是成功更新不足的协议错误，ET-TRC 缺独立评测，DUCA-Unified 缺实现。终态训练 receipt 只能证明训练合同完成，不得从 admission 或中期验证推导最终 mAP 或机制结论。",
        "",
    ]
    return "\n".join(lines)
