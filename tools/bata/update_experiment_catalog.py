...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "scientific_audit": "2026-09-08原因核验：F01-F06全部phase=False，不能代表四相机制已验证。64.2265属于F02 Taylor因子；F09等phase-on配置不能因存在于磁盘而算完成。F臂f2068e18相对参考e553a5a4增加时间参数optimizer组，+0.34不是已经隔离混淆的因果收益。详见13_PERFORMANCE_CAUSE_AUDIT_20260908.md",
        ...
        },
        {
            ...
            "scientific_audit": "2026-09-08远端clean 78cde1be真实selector/head CPU复现确认：G0/G1只有0..383检测点，却接收原样dense GT[500,600]并设置native-axis，正样本0；G2/G3物理网格正样本4。这是当前未修复的坐标错误，不是少3-4更新。G0-G3全部B-AMoD关闭，旧四臂名称错误已纠正。修复计数和既有PRECHECK不证明坐标合同正确",
            ...
            "result_status": "旧四臂仍为PROTOCOL_ERROR；78cde1be虽修复更新计数，G0/G1新确认坐标接线错误仍未修复，不能作为合法均匀/动态比较基线；G2/G3尚无新最终性能",
            ...
            "next_action": "优先在独立修复分支统一G0/G1的GT、proposal和后处理坐标；保持消融定义，不能只开启head混淆G1/G2。复现正样本分配与逆映射后按focused/exact-SHA CUDA/PRECHECK规则重提，旧产物保留。当前作业和既有独立评测不得包装成有效G0/G1基线或B-AMoD结果；本次原因核验未修改或取消作业。G2/G3独立评测沿用既有身份，不从训练telemetry推导正式性能",
        },
        {
            "category": "correction_route",
            "name": "DUCA-Unified 当前 41 单元正交消融控制台",
            "internal_id": "DUCA_UNIFIED_ACTIVE",
            "scientific_audit": "2026-09-08核对生成器_IMPLEMENTATION_BLOCKERS：Taylor P0/P1真实接线、H65 retention/transition仍是实现欠账，无完整矩阵结果，不能将其归类为已训练的低性能路线",
        ...
        },
        {
            ...
            "scientific_audit": "2026-09-08单位纠正：G96是96x96空间分辨率，全局仍48x16=768帧；K16局部是16x16=256原始帧、128 tubelets。不是96全局帧加128局部帧，也不与uniform384/160同输入几何。旧少3-6更新不能单独解释十余个百分点差距；新完整结果未齐前不承诺恢复65/69",
        ...
        },
        {
            ...
            "scientific_audit": "2026-09-08源码核验：固定stride anchors加共享rank64可学习线性残差，不是精确anchor-dependent Jacobian，也未实现事件门控；非恒等初始化和无显式dense残差监督是待验证机制风险。OFF/ON下降7.2672是当前版本负结果，不足以否定全部Taylor路线。OFF的letterbox/增强/训练协议本身也不同于原版AdaTAD",
        ...
        },
        {
            ...
            "scientific_audit": "2026-09-08配置核验：C0是历史H65选择控制，不是均匀384；A2是NO_TIME反事实，仍启用merge/recovery等。59.23/54.28不能充当完整F结果并据此判定路线失败",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "performance_cause_audit_scope": "2026-09-08本次追加的是原因核验，不是完整队列心跳；运行与资源信息仍按各行明确列出的采样时间解释。已确认CT-DP坐标错误未修复、H65四相和CT-DP B-AMoD实验覆盖误标；不改写原始mAP",
