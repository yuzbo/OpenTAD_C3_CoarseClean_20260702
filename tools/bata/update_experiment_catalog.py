...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "receipt_recheck": "2026-09-07 22:06心跳：六份F01-F06 metrics JSON可读，evaluation_sha256逐份复算一致；seed3407、f2068e18、epoch59 EMA绑定不变，六个终态checkpoint仍存在。未重复推理或改写指标",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 22:11 CST：1276669/1276670/1276671/1276672四臂全部RUNNING，G0/G1/G2/G3已完成epoch40/41/40/40，optimizer/scheduler/EMA各自同步为4100/4200/4100/4100，AMP skips为3/3/4/3且已重放补齐。当前stdout/stderr未发现新Traceback、RuntimeError或磁盘配额错误；尚无epoch59 checkpoint。独立终态评测11ced13a不改opentad、configs或训练。账户释放名额后已提交其GPU四臂单批评测PRECHECK1277767，22:17因AssocGrpGRES排队；尚未验证GPU评测或产生新官方结果",
            ...
            "evaluator_validation": "11ced13a本地43 passed/1 CUDA skipped，独立有限审阅未发现阻断；远端clean exact SHA同样43 passed/1 CUDA skipped，CTDP_EVAL_SOURCE_OK。GPU单批评测PRECHECK1277767已提交，22:17 PENDING(AssocGrpGRES)；使用原G0-G3六更新预检权重，不打开指标。没有新官方mAP或正式receipt",
            "evaluator_precheck_job": "1277767",
            "evaluator_precheck_state": "PENDING(AssocGrpGRES), 2026-09-07 22:17 CST",
            ...
            "formal_job_states": {"G0": "RUNNING; 4100 successful updates", "G1": "RUNNING; 4200 successful updates", "G2": "RUNNING; 4100 successful updates", "G3": "RUNNING; 4100 successful updates"},
            "observed_successful_updates": {"G0": 4100, "G1": 4200, "G2": 4100, "G3": 4100},
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 22:06心跳再次读取Unified本地HEAD/clean状态，793c4f9c未改变。此前定位的Taylor P0/P1运行时接线及H65 original retention/transition缺失仍未修复，不能由其他路线测试通过替代。本轮推进CT-DP评测预检和Evidence完整方案续训提交；Unified仍是明确的实现欠账，而不是资源排队，41单元未部署",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 22:11 CST：710ce8a6的新D160教师1276650与G96全局低清1276675仍COMPLETED(0:0)，上一轮已直接验证两份epoch59 EMA的337个AdamW state全部6000，未重复加载。U16均匀分块1276842与晚融合LATE1276843均完成epoch20/2100成功更新，scheduler/EMA均2100，各3次AMP skip已重放，非有限loss及replay耗尽均0，未发现新运行错误。五份旧sealed JSON仍可读，但旧训练5994-5997更新仍协议不合格。少3-6次更新不足以解释全部低分，不宣称性能原因已查清",
            ...
            "repair_validation": {"commit": "710ce8a6246c471742c83bf7c180d9ab87c36fac", "local": "41 passed, 1 Windows Torch skipped", "remote_exact_clean_linux": "44 passed, 1 CUDA skipped", "cuda_and_two_gpu_precheck": "1276618 COMPLETED(0:0): 45 tests passed; D160/G96/U16/LATE/NOKD各两轮三批，BAFDR_SUCCESSFUL_UPDATE_PRECHECKS_OK", "formal_retraining": "22:11 CST: D1601276650/G961276675已真实AdamW6000完成；U161276842/LATE1276843均2100成功更新，健康运行。NOKD未提交；FULL新教师已就绪，其专用PRECHECK/训练仍待额度"},
            ...
            "observed_successful_updates": {"D160": 6000, "G96": 6000, "U16": 2100, "LATE": 2100},
        },
        {
            "category": "correction_route",
            "name": "ET-TRC 当前双臂：完整 Transformer 与局部 Taylor/JVP 近似",
            "internal_id": "ET_TRC_ACTIVE",
            "receipt_recheck": "2026-09-07 22:06心跳：OFF/ON metrics JSON可读，自哈希复算一致；两份收据的optimizer/scheduler均6000、2GPU/global batch2和clean tree绑定不变，epoch59 EMA文件均存在。数值仍62.0768%/54.8096%，未重跑评测",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 22:22 CST：恢复PRECHECK1276876的80tests/CUDA FP32/六臂真实恢复验证已通过，不重复。A1=1277406_2于22:12:30获得GPU，从原1570a725 epoch39/4000恢复；22:19:29完成正式epoch40，新增optimizer/scheduler/EMA各100更新、scheduler总计4100，本轮AMP skip/非有限loss/replay耗尽均0。新正式审计已写入且resume_lineage保留原SHA与4000恢复点，不是复用PRECHECK结果，现已进入epoch41。A6=1277407_7仍PENDING(AssocGrpGRES)。本轮新提交完整方案F=1277768_1，从原73bdd34a epoch29/3000更新恢复到新ce767b4d/formal/F，当前同样因AssocGrpGRES排队。A3/A4/A5与三个新eval仍待账户名额，旧六个失败job/log/checkpoint全保留",
            "receipt_recheck": "2026-09-07 22:06心跳：C0/A2两份metrics JSON可读且evaluation_sha256复算通过，仍分别59.2292%/54.2756%；seed8261、246058f2和epoch59 EMA身份未变，不属于完整Recovery结果",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-07 22:17 CST（GPU统计22:14，训练审计22:11）",
            ...
            "account_constraint": "22:14队列曾降为14项，本轮补交CT评测PRECHECK1277767与Evidence完整方案F1277768_1；22:17账户12 RUNNING/4 PENDING，共16项。Evidence A1已RUNNING，A6/F及CT评测PRECHECK因AssocGrpGRES排队。Evidence另三项续训/新eval及BAFDR NOKD/FULL后续尚无提交额度。公共17块未分配GPU不等于本账户可立即获得资源；不取消其他任务腾额度。实验挂载5.3T总量、401G可用、93%已用，旧实验未删除，配额解除原因仍未确认。历史1267920 FAILED、1267921 CANCELLED均只读观察",
        },
