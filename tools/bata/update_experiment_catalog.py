...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:10 CST：六份F臂小收据mtime与数值未变，最高64.2265%；本地aac9df0f仍clean。复用既有自哈希及终态验证，不重复大权重/推理。F01-F06仍phase-off，TIA与匹配控制未修；官方原始1245842的68.73%仍为此前原始日志值，来源补证及独立复评未完成，不能用修改REF补位",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:10-15:12 CST：07383274独立评测1279211_0/1均COMPLETED(0:0)，分别14:49:56/14:50:58结束，耗时19:21/20:23。两份新自哈希receipt核验通过，绑定fe1c53db、seed3407、epoch59 EMA、真实6000/337个Adam状态、211视频；官方独立Avg-mAP为G0均匀64.2720%、G1动态61.6740%，mAP@0.7为42.6802%/39.9154%。动态相对均匀低2.5980pp，单种子不支持提升，不等于所有动态路线无效。G2/G3仍是78cde1be旧身份结果，geometry四臂均B-AMoD关闭；本轮未新增作业或重复大权重/准入",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:08心跳：实时本地HEAD仍793c4f9c且clean，无新增机制实现；真实Taylor P0/P1训练接线、合法one-swap与H65 retention/transition仍未完成。保持BLOCKED_UNIMPLEMENTED，不将缺失机制归因于GPU排队，不重复全源码审查或占位配置准入",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:10-15:12 CST：U16/LATE/NOKD与FULL PRECHECK1277955仍COMPLETED(0:0)，前三份stdout终行均保存epoch59/update6000，stderr无新fatal；复用04:28真实权重计数验证。710ce8a6输出metrics.json仍0份，FULL无正式训练。padding/screen问题尚未修复，不能开启新FULL或21-cell；历史1267920/1267921未操作",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:10 CST：OFF/ON两份小收据mtime及指标未变，仍62.0768%/54.8096%；本地67d7079d仍clean，复用原74473c27训练和67d7079d评测终态认证，不重复大权重或推理。anchor correction及实际精度核对未完成，保留该低秩残差代理版本的负结果，不称已落地event gate或exact JVP",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:10 CST：A1/A6/F训练及A1评测均COMPLETED(0:0)，stdout/stderr终段已读，无新增运行失败。A1小收据mtime未变，metrics.average_mAP仍0.5159681679375281；A6/F仍无独立metrics。复用04:30终态/跨SHA恢复验证；e0c88c9a本地clean但只是初始化修复，utility/robust/cycle未接线，旧checkpoint不是完整修复版，不补提已知缺陷的A3/A4/A5",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-08 15:10 CST",
            "public_gpu_available": 0,
            "public_gpu_total": 200,
            "gpu_count_basis": "15:10 scontrol show nodes，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING后25个可调度GPU节点CfgTRES200、AllocTRES200；公共未分配0，不等于本账户排队/额度单独获批",
            "user_jobs_in_queue": 11,
            "account_constraint": "15:10展开数组为9 RUNNING/2 PENDING；本任务登记的CT评测已完成，队列其他任务不自动纳入六路线。1279307因Priority排队，1278774为JobHeldUser，均未操作。实验盘182G可用、97%已用；本轮无新增提交、取消、重提或热改，历史BAFDR1267920/1267921保持只读",
        },
        ...
        "heartbeat_evidence": "26_HEARTBEAT_EVIDENCE_20260908_1508.json",
        "performance_cause_audit_scope": "本轮真实触发2026-09-08 15:08:05.401 CST，实际远端采样15:10:16至15:12:20；六路线本地身份与clean状态、Slurm、stdout/stderr、旧小收据和资源均复查。新CT G0/G1独立评测已完成，自哈希receipt与211视频/6000更新身份核验通过，新增64.2720%/61.6740%终态结果；其余未完成机制和缺失评测保持明示。本轮不重复已认证大checkpoint、准入或模型全量审查，无新作业或修复提交",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。本轮收割既有1279211_0/1的独立终态结果并核验收据；没有新提交、重复准入或取消旧任务。分钟dispatcher仍未自动提交当前修复队列。",
        "",
