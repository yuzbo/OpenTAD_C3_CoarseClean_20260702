...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "result_status": "正式训练进行中：REF-D768/REF-U384 已进入 epoch 33，REF-MNV3FC384 已进入 epoch 31；尚无终态性能",
        ...
        },
        {
            ...
            "result_status": "四臂正式训练进行中：G0 已进入 epoch 34、G1 已进入 epoch 35、G2/G3 已进入 epoch 37；尚无终态性能",
        ...
        },
        {
            ...
            "deployment_status": "真实 CUDA/focused 门禁 1267855 已通过；D160 seed 4407 教师 1267884 正在运行，已保存 epoch 54/update 5500 checkpoint；成功后按分钟提交 G96、U16、LATE、NOKD、FULL",
            "result_status": "教师正式训练进行中，学生五臂尚未提交，无终态性能",
            "final_result": "保留 1267818 的 /bin/sh wrap 失败；539287fa 已改为 bash -lc 并重跑门禁；efe69d2e checkpoint 由独立任务按旧 SHA 另行评测，不能归入本提交",
            "next_action": "完成同 SHA、同 seed 的 D160 epoch-59 EMA 教师后开放五臂 screen",
        },
        {
            "category": "correction_route",
            ...
            "result_status": "双臂均已完成 epoch 25，正在进行周期验证；最近一次 epoch-23 验证中 OFF Avg-mAP 59.71%、ON 51.09%，仍不是终态性能",
            "final_result": "epoch-23 阶段验证 OFF/ON 的 tIoU 0.3/0.4/0.5/0.6/0.7 分别为 76.98/71.58/62.87/51.23/35.91% 与 69.13/62.21/53.72/41.87/28.53%；当前 loss 有限，需等待 epoch-59 EMA 才能裁决",
        ...
        },
        {
            ...
            "deployment_status": "actual-ledger admission 1267969 已通过（29 passed、三份 ledger 扫描、CUDA FP32）；C0 seed 8261 正式训练 1267979 正在运行，流水线 PID 2400024 等待 QOS 槽后提交 afterok 评估",
            "result_status": "C0 已完成 epoch 0-21并进入 epoch 22，累计 2200 次成功更新，尚无终态性能",
            "final_result": "最新审计显示 available 219/exposed 100、successful/EMA/scheduler updates 均为 2200、selector schedule 为 0、nonfinite=0；尚无 epoch-59 EMA 或 official mAP",
