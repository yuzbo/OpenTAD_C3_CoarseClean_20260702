...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "branch": "codex/zoomtoken-bafdr-admission-fix-20260903（训练） + codex/zoomtoken-bafdr-eval-pipeline-repair-20260904（评测）",
            "sha": "539287fa8a035765afd7e79863ce77278bef83f2（训练） / 02d34e6b146c62df0300007d75019a6c665ef2cf（评测）",
            "github_commit": "https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/02d34e6b146c62df0300007d75019a6c665ef2cf",
            "local_directory": "E:/DeskTop/TAD/_duca_fix_worktrees/bafdr_admission",
            "deployment_status": "真实 CUDA/focused 门禁 1267855 已通过；D160 教师 1267884、G96 1268698、U16 1269124、LATE 1269129、NOKD 1269137、FULL 重试 1269297 均 COMPLETED(0:0)，全部写出 6000-update epoch-59 receipt；screen_receipt_r2_1269297.json 精确绑定成功 FULL 并为 PASS。评测 1269382 因 PROJECT_DIR 解析错误失败；1269383 完成 G96 prediction 后在 U16 validation 因生成配置错误请求不存在的 gt_segments 失败。独立修复 02d34e6b 删除 val/test 标签依赖并分离训练/评测 SHA，远端静态测试 11 passed、21-cell validator PASS；双 GPU U16 PRECHECK 1269388 COMPLETED(0:0)，五臂正式评测 1269389 已自动启动",
            "result_status": "D160 与五个 screen 臂训练终态有效，screen gate PASS；seed 4407 五臂官方评测在修复 SHA 上运行中，尚无开放性能",
            "final_result": "FULL receipt 绑定训练 539287fa、seed 4407、world_size 2、6000 successful updates、D160 teacher 和 checkpoint SHA256=c0eb8677…；评测器固定为 02d34e6b，screen receipt 仍严格验证训练身份 539287fa。现有训练 receipt 的 eval_results 为空且 metric_opened=false，1269389 完成 prediction 与 metric-opening receipts 前不得报告 mAP",
            "next_action": "监控 1269389 依次完成 G96、U16、LATE、NOKD、FULL 的 prediction-only 与 official metric opening；若再次失败，先读当前臂日志，再按独立修复 SHA 和新作业命名空间处理",
        },
        {
            "category": "correction_route",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 05:57:08 CST",
            "public_gpu_available": 121,
