...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "真实 CUDA/focused 门禁 1267855 已通过；D160 seed 4407 教师 1267884 正在运行，成功后按分钟提交 G96、U16、LATE、NOKD、FULL",
        ...
        },
        {
            ...
            "deployment_status": "修复版 admission/C0 每分钟流水线 PID 2400024 正在等待 QOS 提交名额；队列降到限制后自动先跑 actual-ledger admission，再提交 C0 seed 8261 与 afterok 评估",
            "result_status": "等待重提，尚无终态性能",
            "final_result": "保留 1267747 的 219-vs-100 loader 合同失败和 1267857 的 legacy ledger policy 字段错配；77c8d173 已修复两者并把真实三份 ledger schema 纳入 admission",
            "next_action": "等待每分钟提交器取得名额并通过 admission；C0 首个 100-update epoch 成功后再逐步释放其余七臂",
        },
    ]
