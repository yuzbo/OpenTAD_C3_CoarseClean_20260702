...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "receipt_recheck": "2026-09-07 21:07 CST：六份F01-F06 metrics JSON可读，evaluation_sha256逐份复算一致；按收据中的实际F01-F06路径确认六个epoch59 checkpoint仍存在。代理把F0N占位符当文件名产生的不存在判断已纠正。未重复推理或改写指标",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 21:22 CST：修复版1276669/1276670/1276671/1276672四臂全部RUNNING，G0/G1/G2/G3已完成epoch35/35/34/33，optimizer/scheduler/EMA各自同步为3600/3600/3500/3400，AMP skips为3/3/4/3且已重放补齐。当前命名空间stdout/stderr未发现Traceback、RuntimeError或磁盘配额错误。另直接读取G2 epoch29权重，337个AdamW state均3000步，与metadata/scheduler一致。新增独立终态评测11ced13a，不改opentad、configs或训练；正式模式核验epoch59 EMA和真实6000步后调用原官方评估器。GPU评测预检尚待额度，未提前产生正式结果",
            ...
            "evaluator_validation": "11ced13a本地43 passed/1 CUDA skipped，独立有限审阅未发现阻断；远端clean exact SHA同样43 passed/1 CUDA skipped，CTDP_EVAL_SOURCE_OK。已定位原G0-G3六更新预检权重，GPU单批评测PRECHECK未提交：账户16项达到额度。没有新官方mAP或正式receipt",
            "evaluator_precheck_job": None,
            ...
            "formal_job_states": {"G0": "RUNNING; 3600 successful updates", "G1": "RUNNING; 3600 successful updates", "G2": "RUNNING; 3500 successful updates", "G3": "RUNNING; 3400 successful updates"},
            "observed_successful_updates": {"G0": 3600, "G1": 3600, "G2": 3500, "G3": 3400},
            ...
            "next_action": "监督1276669-1276672，不重跑已通过的训练admission或重提现有四臂。额度释放后按evaluator_precheck_launch提交11ced13a的单GPU四臂单批评测PRECHECK；它只读取既有epoch1/6更新预检权重，不打开指标。通过后用同一脚本PRECHECK_ONLY=0、array0-3逐臂afterok对应训练，CTDP_TRAIN_ROOT改为repair_run_root/formal，CTDP_EVAL_ROOT为ctdp_terminal_eval_11ced13a/formal。预期receipt为formal/gN/gpu1_id0/official_eval/metrics.json，不把训练telemetry改称最终性能",
        },
        {
            "category": "correction_route",
            "name": "DUCA-Unified 当前 41 单元正交消融控制台",
            "internal_id": "DUCA_UNIFIED_ACTIVE",
            "current_cycle": "2026-09-07本轮未修改Unified，目录生成时再次读取其本地HEAD/clean状态。此前定位的Taylor P0/P1运行时接线及H65 original retention/transition缺失仍未修复，不能由其他路线的测试通过替代。本轮完成Evidence两臂续训重提和CT-DP独立评测接线，Unified仍是明确的实现欠账，而不是资源排队",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 21:22 CST：710ce8a6的新D160教师1276650与G96全局低清1276675均COMPLETED(0:0)，各有epoch59 EMA。已直接加载两份checkpoint，337个已初始化AdamW参数的step全部6000，scheduler/EMA审计也6000；各4次AMP skip已重放，非有限loss和replay耗尽均0。U16均匀分块1276842运行到epoch4/500更新，晚融合LATE1276843到epoch5/600更新，各3次AMP skip已恢复，未发现新运行错误。五份旧sealed收据保留可读，但旧训练5994-5997次更新仍协议不合格。少3-6次更新不足以解释全部低分，教师/KD等静态检查未发现能解释大幅退化的确定缺陷；不宣称性能原因已查清",
            ...
            "next_action": "D160/G96已完成，不重训；监督U16/LATE。额度释放后补交已通过PRECHECK的NOKD，并以新D160的epoch59 EMA明确绑定Teacher配置/权重，完成FULL自身PRECHECK后正式训练。新G96及其他学生完成后安排独立官方评测。D160新教师位于repair_run_root/work_dirs/bafdr_k16_d160_seed4407/checkpoint/epoch_59.pth；不得复用旧5996步教师或旧五臂分数",
            ...
            "repair_validation": {"commit": "710ce8a6246c471742c83bf7c180d9ab87c36fac", "local": "41 passed, 1 Windows Torch skipped", "remote_exact_clean_linux": "44 passed, 1 CUDA skipped", "cuda_and_two_gpu_precheck": "1276618 COMPLETED(0:0): 45 tests passed; D160/G96/U16/LATE/NOKD各两轮三批，BAFDR_SUCCESSFUL_UPDATE_PRECHECKS_OK", "formal_retraining": "21:22 CST: D1601276650/G961276675均epoch59、实际AdamW6000、scheduler/EMA6000；U161276842为500、LATE1276843为600更新，健康运行。NOKD未提交；FULL新教师已就绪，但其专用PRECHECK/训练仍待额度"},
            ...
            "observed_successful_updates": {"D160": 6000, "G96": 6000, "U16": 500, "LATE": 600},
        },
        {
            "category": "correction_route",
            "name": "ET-TRC 当前双臂：完整 Transformer 与局部 Taylor/JVP 近似",
            "internal_id": "ET_TRC_ACTIVE",
            "receipt_recheck": "2026-09-07 21:07 CST：OFF/ON metrics JSON可读，自哈希复算一致；两份收据的optimizer/scheduler均6000，epoch59 EMA文件均存在。数值仍62.0768%/54.8096%，未重跑评测",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-07 21:22 CST：恢复验证1276876于20:35:36 COMPLETED(0:0)，80tests、CUDA FP32门禁及六臂真实恢复各100更新全部通过。六个新增epoch均无AMP skip、非有限loss或replay耗尽；A1/A6从4000验证到4100，F/A3/A4/A5从3000到3100，均保留resume_precheck标记，不是正式6000结果。核验后已提交A1=1277406_2、A6=1277407_7，两者从原epoch39/4000恢复而非使用预检权重；当前因AssocGrpGRES排队。F/A3/A4/A5及两项新eval因账户16项满额尚未提交。旧六个存储失败job/log/checkpoint全保留，未远端热改",
            "receipt_recheck": "2026-09-07 21:06 CST：C0/A2两份metrics JSON可读且evaluation_sha256复算通过，仍分别59.2292%/54.2756%；不属于完整Recovery结果",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-07 21:22 CST",
            "public_gpu_available": 17,
            "public_gpu_total": 200,
            "gpu_count_basis": "scontrol show nodes可见gpu分区，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING节点后25节点CfgTRES为200、AllocTRES为183，不沿用旧快照240作为分母",
            "user_jobs_in_queue": 16,
            "account_constraint": "账户12 RUNNING/4 PENDING，共16项；新Evidence A1/A6因AssocGrpGRES排队，CT评测GPU预检、Evidence其余四项续训及评测、BAFDR NOKD/FULL后续尚无提交额度。公共17块未分配GPU不等于本账户可立即获得资源；不取消其他任务腾额度。实验挂载5.3T总量、416G可用、93%已用，旧实验未删除，配额解除原因仍未确认。历史1267920 FAILED、1267921 CANCELLED均只读观察",
        },
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。H65本协议参考臂/F01-F06、Evidence C0/A2及ET-TRC OFF/ON已有可追溯的官方终态数值，本轮收据再次复核，原始约69/65基线不与低分内部参考混用。BAFDR旧五臂分数仍仅诊断；新D160/G96已实际6000更新完成，U16/LATE运行，NOKD和FULL后续待额度。CT-DP四臂运行；新独立evaluator11ced13a已通过本地/远端CPU，GPU评测PRECHECK尚未提交。Evidence六臂恢复PRECHECK1276876已全部通过，新A1/A6正式续训1277406_2/1277407_7排队，另四臂和新独立eval仍待额度。DUCA-Unified缺失机制尚未实现。收据完整不等于官方配方复现、模型无错或机制有效；不从预检或中期训练推导最终mAP。",
        "",
    ]
    return "\n".join(lines)
