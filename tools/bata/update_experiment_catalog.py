...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "远端 exact-SHA admission 1267684 已通过；REF-D768、REF-U384、REF-MNV3FC384 正式训练 1267709/1267711/1267737 正在运行；原依赖评估 1267710/1267712/1267738 为释放 QOS 槽已取消，watcher PID 2420586 会在训练成功后按分钟重提",
            "result_status": "正式训练进行中：REF-D768/REF-U384 已进入 epoch 48，REF-MNV3FC384 已进入 epoch 46；尚无终态性能",
            "final_result": "已修复短窗口 padding 后 K=383 与输入 K=384 不一致；三臂 loss 有限且无 Traceback/OOM/非有限值，当前无 epoch-59 EMA 或官方 mAP",
            "next_action": "持续核验成功更新、有限 loss、终态 EMA；训练成功后自动运行官方 THUMOS14 评估",
        },
        {
            "category": "correction_route",
            ...
            "deployment_status": "远端 admission 1267221 已通过；G0、G1、G2、G3 正式训练 1267229-1267232 正在运行",
            "result_status": "四臂正式训练进行中：G0 已完成 epoch 41，G1 已进入 epoch 43，G2/G3 已完成 epoch 43；部分作业正在周期推理，尚无终态性能",
            "final_result": "远端 26 项测试与 CUDA 几何/梯度门禁通过；四臂 loss 有限且错误日志无异常，尚无 epoch-59 EMA 或官方 mAP",
        ...
        },
        {
            ...
            "deployment_status": "真实 CUDA/focused 门禁 1267855 已通过；D160 seed 4407 教师 1267884 已完成，G96 正式训练 1268698 正在运行并已保存 epoch 14/update 1500；U16、LATE、NOKD、FULL 等待流水线释放",
            "result_status": "D160 已完成 6000 次成功更新并生成 epoch-59 checkpoint 与训练 receipt；G96 训练中，尚无当前路线终态性能",
            "final_result": "D160 receipt 绑定 539287fa、seed 4407、6000 updates 与 checkpoint SHA256；保留 1267818 的 /bin/sh wrap 失败；efe69d2e checkpoint 仍由独立任务另行评测，不能归入本提交",
            "next_action": "等待 G96 训练/筛选完成，并由现有流水线依资源顺序释放 U16、LATE、NOKD、FULL",
        },
        {
            "category": "correction_route",
            ...
            "result_status": "双臂正式训练进行中：OFF 已进入 epoch 34，ON 已完成 epoch 33 并正在周期推理；最新中期验证 OFF Avg-mAP 62.97%、ON 54.87%，仍不是终态性能",
            "final_result": "最新中期验证 OFF/ON 的 tIoU 0.3/0.4/0.5/0.6/0.7 分别为 78.69/73.90/65.20/55.36/41.71% 与 71.43/65.44/57.57/46.83/33.05%；当前 loss 有限，需等待 epoch-59 EMA 才能裁决",
        ...
        },
        {
            ...
            "deployment_status": "actual-ledger admission 1267969 已通过（29 passed、三份 ledger 扫描、CUDA FP32）；C0 seed 8261 正式训练 1267979 正在运行，afterok 官方评估 1268680 已提交并等待依赖",
            "result_status": "C0 已完成 epoch 0-34 并进入 epoch 35，累计 3500 次成功更新，尚无终态性能",
            "final_result": "最新审计显示 available 219/exposed 100、successful/EMA/scheduler updates 均为 3500、selector schedule 为 0、nonfinite=0；尚无 epoch-59 EMA 或 official mAP",
            "next_action": "持续核验 6000 成功更新与终态 EMA；C0 稳定后再逐步释放其余七个消融臂",
        },
    ]
