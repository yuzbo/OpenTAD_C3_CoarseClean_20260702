...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 12:27 CST：六份F臂小收据mtime与数值未变，最高64.2265%；复用既有自哈希及终态身份验证，不重复大权重加载或推理。F01-F06仍phase-off，TIA与匹配控制未修。官方原配方1245842的68.73%仍是此前日志值，预训练/数据身份和独立复评待补，修改REF不能替代官方锚点",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 12:27 CST：新fe1c53db G0/G1均RUNNING，已运行约9小时33分；落盘audit为epoch53/54、optimizer/scheduler/EMA各5400/5500，完成90.0%/91.7%，AMP skip均3且已重放。G0到epoch54 batch50，loss0.4269；G1结束epoch55 batch99，loss0.4402，训练内评测到226批；无epoch59或新fatal。按每两轮约39-40分钟估计G0于14:10-14:45、G1于13:50-14:20结束训练，独立评测另需30-60分钟及排队。复用07383274既有预检；无新正式mAP，旧G2/G3沿用原身份56.5234%/57.8489%",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 12:25心跳：本地身份由目录生成器刷新，HEAD仍793c4f9c且clean，无新增实现提交。真实Taylor P0/P1、合法one-swap及H65 retention/transition仍未接通，保持BLOCKED_UNIMPLEMENTED。这是实现欠账，不是资源排队；本轮不重复全源码审查",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 12:27 CST：U16/LATE/NOKD及FULL PRECHECK1277955均COMPLETED(0:0)，前三臂终轮日志仍为epoch59/update6000，最新stderr无新fatal；复用04:28实际权重验证。710ce8a6输出仍无metrics.json，FULL只有PRECHECK。padding/screen问题未修，不开放正式FULL或21单元；历史1267920/1267921未操作",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 12:27 CST：OFF/ON两份小收据mtime及数值未变，仍62.0768%/54.8096%；复用既有自哈希与原74473c27训练/67d7079d评测终态认证，不重复大checkpoint或推理。anchor correction及实际精度核对未完成，保留旧负结果",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 12:27 CST：A1/A6/F训练及A1评测均COMPLETED(0:0)，读取stdout/stderr终段未见新运行失败；A1小收据mtime未变，直接读取metrics.average_mAP为0.5159681679375281。A6/F仍无独立metrics。复用04:30终态及跨SHA恢复验证；utility/robust/cycle缺失仍阻止A3/A4/A5扩展，旧权重不是e0c88c9a初始化修复版",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-08 12:27 CST",
            "public_gpu_available": 27,
            "public_gpu_total": 200,
            "gpu_count_basis": "12:27 scontrol show nodes，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING后25个可调度GPU节点CfgTRES200、AllocTRES173；公共未分配27，不等于本账户额度",
            "user_jobs_in_queue": 8,
            "account_constraint": "12:27展开数组为7 RUNNING/1 PENDING，其中本任务CT G0/G1占2项；1278774为未登记的其他任务JobHeldUser，不解除其hold或视为本路线资源失败。实验挂载209G可用、97%已用，公共空间不保证个人配额。未提交、取消或重提作业，历史BAFDR1267920/1267921只读不动",
        },
        ...
        "heartbeat_evidence": "22_HEARTBEAT_EVIDENCE_20260908_1225.json",
        "performance_cause_audit_scope": "本轮触发2026-09-08 12:25:32.886 CST，实际远端采样12:27，核查六路线代码身份、Slurm、CT真实audit及日志、已有小收据与资源。G0/G1落盘5400/5500，无新终态；没有新正式mAP、模型修复或Slurm提交，不重复大checkpoint、已认证哈希及已通过准入。未执行或只完成部分工作的历史触发不补记为完整检查",
