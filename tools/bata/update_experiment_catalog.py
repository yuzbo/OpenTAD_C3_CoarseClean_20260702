...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:53 CST：六份F臂小收据mtime和指标未变，最高64.2265%；复用此前终态/自哈希认证，没有重复大checkpoint或推理。F01-F06仍phase-off，TIA/匹配控制未修。原始官方1245842日志68.73仍待来源补证及独立复评，不用修改REF替代",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:53 CST：G0/G1训练1278011/1278012与独立评测1279211_0/1均保持COMPLETED(0:0)，epoch59 audit仍6000/6000/6000，checkpoint大小/mtime及两份receipt数值未变。复用14:29实际337个Adam状态认证与15:10新收据自哈希验证；G0/G1 Avg-mAP仍64.2720%/61.6740%，mAP@0.7为42.6802%/39.9154%。没有新作业或失败，不重复评测。后续Pro入口检查因Computer Use的kernel assets路径错误阻塞，未发送材料或获得Pro意见",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:51心跳：本地身份由生成器实时刷新；真实Taylor P0/P1训练接线、合法one-swap和H65 retention/transition仍未完成，保持BLOCKED_UNIMPLEMENTED。没有新模型实现或正式结果，不将实现欠账写成资源排队，也不重复全源码审查",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:53 CST：U16/LATE/NOKD及FULL PRECHECK1277955仍COMPLETED(0:0)，前三stdout终行仍为epoch59/update6000，stderr无新fatal。复用04:28真实终态验证；710ce8a6新metrics仍0份，FULL无正式训练。padding/真实终态screen仍未修复，不扩旧SHA矩阵，历史1267920/1267921未操作",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:53 CST：OFF/ON小收据mtime及指标未变，62.0768%/54.8096%；复用既有74473c27训练/67d7079d评测及终态认证，不重跑推理。anchor correction和实际精度核对尚未完成，继续保留fixed-stride低秩代理负结果，不能称event gate或exact JVP已实现",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 15:53 CST：A1/A6/F训练和A1评测均COMPLETED(0:0)，stdout/stderr未见新fatal；ce767b4d输出仍仅有A1一份metrics，metrics.average_mAP=0.5159681679375281且mtime未变，A6/F独立评测仍缺失。复用04:30真实更新/恢复认证；utility/robust/cycle仍缺失，不将e0c88c9a初始化修复或旧权重当完整实现，不补提旧SHA缺陷矩阵",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-08 15:53 CST",
            "public_gpu_available": 1,
            "public_gpu_total": 200,
            "gpu_count_basis": "15:53 scontrol show nodes，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING后25个可调度GPU节点CfgTRES200、AllocTRES199，公共未分配1；不等于本账户配额或额外任务必然可提交",
            "user_jobs_in_queue": 6,
            "account_constraint": "15:53展开数组为5 RUNNING/1 PENDING，共6项，均不自动纳入本次六路线登记。1278774仍JobHeldUser，未操作。实验盘179G可用、97%已用。没有新增提交、取消、重提或热改，历史BAFDR1267920/1267921保持只读",
        },
        ...
        "heartbeat_evidence": "27_HEARTBEAT_EVIDENCE_20260908_1551.json",
        "performance_cause_audit_scope": "本轮真实触发2026-09-08 15:51:06.015 CST，实际远端采样15:53:03至15:53:09；复查六路线代码身份、登记Slurm、日志终段、CT audit/checkpoint stat、各小收据与资源，未发现新运行失败或性能变化。复用已认证终态和自哈希，不重复大权重/准入。额外尝试指定Pro的Computer Use入口，因kernel assets路径错误阻塞，未访问账号或传送材料；不冒称已咨询",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。本轮既有结果未变，无新提交、重复准入或取消旧任务。指定Pro的Computer Use入口因运行环境初始化失败而阻塞，未完成咨询；分钟dispatcher仍未自动提交当前修复队列。",
        "",
