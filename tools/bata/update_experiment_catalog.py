...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "result_status": "seed 4407 的 D160 Teacher、五个 screen 臂训练、screen gate、五臂 prediction 与 official metric receipts 全部完成；该单种子路线已有完整正式结果",
            ...
            "next_action": "保留五份 sealed eval receipt 作为 seed 4407 正式结果；如论文需要总体结论，再按同一合同运行其余预注册种子并做配对统计，不重复运行已完成的 seed 4407",
        },
        {
            "category": "correction_route",
        ...
        },
        {
            ...
            "branch": "codex/duca-evidence-fullgrid-repair-20260904（C0/F/A1-A5） + codex/duca-evidence-a6-numerical-repair-20260904（A6）",
            "sha": "246058f2c24edc78818ada60eec26249bbf7d5d2（C0/F/A1-A5） / 9739811e89acef380806a271bc224c61c66bce0f（A6 修复）",
            "github_commit": "https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/9739811e89acef380806a271bc224c61c66bce0f",
            ...
            "deployment_status": "246058f2 使用 train/val 精确旧网格与 test full-grid，admission 1269357 COMPLETED(0:0)；C0 seed 8261 训练 1269374_0 与官方评测 1270672_0 均 COMPLETED(0:0)，metrics.json 已绑定 exact SHA、config/evaluator/annotation/class-map、epoch 59 EMA checkpoint/prediction SHA 与自哈希。同 seed 的 F/A1-A5 在数组 1270870_1-6 中健康运行，截至 14:34 CST 达到 epoch 14-15，未见 traceback/non-finite/NaN/OOM。A6 的 1270870_7 获得资源后在 batch 2 连续 50 次确定性 replay 均产生 pre-AMP non-finite cost，于 13:51 CST FAILED(1:0)；日志定位到 H65 replay 与完整 time/merge/recovery 组合。独立修复 9739811e 将 bounded interval decomposition 与 dense recovery accumulation 固定为 FP32，同时保留全部 A6 机制，并新增真实 ledger/真实视频前三 batch PRECHECK。35340cad admission 1270978 已通过 36 tests 和 CUDA FP32 gate；首次 PRECHECK 1270986 在模型构建前因误继承 6000-update formal 标志失败，9739811e 已修正该预检合同。最终 exact-SHA admission 1271006 因 AssocGrpGRES 等待；PRECHECK 与正式 A6 尚未提交，原因是 AssocMaxSubmitJobLimit，而不是忽略失败",
            "result_status": "C0 seed 8261 已有完整正式结果；F/A1-A5 正式训练运行中；A6 旧实现失败，修复版正在等待 exact-SHA admission/PRECHECK，完整 Evidence 机制结论尚未形成",
            "final_result": "C0 MATCHED_H65_60 official Avg-mAP=59.23%，mAP@0.3/0.4/0.5/0.6/0.7=75.22/70.01/61.54/50.90/38.48%。test 的 792 次暴露对应 791 个唯一物理窗口，重复项是 video_test_0001431|7680；246058f2 按唯一物理窗口严格覆盖。该结果是 matched-H65 基线，不是完整 recovery 机制；只有 F/A1-A6 完成后才能判断 recovery 增益",
            "next_action": "监控 F/A1-A5 到 6000 successful updates；待 1271006 通过并释放提交名额后，先提交 9739811e 的三 batch A6 PRECHECK，通过后才以新命名空间重提 A6。逐臂完成后按 exact-SHA evaluator 合同生成 receipt",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 14:34 CST",
            "public_gpu_available": 34,
            "public_gpu_total": 240,
            "user_jobs_in_queue": 16,
            "account_constraint": "公共口径 34/240 可用，但本用户 14 RUNNING、2 PENDING；A6 admission 因 AssocGrpGRES 等待，追加提交受 AssocMaxSubmitJobLimit 限制",
        },
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。当前 H65 D768、U384、MNV3FC384 参考臂、BAFDR G96/U16-UNIFORM-A0/LATE/NOKD/FULL 和 Evidence C0 具有合法终态 evaluator metrics；其余路线或比较臂仍为 `NO_FINAL_PERFORMANCE_RESULTS`。终态训练 receipt 只能证明训练合同完成，不得从 admission 或中期验证推导最终 mAP 或机制结论。",
        "",
    ]
    return "\n".join(lines)
