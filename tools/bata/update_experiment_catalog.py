...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "2026-09-09 06:06核查：训练1280127仍COMPLETED(0:0)，12次完整测试/best/协议收据有效，best匹配未四舍五入最大值和最早同分规则；远端629162cd exact/clean，终态及best仍在。全局optimizer/scheduler/EMA/DUCA schedule审计均6000，复用36号实际权重核验，不重复加载大checkpoint；独立终态评测及少步参数归属待补",
            "result_status": "60轮训练与12次全测试完成；测试集选模最佳及训练内终轮成绩可报告，独立终态receipt仍待补",
            "final_result": "完整THUMOS14测试集211视频/792窗口：第5轮 5.2404%；第10轮 30.9742%；第15轮 38.1240%；第20轮 49.0216%；第25轮 56.7257%；第30轮 60.7207%；第35轮 62.5658%；第40轮 63.4838%；第45轮 63.8428%；第50轮 63.9646%；第55轮 63.9520%；第60轮 63.8596%；最佳第50轮 63.9646%，最佳对应mAP@0.7 41.7472%。第60轮训练内EMA评测 63.8596%，mAP@0.7 42.0630%。12份周期与best收据有效；未变best复用已有实际EMA认证。全局6000与参数实际参与计数分开记录，不把scheduler替代optimizer证据。新SHA独立terminal receipt仍缺；测试集参与选模/调参，不称未见测试泛化或原论文公平复现。",
            ...
            "test_guided_hyperparameter_trials": [],
            "next_action": "保留全部12次曲线、最佳与终轮EMA，补629162cd独立终态评测绑定，并核对少步参数的模块归属；四相开启终轮比关闭低6.7774点，优先定位该匹配配方下的分配/学习机制。不得因跑完就宣称方法有效，也不盲目重训或按同SHA热改；任何测试指导调参必须保留全部尝试。",
        ...
        },
        {
            ...
            "deployment_status": "2026-09-09 06:06核查：训练1280128仍COMPLETED(0:0)，12次完整测试/best/协议收据有效，best匹配未四舍五入最大值和最早同分规则；远端629162cd exact/clean，终态及best仍在。全局optimizer/scheduler/EMA/DUCA schedule审计均6000，复用36号实际权重核验，不重复加载大checkpoint；独立终态评测及少步参数归属待补",
            "result_status": "60轮训练与12次全测试完成；测试集选模最佳及训练内终轮成绩可报告，独立终态receipt仍待补",
            "final_result": "完整THUMOS14测试集211视频/792窗口：第5轮 5.4013%；第10轮 30.8467%；第15轮 38.9224%；第20轮 49.1960%；第25轮 55.8911%；第30轮 60.2283%；第35轮 62.1153%；第40轮 63.2568%；第45轮 63.5516%；第50轮 63.4065%；第55轮 63.7444%；第60轮 64.2632%；最佳第60轮 64.2632%，最佳对应mAP@0.7 43.0885%。第60轮训练内EMA评测 64.2632%，mAP@0.7 43.0885%。12份周期与best收据有效；新best实际EMA认证通过。全局6000与参数实际参与计数分开记录，不把scheduler替代optimizer证据。新SHA独立terminal receipt仍缺；测试集参与选模/调参，不称未见测试泛化或原论文公平复现。",
        ...
        },
        {
            ...
            "deployment_status": "2026-09-09 06:06核查：训练1280129仍COMPLETED(0:0)，12次完整测试/best/协议收据有效，best匹配未四舍五入最大值和最早同分规则；远端629162cd exact/clean，终态及best仍在。全局optimizer/scheduler/EMA/DUCA schedule审计均6000，复用36号实际权重核验，不重复加载大checkpoint；独立终态评测及少步参数归属待补",
            "result_status": "60轮训练与12次全测试完成；测试集选模最佳及训练内终轮成绩可报告，独立终态receipt仍待补",
        ...
        },
        {
            ...
            "deployment_status": "2026-09-09 05:04再次读取stdout/stderr及sacct：signed1280314/control链1280316仍FAILED(1:0)，epoch16 AMP replay后p_action非有限。没有本登记命名空间的fulltia正式启动/恢复证据；由原任务修复，旧job/log/checkpoint保留，本监督未接管、删除校验或盲目重提",
            ...
            "open_failure": {"state": "OPEN_NUMERICAL_ROOT_CAUSE_PENDING", "jobs": ["1280314", "1280316"], "source_sha": "96e5b3a5e9d77d65b7a0afca86277261dd63295b", "throw_site": "opentad/models/duca/acquisition.py:2042", "message": "p_action calibration check received non-finite values", "last_persisted_successful_updates": 1600, "logs": [{"job": "1280314", "stdout": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/signed_1280314.out", "stderr": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/signed_1280314.err"}, {"job": "1280316", "stdout": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/control_fulltia_1280316.out", "stderr": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/control_fulltia_1280316.err"}], "retry_submitted": False, "owner_notified_via_thread_tool": False, "note": "Thread coordination tool unavailable in this turn; recorded in shared catalog, not claimed delivered to the owner.", "last_rechecked_at": "2026-09-09T05:04:40.534096+08:00"},
        },
        {
            "category": "related_task_repair",
            "github_commit": None,
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {"checked_at_cst": "2026-09-09 06:06 CST", "public_gpu_available": 66, "public_gpu_total": 200, "gpu_count_basis": "scontrol CfgTRES/AllocTRES，排除DOWN/DRAIN/FAIL/UNKNOWN/FUTURE/POWERED_DOWN；公共余量不保证个人即时配额", "user_jobs_in_queue": 2, "account_constraint": "账户1 RUNNING/1 PENDING，均为其他任务，held未操作；本轮ET评测已完成，五项新训练也已完成。实验盘120.98GiB可用；缺失机制和未评测路线不能解释为本任务GPU排队"},
        ...
        "heartbeat_evidence": "39_HEARTBEAT_EVIDENCE_20260909_0559.json",
        "performance_cause_audit_scope": "2026-09-09 06:04/06:06实际核验：ET独立评测1281049_0/1均完成，OFF61.7587%、ON54.5976%，新edfbd7e5原始收据与配置/预测/evaluator校验通过，六项mAP与训练内终轮完全一致。五项完整训练60份周期/best未变，H65四相和ET近似开启仍是负结果；CT当前最高64.2720%。本轮没有模型修改、新训练、调参或取消；H65独立收据、其他缺失机制及历史90轮失败继续明确保留",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。17:21/17:26由监督任务提交的5项新协议作业当前状态见独立行，部署身份见29号记录；本轮04:56触发，05:04检查确认五项训练均完成、60次全测试齐全且分数未变。本监督补齐edfbd7e5独立评测入口；1281042双GPU预检通过后，1281049_0/1于05:13正式运行，尚未形成独立receipt。历史90轮两项数值失败未恢复，见38号记录。没有新训练、取消或调参；新增评测由本监督提交，不是分钟dispatcher自动提交。已有Computer Use初始化阻塞仍保留，本轮未咨询Pro，也未取消旧任务。",
        "",
