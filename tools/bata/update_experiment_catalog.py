...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-09 02:38：新629162cd三项仍RUNNING，第55轮均匀/相位关/四相开63.9520%/63.7444%/57.5081%；最佳63.9646%/63.7444%/57.5081%。更新审计5700/5800/6000，AMP skip3/9/9且耗尽0。四相组已产生epoch59但未返回终轮成绩。另任务历史90轮signed/control因p_action非有限FAILED，单独登记，不与本任务新三项混同",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-09 02:22：G0/G1独立成绩仍64.2720%/61.6740%，原始收据自哈希有效；复用已有真实6000认证。后续机制矩阵eval5未接入，不把这两个几何控制当完整B-AMoD结论",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-09本地状态随目录刷新；后继0fcef0bf修复了时间轴，不代表Taylor P0/P1、合法one-swap和H65 retention/transition完成。无正式性能，不给未实现机制签发通过",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-09 02:22：710ce8a6五份终态仍存在，独立metrics文件仍0，FULL1277955只有PRECHECK。后继b142bffa padding修复仍待真实terminal screen/eval5和生产验证；旧不合格更新数成绩不恢复为新最终结果。未操作历史1267920/1267921",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-09 02:19-02:24：9a346f0d ON/OFF均COMPLETED(0:0)，最佳55.3617%/62.5093%，终轮训练内54.5976%/61.7587%。OFF实际337个optimizer state6000、scheduler6000、EMA有限；ON此前实际373 states6000认证保留。两组各12次全测试，独立terminal receipt均待补；未改参或重训",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-09 02:22：旧A1/A6/F终态存在，A1评测文件51.5968%未变，F/A6缺独立receipt；本次未将旧评测文件迁移为79aea08a修复版最终成绩。utility/真实two-view/cycle/eval5与生产验证仍未完成",
        ...
        },
        {
            ...
            "deployment_status": "2026-09-09 02:19核查：1280117 COMPLETED(0:0)，09-08 23:23:38结束、耗时6:02:07；全部12次完整测试已返回。远端exact SHA/clean，stdout/stderr无新增fatal",
            "result_status": "训练与12次全测试完成；最佳及终轮训练内成绩可报告，独立终态receipt尚缺",
            "final_result": "完整THUMOS14测试集211视频/792窗口：第5轮 5.7073%；第10轮 32.0496%；第15轮 42.1020%；第20轮 47.3332%；第25轮 51.8133%；第30轮 54.2127%；第35轮 55.3159%；第40轮 55.3617%；第45轮 55.2871%；第50轮 55.0948%；第55轮 54.8392%；第60轮 54.5976%；最佳第40轮 55.3617%，对应mAP@0.7 33.6320%。曲线及best自哈希有效，实际best EMA身份/数值已认证；测试集选模，不称未见测试泛化。第60轮训练内EMA官方评测 54.5976%，终态实际373个optimizer state均6000、scheduler6000、EMA有限。未存独立EMA调用计数，次数依据成功更新控制流推导；独立terminal evaluator receipt仍待补，不能称独立封存完成。",
        ...
        },
        {
            ...
            "deployment_status": "2026-09-09 02:22-02:28实际sacct/stdout/stderr：signed1280314 FAILED(1:0)，耗时2:29:52；control链1280316 FAILED(1:0)，耗时2:27:54。均于stage2 epoch16在AMP replay后由acquisition.py:2042拒绝非有限p_action。没有fulltia启动证据；旧job/log/checkpoint保留，不重复提交其他任务作业",
            ...
            "open_failure": {"state": "OPEN_NUMERICAL_ROOT_CAUSE_PENDING", "jobs": ["1280314", "1280316"], "source_sha": "96e5b3a5e9d77d65b7a0afca86277261dd63295b", "throw_site": "opentad/models/duca/acquisition.py:2042", "message": "p_action calibration check received non-finite values", "last_persisted_successful_updates": 1600, "logs": [{"job": "1280314", "stdout": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/signed_1280314.out", "stderr": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/signed_1280314.err"}, {"job": "1280316", "stdout": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/control_fulltia_1280316.out", "stderr": "/data/run01/sczc063/yuzibo/experiments/h65_matched_96e5b3a5_seed3407/logs/control_fulltia_1280316.err"}], "retry_submitted": False, "owner_notified_via_thread_tool": False, "note": "Thread coordination tool unavailable in this turn; recorded in shared catalog, not claimed delivered to the owner."},
        },
        {
            "category": "related_task_repair",
            "github_commit": None,
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {"checked_at_cst": "2026-09-09 02:22队列/磁盘，02:28节点", "public_gpu_available": 47, "public_gpu_total": 200, "gpu_count_basis": "scontrol CfgTRES/AllocTRES；排除DOWN/DRAIN/FAIL/UNKNOWN/FUTURE；公共余量不代表账户配额", "user_jobs_in_queue": 10, "account_constraint": "5 RUNNING/5 PENDING；本任务ET两项完成、H65三项运行，另一任务历史H65两项数值失败。实验盘123.97GiB可用；未提交或取消任务"},
        ...
        "heartbeat_evidence": "35_PERFORMANCE_CHECK_20260909_0218.json",
        "performance_cause_audit_scope": "2026-09-09 02:19-02:38用户交互检查：ET两组已完成，最佳ON55.3617%/OFF62.5093%，终轮54.5976%/61.7587%；独立terminal receipt待补。H65第55轮四相开57.5081%低于相位关63.7444%，仍待完整终轮。另任务90轮两项p_action非有限失败已读日志登记；不以资源不足解释该错误。本轮没有调参、热改或重提",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。17:21/17:26由监督任务提交的5项新协议作业当前状态见独立行，部署身份见29号记录；本次用户交互已检查ET两组训练及各12次全测试完成，独立终态封存仍待补；H65新三项仍运行、旧90轮两项数值失败，见35号记录。未新增提交、取消或改参。不是分钟dispatcher自动提交。已有Computer Use初始化阻塞仍保留，本轮未咨询Pro，也未取消旧任务。",
        "",
