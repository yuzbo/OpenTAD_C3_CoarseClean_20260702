...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "旧 77c8d173 C0 训练 1267979 虽完成 6000 updates，但旧 test ledger 不覆盖 792 次真实 loader 暴露，因此只保留为诊断。最终 codex/duca-evidence-fullgrid-repair-20260904@246058f2 使用 train/val 精确旧网格与 test full-grid，强制 allow_missing=False 并按唯一物理窗口校验；admission 1269357 COMPLETED(0:0)，验证 train 438/438、val 487/487、test 792 exposures/791 unique/791 ledger，34 tests 与 CUDA FP32 gate 均通过。新 C0 seed 8261 训练 1269374_0 于 11:32 CST COMPLETED(0:0)：60 epochs、6000 successful updates、6000 EMA/scheduler updates、nonfinite loss 0，终态 epoch_59.pth SHA256=a0b6e9dc…，training audit SHA256=46154ab3…。官方评测 1270672_0 于 11:58 CST COMPLETED(0:0)，结构化 metrics.json 已绑定 exact SHA 246058f2、config/evaluator/annotation/class-map SHA、seed 8261、epoch 59 state_dict_ema、checkpoint 与 prediction SHA，并写入 evaluation self-hash",
            "result_status": "最终 ledger 合同、C0 seed 8261 终态训练和 official evaluator receipt 均有效；C0 已有完整正式结果",
            "final_result": "C0 MATCHED_H65_60 official Avg-mAP=59.23%，mAP@0.3/0.4/0.5/0.6/0.7=75.22/70.01/61.54/50.90/38.48%。test 的 792 次暴露对应 791 个唯一物理窗口，重复项是 video_test_0001431|7680；246058f2 按唯一物理窗口严格覆盖。该结果是 Evidence 路线的 matched-H65 基线臂，不是完整 recovery 机制臂，因而不能用于宣称 Evidence-Recovery 带来增益",
            "next_action": "保留 C0 sealed metrics 作为基线；只有完成并评测 F/A1-A6 对照臂后，才能比较不确定性、coverage、time、robust、merge 与 recovery 各机制贡献",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 12:13 CST",
            "public_gpu_available": 7,
