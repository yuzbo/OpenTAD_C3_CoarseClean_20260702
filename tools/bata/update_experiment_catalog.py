...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "真实 CUDA/focused 门禁 1267855 已通过；D160 教师 1267884、G96 1268698、U16 1269124、LATE 1269129、NOKD 1269137、FULL 1269297 均 COMPLETED(0:0)，全部写出 6000-update epoch-59 receipt，screen_receipt_r2_1269297.json 为 PASS。评测器 29b5a7a2 已通过本地/远端 11 tests、21-cell validator 和双 GPU U16 PRECHECK 1269540。五臂评测 1269541 已生成 G96/U16/LATE/NOKD 四份 prediction receipt，FULL 在解码一个损坏视频尾部时因 DECORD EOF 失败；1269763 的首次定向重试又因 wrapper 未显式传入外部 D160 Teacher 路径而 fail-closed。读取两次日志后，重试 1269771 已绑定训练 checkout 的 Teacher config/checkpoint 及其既有 SHA256，PRECHECK 已成功加载真实 Teacher，当前仅重跑 FULL prediction，随后将打开五臂官方指标",
            "result_status": "D160 与五个 screen 臂训练终态有效，screen gate PASS；四个臂已有独立 prediction receipt，FULL 定向重试运行中，尚无五臂官方指标",
            "final_result": "FULL 训练 receipt 绑定训练 539287fa、seed 4407、world_size 2、6000 successful updates、Teacher checkpoint SHA256=ced2ee78…和自身 checkpoint SHA256=c0eb8677…；评测器固定为 29b5a7a2。G96/U16/LATE/NOKD 四份 prediction receipt 均为 metric_opened=false；1269771 完成前不得报告 mAP",
            "next_action": "监控 1269771 完成 FULL prediction 和五臂 official metric opening；若 DECORD EOF 在提高明确 retry 上限后仍复现，保留具体视频证据并按数据解码路径做最小修复，再重新运行 FULL PRECHECK 与定向评测",
        },
        {
            "category": "correction_route",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 08:18 CST",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。当前只有 H65 D768、U384 与 MNV3FC384 三个参考臂具有合法终态 evaluator metrics；其余路线或比较矩阵仍为 `NO_FINAL_PERFORMANCE_RESULTS`。终态训练 receipt 只能证明训练合同完成，不得从 admission 或中期验证推导最终 mAP 或机制结论。",
        "",
    ]
    return "\n".join(lines)
