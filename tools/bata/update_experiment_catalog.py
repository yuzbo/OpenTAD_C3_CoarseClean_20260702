...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "actual-ledger admission 1267969 已通过；C0 seed 8261 正式训练 1267979 COMPLETED(0:0)。1268680 漏掉 gpu1_id0，1269286 把不同 clean-checkout 绝对 config 路径误作同一路径，1269298 未恢复训练时 raw data/三份 ledger 环境，三次评测均 fail-closed 且未污染 checkpoint。最终修复 codex/duca-evidence-eval-path-repair-20260903@d6f1cc28 已推送并部署到 /data/run01/sczc063/yuzibo/projects/duca_evidence_eval_d6f1cc28，远端 focused tests 13 PASS；watcher PID 4167080 已提交新 admission 1269322，当前因 AssocGrpGRES 等待 GPU，C0 evaluator 将在后续额度释放时以 afterok 提交",
            "result_status": "C0 训练终态有效；d6f1cc28 admission 1269322 已入队，官方评测尚待提交；尚无最终性能",
            ...
            "next_action": "等待 1269322 通过 d6f1cc28 admission；每分钟 watcher 随后以 afterok 提交 C0 官方评测，若失败继续读取完整日志并按绑定字段最小修复",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 01:50:06 CST",
