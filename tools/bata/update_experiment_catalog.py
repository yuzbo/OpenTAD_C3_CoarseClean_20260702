...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "远端 exact-SHA admission 1267684 已通过；REF-D768、REF-U384、REF-MNV3FC384 正式训练 1267709/1267711/1267737 均 COMPLETED(0:0)。D768 官方评测 1269303 已完成。U384/MNV 评测 1269323/1269324 因 H65 运行绑定被错误读取 generic gate 字段而失败；修复 codex/h65-pro-eval-binding-repair-20260903@67c8f39f 已推送，远端 21 tests 与 admission 1269353 均 PASS，每分钟 watcher PID 131429 等待账户作业数低于 10 后续提。F01 训练 1269287 在 epoch 20/update 约 2056 因 p_action calibration 一致性异常失败，依赖评测 1269288 已取消；修复 codex/h65-pro-paction-tolerance-repair-20260904@30514803 已推送并通过远端 23 tests，watcher PID 204955 将先跑 admission 再重训。F02/F03/F04 训练 1269291/1269341/1269358 正在运行，对应评测 1269292/1269342/1269359 等待依赖",
            "result_status": "D768 单臂官方终态性能有效；U384/MNV 训练已完成但评测待重提；F01 失败后待修复准入与重训；F02-F04 正式训练中",
            ...
            "next_action": "三个 watcher 按账户额度自动推进 U384/MNV 评测、F01 admission/重训；持续监控 F02-F04，收齐独立终态 metrics 后再做同协议比较",
        },
        {
            "category": "correction_route",
        ...
        },
        {
            ...
            "result_status": "最终 ledger 合同已通过准入；旧 C0 正式结果身份被取代，新 C0 尚待账户额度释放后重训；无最终性能",
            ...
            "next_action": "watcher 自动提交 246058f2 C0 seed 8261 正式重训；完成后只在同一 ledger 身份下运行官方评测并生成 metrics receipt",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 03:18:42 CST",
        ...
        "excluded_remote_jobs": [
            ...
            {"job_ids": "1269287,1269288", "remote_directory": "/data/run01/sczc063/yuzibo/projects/h65_pro_formal_e553a5a4", "source_head": "e553a5a4a1063a755900d3dfa4bf8909bf97d466", "reason": "H65 F01 在 epoch 20/update 约 2056 遇到 p_action calibration 一致性异常并失败，依赖评测 1269288 已取消释放额度。30514803 增加 dtype-aware 有界容差、非有限值专门失败和回归测试，远端 23 tests PASS；仍须重跑 admission 并跨过原故障点后才能确认修复"},
