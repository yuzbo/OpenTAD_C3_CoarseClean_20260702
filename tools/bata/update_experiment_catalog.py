...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 14:28-14:32 CST用户进度查询：fe1c53db G0/G1训练均COMPLETED(0:0)，分别02:53:59至14:20:05、耗时11:26:06，以及02:54:29至13:58:46、耗时11:04:17。14:29读取两份真实epoch59 EMA，各337个Adam状态均6000，optimizer/scheduler/EMA一致6000；AMP skip均3且已重放。沿用已通过1278042的07383274，不重复预检，14:30:07提交正式独立评测1279211_0/1，14:30:35均RUNNING。14:32已观察两臂CTDP_EVAL_CHECKPOINT_OK与EMA加载；暂无独立metrics。预计15:00-15:30取得评测结果，非承诺；旧G2/G3保持原身份",
            ...
            "result_status": "78cde1be的G2/G3已有strict6000终态独立官方评测与收据；新fe1c53db G0/G1训练及实际6000验证已完成，07383274独立评测1279211_0/1运行中，暂无新最终性能。最早四臂更新预算不合格，78cde1be旧G0/G1坐标错误结果不能作合法控制",
            ...
            "next_action": "监督已运行的1279211_0/1，完成后核验独立官方metrics、视频覆盖和自哈希receipt再报告fe1c53db结果；不要重复提交或重跑1278007/1278042。约15:00-15:30为估计而非完成承诺。旧G0/G1仅作诊断，G2/G3沿用原身份，不盲目重跑全矩阵",
        },
        {
            "category": "correction_route",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-08 14:31 CST",
            "public_gpu_available": 17,
            "public_gpu_total": 200,
            "gpu_count_basis": "14:31 scontrol show nodes，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING后25个可调度GPU节点CfgTRES200、AllocTRES183；公共未分配17，不等于本账户额度",
            "user_jobs_in_queue": 8,
            "account_constraint": "14:31展开数组为7 RUNNING/1 PENDING，本任务两项为刚提交的CT独立评测1279211_0/1，已分配GPU。1278774为其他任务JobHeldUser，未操作。实验挂载186G可用、97%已用，公共空间不保证个人配额。只新增本次两项评测，无取消、热改或重复预检；历史BAFDR1267920/1267921不动",
        },
        ...
        "heartbeat_evidence": "25_RUNTIME_AND_CT_EVAL_DEPLOYMENT_20260908_1430.json",
        "performance_cause_audit_scope": "本次为用户询问运行时长后的定向查询，远端采样2026-09-08 14:28-14:32 CST；CT两臂训练已完成、真实终态6000验证通过，正式独立评测1279211_0/1已提交并运行。同步资源、队列和分钟监督器；其他路线机制与已有性能沿用各行原时间戳，不冒充本次六路线完整心跳或重新审计。没有新独立mAP、模型修改或重复PRECHECK",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。本次定向查询确认G0/G1终态6000通过，随后由本任务提交1279211_0/1正式独立评测并已运行；不是分钟dispatcher自动提交。未重复准入或取消旧任务。",
        "",
