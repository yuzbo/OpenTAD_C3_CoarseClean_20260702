...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "修正已提交并应用到远端验证 worktree；资源释放后已完成真实 CUDA P0 复验",
            "result_status": "修正已通过远端 P0 复验，尚未形成新冻结结果",
            "final_result": "远端修正提交 7f90a48d 的 H65 P0 测试 15 passed；仍不能晋级冻结 H65 结果",
            "next_action": "把修正提交重新冻结为新的 H65 SHA，再运行正式 strict-60 admission",
        },
        {
            "category": "correction_route",
        ...
        {
            "category": "correction_route",
            "name": "BAFDR 生成器、预训练路径与单进程 PRECHECK 修正实验（不替代冻结 BAFDR）",
            "internal_id": "BAFDR_ADMISSION_FIX",
            "branch": "codex/zoomtoken-bafdr-admission-fix-20260903",
            "sha": "c750cdae1b7dce8ab1ef9b3d2fa04ebb9853926a",
            "github_commit": "https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/c750cdae1b7dce8ab1ef9b3d2fa04ebb9853926a",
            "local_directory": "E:/DeskTop/TAD/_duca_fix_worktrees/bafdr_admission",
            "deployment_status": "修复已推送；远端 21-cell validator 与 D160 单进程 PRECHECK 已通过，2-GPU CUDA admission 受提交上限阻塞",
            "result_status": "修正尚未获得正式训练结果",
            "final_result": "修复 LoadFrames 窗口参数、canonical pretrained 绑定和单进程 optimizer；PRECHECK 报告 train_len=200、eval_windows=792、PASS",
            "next_action": "资源释放后运行 world-size=2 CUDA PRECHECK 与五臂 screen，再决定是否开放 21-cell",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "excluded_remote_jobs": [
            ...
            {"job_ids": "1266401-1266420", "remote_directory": "/data/run01/sczc063/yuzibo/projects/bafdr_k16_fullmatrix_5dba75c7", "source_head": "efe69d2ea10accd01d0129dfe99cba4d1d5773cb", "reason": "非当前冻结 BAFDR checkout；1266401/1266402 因 LoadFrames 不支持窗口参数失败；1266410-1266414 已写 6000-update train receipt 但 metric_opened=false/eval_results 为空，其余作业仍运行或依赖等待，全部不纳入当前结果"},
            {"job_ids": "1266475,1266479,1266480", "remote_directory": "/data/run01/sczc063/yuzibo/projects/bafdr_k16_fullmatrix_5dba75c7", "source_head": "efe69d2ea10accd01d0129dfe99cba4d1d5773cb", "reason": "非当前冻结 BAFDR checkout 的下游 cexec/metrics/summary 依赖作业，全部不纳入当前结果"},
