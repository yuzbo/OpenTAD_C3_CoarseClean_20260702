...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "receipt_recheck": "2026-09-08 04:30-04:31 CST：六份F01-F06 metrics JSON的evaluation_sha256逐份复算一致，f2068e18训练/67c8f39f评测身份及六个epoch59 EMA文件存在性已复核；六份Avg-mAP不变。未重复加载已认证的大权重或重新推理，四相仍未开启，TIA及匹配控制仍待修复",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 04:25 CST：新fe1c53db G0/G1正常RUNNING，已完成epoch11/12，optimizer/scheduler/EMA各1200/1300，AMP skip累计2/3且均重放成功。04:31 sacct确认旧78cde1be G0-G3全部COMPLETED(0:0)；旧G1独立receipt自哈希通过，Avg-mAP14.6613%仅保留坐标缺陷诊断。04:32读取G2/G3真实epoch59 EMA，各337个AdamW状态step=6000且scheduler/EMA6000；复用已通过的1277767，仅补独立评测1278026_2/3，不重训、不扩展旧G0/G1",
            ...
            "latest_repair": {"branch": "codex/duca-ctdp-coordinate-repair-20260908", "sha": "fe1c53db1b2a7e467a6af2eccfe8b7636f667a81", "local_directory": "E:/DeskTop/TAD/_duca_fix_worktrees/ctdp_successful_updates", "remote_source": "/data/run01/sczc063/yuzibo/projects/ctdp_coordinate_fe1c53db", "validation": "本地32 passed/1 CUDA skipped；最终SHA远端clean CPU52 passed/1 CUDA skipped。1278007已COMPLETED(0:0)，71 tests、CT_CUDA_GATE_OK、G0-G3真实视频PRECHECK全部通过，四份audit均各6次更新。04:25新正式G0/G1各1200/1300成功更新，未重复准入，无新最终性能", "status": "CUDA_AND_REAL_PRECHECK_PASS; G0_G1_FORMAL_RUNNING", "admission_job": "1278007", "admission_state": "COMPLETED(0:0), ended 2026-09-08 02:51:01 CST", "admission_stdout": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-coordinate-fe1c53db_1278007.out", "admission_stderr": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-coordinate-fe1c53db_1278007.err", "precheck_run_root": "/data/run01/sczc063/yuzibo/experiments/ctdp_coordinate_fe1c53db/precheck", "formal_jobs": {"G0": "1278011", "G1": "1278012"}, "formal_job_states": {"G0": "RUNNING, epoch11 completed, optimizer/scheduler/EMA1200; checked 2026-09-08 04:25 CST", "G1": "RUNNING, epoch12 completed, optimizer/scheduler/EMA1300; checked 2026-09-08 04:25 CST"}, "formal_run_root": "/data/run01/sczc063/yuzibo/experiments/ctdp_coordinate_fe1c53db/formal", "formal_stdout": {"G0": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g0-fe1c53db_1278011.out", "G1": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g1-fe1c53db_1278012.out"}, "formal_stderr": {"G0": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g0-fe1c53db_1278011.err", "G1": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g1-fe1c53db_1278012.err"}, "launch_contract": "PRECHECK_ONLY=0; CTDP_STAGE=geometry; seed3407; one GPU per arm; exact new SHA; fresh output; 60 epochs/6000 successful updates; no resume from PRECHECK", "first_log_observation": "历史epoch0各一次AMP skip重放；04:25累计G0/G1分别2/3次，无replay耗尽或Traceback，loss有限。stderr有既有linear1d CUDA nondeterministic warning，不能声称消除了全部随机性"},
            ...
            "new_evaluation_states": {"G2": "RUNNING; checkpoint identity PASS; inference157 batches at 2026-09-08 04:40 CST", "G3": "RUNNING; checkpoint identity PASS; inference154 batches at 2026-09-08 04:40 CST"},
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08本轮由目录生成器重新读取本地HEAD/clean状态，记录于local_head/local_clean_tree。Taylor P0/P1运行时接线及H65 retention/transition本轮未修复，41单元未部署。该路线是明确的实现欠账，而不是资源排队，继续BLOCKED_UNIMPLEMENTED",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 04:28 CST：U16=1276842/LATE=1276843/NOKD=1277954全部COMPLETED(0:0)。首次以710ce8a6现有terminal validator核验三份epoch59 EMA：实际AdamW state分别352/348/352个且全部step6000，scheduler/EMA6000；AMP skips5/5/4均成功重放。train receipt无eval_results，未发现新独立评测receipt。FULL PRECHECK1277955通过，但padding/screen未修复，不开放FULL正式训练或21单元",
            ...
            "deployment_status": "历史539287fa更新不足仍保留。710ce8a6新Teacher/G96此前已核验，U16/LATE/NOKD本轮均实际终态6000验证通过；FULL只有PRECHECK，无正式训练。padding/screen缺陷尚未修复，不继续自动扩展有缺陷代码",
        ...
        },
        {
            ...
            "receipt_recheck": "2026-09-08 04:31 CST：OFF/ON metrics JSON可读且receipt_sha256复算一致，原74473c27训练身份和epoch59 EMA文件存在性复核通过，数值仍62.0768%/54.8096%。未重新加载已认证大权重或重跑推理；anchor correction及实际精度问题仍待修复/核对",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 04:30-04:31 CST：ce767b4d的A1/A6/F全部COMPLETED(0:0)，三份真实epoch59 EMA及终态训练绑定通过；非条件AdamW状态均6000，scheduler/EMA6000。时间conditioner四参数分别只更新794/1600/794次，是positions优先的if/elif条件调用，不是全局漏更新；缺少state的utility另属已知缺陷。A1关闭覆盖约束官方Avg-mAP51.5968%，收据自哈希、训练权重/预测文件/官方evaluator绑定均通过，跨SHA恢复点保留。A6/F无独立评测receipt；A3/A4/A5仍因已知机制缺陷停止扩展，不再误记仅等待资源",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-08 04:25 CST公共资源；04:40 CST提交后展开数组队列",
            "public_gpu_available": 49,
            "public_gpu_total": 192,
            "gpu_count_basis": "04:25 scontrol show nodes的gpu分区，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING后24节点CfgTRES192、AllocTRES143；这是提交新eval前快照，不用旧200/240分母或推算提交后余量",
            "user_jobs_in_queue": 8,
            "account_constraint": "04:25本用户6 RUNNING/0 PENDING；仅补CT G2/G3独立评测1278026_2/3后，04:40展开数组为8 RUNNING/0 PENDING，两项新eval均获得GPU并开始真实推理。公共未分配GPU不等于账户可使用额度。04:25实验挂载5.3T总量、325G可用、95%已用，需持续关注空间/配额，不删除旧实验腾空间；本轮无新提交或运行失败。历史1267920/1267921只读不动",
        },
        ...
        "heartbeat_evidence": "16_HEARTBEAT_TERMINAL_EVIDENCE_20260908_0424.json",
        "performance_cause_audit_scope": "本轮2026-09-08 04:24起实际核查；04:25资源/CT训练，04:28 BAFDR新terminal，04:30-04:31 Evidence原始权重及收据，04:32 CT G2/G3真实terminal，04:40新eval启动日志。新增A1旧实现官方评测51.5968%，不是新修复版性能或完整机制验收。H65/ET只复查小收据和权重存在性。03:35未完成的诊断不补记为一次已完成的全面心跳",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。本轮仅显式补交已认证CT G2/G3独立评测1278026_2/3，04:40均真实推理中；未取消旧任务或重复已通过的准入。",
        "",
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。历史性能保留原身份；本轮A1的51.5968%是旧实现终态评测，不是新refiner修复后性能。官方原配方68.73仍为此前核查的原始日志值，不能替代尚待补证的独立复评。各行区分训练完成、收据认证和机制验收，不从CPU反例、PRECHECK或中期训练推导最终性能。",
        "",
    ]
    return "\n".join(lines)
