...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "246058f2 使用 train/val 精确旧网格与 test full-grid，admission 1269357 COMPLETED(0:0)；C0 seed 8261 训练 1269374_0 与官方评测 1270672_0 均 COMPLETED(0:0)，metrics.json 已绑定 exact SHA、config/evaluator/annotation/class-map、epoch 59 EMA checkpoint/prediction SHA 与自哈希。数组 1270870 中 F、A2、A3、A4、A5 均完成 60 轮训练；A1 在 epoch 54 batch 11 的 AMP replay 后产生 768 个非有限 utility 值并 FAILED(1:0)，其 epoch-49 checkpoint 与完整日志保留，不能算作完成。A6 的 1270870_7 在 batch 2 连续 50 次确定性 replay 均产生 pre-AMP non-finite cost；独立修复 9739811e 将 bounded interval decomposition 与 dense recovery accumulation 固定为 FP32，同时保留全部 A6 机制。最终 exact-SHA admission 1271006 与真实数据 PRECHECK 1271146 均 COMPLETED(0:0)。正式 A6 seed 8261 已以新命名空间提交为 1274671_7，并已越过旧 batch-2 故障点继续运行",
            "result_status": "C0 seed 8261 已有完整正式结果；F/A2-A5 已完成训练但尚无独立官方评测；A1 是已定位但尚未修复的数值失败；A6 修复版正式训练运行中，因此完整 Evidence 机制结论尚未形成",
            ...
            "next_action": "监控 A6 作业 1274671_7 到 epoch 59/6000 successful updates；为 A1 从 epoch-49 checkpoint 建立独立 exact-RNG 恢复诊断和对应 PRECHECK，确认最小数值修复后以新 SHA/新命名空间重提。随后对 F/A1-A6 分别按 exact-SHA evaluator 合同生成 official receipt",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-07 11:07 CST",
            "public_gpu_available": 28,
            "public_gpu_total": 240,
            "user_jobs_in_queue": 5,
            "account_constraint": "公共口径 28/240 可用；本用户 4 RUNNING、1 PENDING。Evidence A6 已获得 1 张 GPU 并正式运行，当前未见新增提交额度阻塞",
        },
