...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "deployment_status": "旧 77c8d173 C0 训练 1267979 虽完成 6000 updates，但旧 test ledger 不覆盖 792 次真实 loader 暴露，因此只保留为诊断。最终 codex/duca-evidence-fullgrid-repair-20260904@246058f2 使用 train/val 精确旧网格与 test full-grid，强制 allow_missing=False 并按唯一物理窗口校验；admission 1269357 COMPLETED(0:0)，验证 train 438/438、val 487/487、test 792 exposures/791 unique/791 ledger，34 tests 与 CUDA FP32 gate 均通过。新 C0 seed 8261 训练 1269374_0 于 11:32 CST COMPLETED(0:0)：60 epochs、6000 successful updates、6000 EMA/scheduler updates、nonfinite loss 0，终态 epoch_59.pth SHA256=a0b6e9dc…，training audit SHA256=46154ab3…。官方评测 1270672_0 已绑定同一 exact SHA、seed、run root 和 state_dict_ema checkpoint 并于 11:36 CST 启动",
            "result_status": "最终 ledger 合同与 C0 seed 8261 终态训练均有效；官方评测 1270672_0 运行中，尚无最终性能",
            "final_result": "test 的 792 次暴露对应 791 个唯一物理窗口，因为 video_test_0001431|7680 是 OpenTAD 尾窗逻辑产生的同一窗口重复暴露；246058f2 按唯一物理窗口严格覆盖，不是放宽或漏掉样本。终态 EMA checkpoint、sidecar 和 complete training audit 已齐，但 official metrics 尚未生成，因此当前只能确认训练完成，不能报告 mAP",
            "next_action": "监控 1270672_0 完成同一 ledger 身份下的 official THUMOS14 evaluation；核验 metrics JSON 的 checkpoint/config/evaluator 身份并形成合法 receipt 后再报告 C0 mAP",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 11:34 CST",
            "public_gpu_available": 22,
            "public_gpu_total": 240,
            "user_jobs_in_queue": 10,
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。当前 H65 D768、U384、MNV3FC384 参考臂和 BAFDR G96、U16-UNIFORM-A0、LATE、NOKD、FULL 具有合法终态 evaluator metrics；其余路线或比较臂仍为 `NO_FINAL_PERFORMANCE_RESULTS`。终态训练 receipt 只能证明训练合同完成，不得从 admission 或中期验证推导最终 mAP 或机制结论。",
        "",
    ]
    return "\n".join(lines)
