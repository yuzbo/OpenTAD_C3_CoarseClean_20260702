...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "next_action": "保留已核验的 epoch-59 EMA/6000 optimizer steps，补齐独立 OFF/ON official evaluator 和自哈希 receipt；不把训练内 telemetry 提前称为最终性能，不要求重训已确认达到更新预算的双臂",
        },
        {
        ...
        },
        {
            ...
            "branch": "codex/duca-evidence-fullgrid-repair-20260904（C0/A2） + codex/duca-evidence-optimizer-repair-20260907（时间参数优化器与 DDP 修复）",
            "sha": "246058f2c24edc78818ada60eec26249bbf7d5d2（C0/A2） / 1570a72507491899a50767700a35a04eee3f5fe9（修复）",
            "github_commit": "https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/1570a72507491899a50767700a35a04eee3f5fe9",
            "local_directory": "E:/DeskTop/TAD/_duca_fix_worktrees/evidence_optimizer_repair",
            "supporting_local_directories": ["E:/DeskTop/TAD/_duca_fix_worktrees/evidence_eval_repair", "E:/DeskTop/TAD/OpenTAD_Evidence_FormalRepair_20260903"],
            ...
            "result_status": "C0 和 A2 有效官方结果；旧 F/A3/A4/A5 虽完成训练，但启用时间参数的优化器缺项使机制训练不完整，列为待修复重训而非待评测即可。A1/A6 修复版正式训练运行中，已有逐臂 afterok 官方评测依赖；完整 Evidence 机制结论尚未形成",
            ...
            "next_action": "监督 1274924_2/7 到 epoch59 EMA/6000 successful updates，随后核验 1274926_2/1274927_7 的 official metrics receipt。为同样受遗漏时间参数影响的 F/A3-A5 安排对应 PRECHECK 和新 SHA 重训，不把旧终态 checkpoint 直接评测后宣称机制有效。预检 200 batches 不等于正式 6000 updates，也不是对旧 epoch54 RNG 序列的精确重放",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-07 12:13 CST",
            "public_gpu_available": 22,
            "public_gpu_total": 240,
            "user_jobs_in_queue": 8,
            "account_constraint": "公共口径 22/240 可用；本用户 5 RUNNING、3 PENDING。Evidence A1/A6 正式训练已获得各 1 GPU，两项官方评测等待逐臂 afterok；历史 BAFDR 1267921 因 DependencyNeverSatisfied 等待，保持只读",
        },
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    for entry in payload["entries"]:
        ...
        commit = f"{entry['branch']}<br>[`{entry['sha'][:8]}`]({entry['github_commit']})"
