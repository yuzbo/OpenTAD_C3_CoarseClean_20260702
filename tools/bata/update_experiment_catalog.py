...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "next_action": "持续监控六臂完成 epoch 59/6000 successful updates；任何失败先读取该臂日志和 checkpoint，完成最小修复与 exact-SHA PRECHECK 后才重提；六臂完成 terminal EMA 后分别运行官方 evaluator 并生成 receipt",
        },
        {
            "category": "correction_route",
        ...
        },
        {
            ...
            "deployment_status": "旧 77c8d173 C0 训练 1267979 虽完成 6000 updates，但旧 test ledger 不覆盖 792 次真实 loader 暴露，因此只保留为诊断。最终 codex/duca-evidence-fullgrid-repair-20260904@246058f2 使用 train/val 精确旧网格与 test full-grid，强制 allow_missing=False 并按唯一物理窗口校验；admission 1269357 COMPLETED(0:0)，验证 train 438/438、val 487/487、test 792 exposures/791 unique/791 ledger，34 tests 与 CUDA FP32 gate 均通过。C0 seed 8261 训练 1269374_0 与官方评测 1270672_0 均 COMPLETED(0:0)，结构化 metrics.json 已绑定 exact SHA 246058f2、config/evaluator/annotation/class-map SHA、epoch 59 EMA checkpoint/prediction SHA 与自哈希。为完成机制对照，同一 seed 的 F/A1-A6 已以 exact checkout 数组 1270870 提交；截至 13:29 CST，F、A1-A5 六臂均已完成 epoch 4 并进入 epoch 5，未见 traceback/non-finite/NaN/OOM；A6 因 AssocGrpGRES 等待账户 GPU 配额，尚未创建运行日志。这是资源等待，不是提交或实现失败",
            "result_status": "C0 seed 8261 已有完整正式结果；F/A1-A5 正式训练运行中，A6 等待账户 GPU 配额，完整 Evidence 机制结论尚未形成",
            ...
            "next_action": "监控数组 1270870 的 F/A1-A6 直到 6000 successful updates 和 terminal EMA；A6 在账户配额释放后自动启动。逐臂完成后按同一 exact-SHA evaluator 合同生成 metrics receipt，再做机制差分",
        },
    ]
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-04 13:29 CST",
            "public_gpu_available": 0,
            "public_gpu_total": 240,
            "user_jobs_in_queue": 16,
            "account_constraint": "面向提交器的 public 口径显示 0/240 可用；A6 因 AssocGrpGRES 等待。底层 gpu 分区另报 432 卡，二者统计口径不同，不混算",
        },
