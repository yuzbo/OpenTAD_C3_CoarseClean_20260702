...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 16:33 CST：六份F臂小收据mtime/指标未变，最高64.2265%；复用既有终态与自哈希认证，不重跑大checkpoint或推理。F01-F06仍phase-off，TIA/匹配控制未修。原始官方1245842的68.73%仍仅保留原日志身份，待来源补证和独立复评",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 16:33 CST：OFF/ON收据mtime和指标未变，仍62.0768%/54.8096%；复用既有终态/自哈希认证，不重跑推理。anchor correction及实际精度核对未完成，保持该fixed-stride低秩代理版本负结果，不能称真实event gate或exact JVP",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "cluster_observation": {
            "checked_at_cst": "2026-09-08 16:33 CST",
            "public_gpu_available": 3,
            "public_gpu_total": 200,
            "gpu_count_basis": "16:33 scontrol show nodes，排除DOWN/DRAIN/FAIL/MAINT/RESERVED/NOT_RESPONDING后25个可调度GPU节点CfgTRES200、AllocTRES197，公共未分配3；不等于本账户配额或新任务已获准",
            "user_jobs_in_queue": 11,
            "account_constraint": "16:33展开数组为9 RUNNING/2 PENDING，共11项；队列其他任务不自动纳入六路线。1279871因Priority排队，1278774仍JobHeldUser，均未操作。实验盘175G可用、97%已用。本轮无提交、取消、重提或热改，历史BAFDR1267920/1267921保持只读",
        },
        ...
        "heartbeat_evidence": "28_HEARTBEAT_EVIDENCE_20260908_1631.json",
        "performance_cause_audit_scope": "本轮真实触发2026-09-08 16:31:36.597 CST，实时远端采样16:32:59至16:33:06；核查六路线本地身份、登记Slurm、stdout/stderr、CT audit/checkpoint stat、已有小收据与资源，无新运行失败或性能变化。复用已认证终态与自哈希，不重复大checkpoint/准入。上一轮Computer Use的kernel assets路径错误仍记录为未解决，本轮未重试或声称已咨询Pro",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程因磁盘配额退出的日志保留；新进程恢复只读轮询，超过180秒标STALE。dispatcher不自动续提当前修复队列。本轮既有结果未变，无新提交、重复准入或取消旧任务；上一轮Computer Use初始化故障未解决，本轮未重试或咨询Pro。分钟dispatcher仍未自动提交当前修复队列。",
        "",
