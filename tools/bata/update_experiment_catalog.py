...
def route_entries() -> list[dict[str, Any]]:
    return [
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08原始证据补充：已找到并读取共享job1245842的stdout、有效配置和epoch59 EMA；当前source为官方01c58b9f且clean，有效配置与该官方配置仅差work_dir，日志终轮Avg-mAP=68.73%。这是原始日志值而非新推理；仍待历史预训练/数据身份及独立评测补证。真实optimizer5995、scheduler6000属于官方原配方AMP行为，不追溯套用后来的strict6000合同。REF67.58%/63.89%仍只作修改模型/配方参考，不能替代69.03%官方公开锚点。历史uniform384的64.352%/65.696%仍protocol-unmatched",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 02:35 CST只读sacct：旧G1=1276670 COMPLETED(0:0)，G0/G2/G3仍RUNNING；旧独立评测PRECHECK1277767已COMPLETED(0:0)，G1评测1277962_1 RUNNING。此状态查询未认证新checkpoint/receipt，旧G0/G1即使完成仍有坐标错误。新坐标代码已推送并同步独立clean目录，CUDA/真实视频验证状态见最新修复记录，不热改旧训练",
            ...
            "latest_repair": {"branch": "codex/duca-ctdp-coordinate-repair-20260908", "sha": "fe1c53db1b2a7e467a6af2eccfe8b7636f667a81", "local_directory": "E:/DeskTop/TAD/_duca_fix_worktrees/ctdp_successful_updates", "remote_source": "/data/run01/sczc063/yuzibo/projects/ctdp_coordinate_fe1c53db", "validation": "本地32 passed/1 CUDA skipped；最终SHA远端clean CPU52 passed/1 CUDA skipped。1278007已COMPLETED(0:0)，71 tests、CT_CUDA_GATE_OK、G0-G3真实视频PRECHECK全部通过，四份结构化audit均optimizer/scheduler/EMA各6次。按已有授权仅替换坐标错误的G0/G1：新正式job1278011/1278012已RUNNING并开始epoch0，无新最终性能", "status": "CUDA_AND_REAL_PRECHECK_PASS; G0_G1_FORMAL_RUNNING", "admission_job": "1278007", "admission_state": "COMPLETED(0:0), ended 2026-09-08 02:51:01 CST", "admission_stdout": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-coordinate-fe1c53db_1278007.out", "admission_stderr": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-coordinate-fe1c53db_1278007.err", "precheck_run_root": "/data/run01/sczc063/yuzibo/experiments/ctdp_coordinate_fe1c53db/precheck", "formal_jobs": {"G0": "1278011", "G1": "1278012"}, "formal_job_states": {"G0": "RUNNING, epoch0 started; checked 2026-09-08 02:57 CST", "G1": "RUNNING, epoch0 started; checked 2026-09-08 02:57 CST"}, "formal_run_root": "/data/run01/sczc063/yuzibo/experiments/ctdp_coordinate_fe1c53db/formal", "formal_stdout": {"G0": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g0-fe1c53db_1278011.out", "G1": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g1-fe1c53db_1278012.out"}, "formal_stderr": {"G0": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g0-fe1c53db_1278011.err", "G1": "/data/run01/sczc063/yuzibo/slurm_logs/ctdp-g1-fe1c53db_1278012.err"}, "launch_contract": "PRECHECK_ONLY=0; CTDP_STAGE=geometry; seed3407; one GPU per arm; exact new SHA; fresh output; 60 epochs/6000 successful updates; no resume from PRECHECK", "first_log_observation": "epoch0 G0 batch10与G1 batch17各一次AMP skip并进入1/8重放；暂无replay耗尽或Traceback，不将正常GradScaler降scale当作训练失败。stderr有既有linear1d CUDA nondeterministic warning，不能声称消除了全部随机性"},
            ...
            "formal_job_states": {"G0": "RUNNING at 2026-09-08 02:35 CST", "G1": "COMPLETED(0:0); coordinate defect remains", "G2": "RUNNING at 2026-09-08 02:35 CST", "G3": "RUNNING at 2026-09-08 02:35 CST"},
            "observed_successful_updates": {"G0": 4500, "G1": 4500, "G2": 4500, "G3": 4400},
            "successful_update_observation_at_cst": "2026-09-07 22:56; historical counts, not the current terminal state",
            ...
            "result_status": "最早四臂更新预算不合格；78cde1be的G0/G1坐标错误仍存在于旧训练，不能作合法控制。fe1c53db是新代码修复而非新性能结果，G2/G3尚无本轮认证的独立最终性能",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 02:35 CST sacct：U16=1276842和LATE=1276843已COMPLETED(0:0)，NOKD=1277954 RUNNING，FULL PRECHECK=1277955 COMPLETED(0:0)。这轮未重新加载这些终态权重或认证新mAP；旧五臂更新预算问题不变。已知padding及screen验收缺陷修复前不自动开放21单元，也不把FULL预检结束当作机制验收",
            ...
            "deployment_status": "历史旧训练539287fa已结束，但真实更新不足，旧screen PASS及补封存不修复该问题。710ce8a6通过1276618的CUDA/五臂PRECHECK后，新Teacher/G96已核验完成；U16/LATE在02:35查询时已结束，NOKD运行、FULL预检结束。新增审查发现的padding/screen缺陷尚未修复，旧作业保留，不继续自动扩展有缺陷代码",
            "result_status": "旧五臂及旧D160仍PROTOCOL_ERROR；新710ce8a6教师/G96曾验证实际6000，U16/LATE在02:35查询时已结束，但尚无本轮认证的新独立性能。padding与screen门禁未修复，更新修复不能覆盖这些问题。以下旧分数仅供诊断",
            ...
            "successful_update_observation_at_cst": "2026-09-07 22:56; U16/LATE counts are historical, not their terminal counts",
        },
        {
            "category": "correction_route",
        ...
        },
        {
            ...
            "current_cycle": "2026-09-08 02:35 CST sacct：旧版本A1=1277406_2、A6=1277407_7及A1评测1277953_2均COMPLETED(0:0)，F=1277768_1 RUNNING。该查询未逐项认证这些新增终态checkpoint与receipt，因此不新增mAP。局部refiner修复与旧续训身份分开；A3/A4/A5不得在已知utility/refiner/损失语义缺陷下自动扩展",
            ...
            "result_status": "C0/A2已登记官方终态值不变；A1/A6旧续训已结束，新增终态产物尚待本轮完整认证。e0c88c9a仅修复refiner新模型的可学习初始化，无新最终性能；utility及robust/cycle仍未完成，不能以旧FULL命名证明完整机制有效",
            ...
            "resume_validation": {"commit": "ce767b4df49a82b42f4fab4897ac4ea562e4b548", "local": "49 passed, 5 Windows Torch skipped", "remote_exact_clean_linux": "53 passed, 1 CUDA skipped", "real_checkpoint_bindings": "ALL_SIX_REAL_CHECKPOINT_BINDINGS_OK", "cuda_and_real_resume_precheck_job": "1276876", "cuda_and_real_resume_status": "COMPLETED(0:0): 80 tests, CUDA FP32, all six real resume prechecks PASS; historical resume validation, not utility/refiner acceptance", "formal_resume_jobs": {"A1": "1277406_2", "A6": "1277407_7", "F": "1277768_1"}, "formal_resume_job_states": {"A1": "COMPLETED(0:0), sacct 2026-09-08 02:35 CST", "A6": "COMPLETED(0:0), sacct 2026-09-08 02:35 CST", "F": "RUNNING, sacct 2026-09-08 02:35 CST"}, "admission_satisfied_by": "1276876 already COMPLETED before submission", "pending_formal_arms": ["A3", "A4", "A5"], "new_eval_jobs": {"A1": "1277953_2 COMPLETED(0:0); new receipt not certified this audit"}, "new_eval_dependencies": {"A1": "afterok:1277406_2", "A6": "afterok:1277407_7", "F": "afterok:1277768_1"}, "new_eval_blocker": "Known mechanism defects must be separated from old-run artifact certification; no automatic expansion of defective SHA"},
            "observed_successful_updates": {"A1": 4700},
            "successful_update_observation_at_cst": "2026-09-07 22:59; historical count, not A1 terminal state",
...
def catalog() -> dict[str, Any]:
    ...
    return {
        ...
        "performance_cause_audit_scope": "2026-09-08追加Pro全文/CPU证据核对、三个独立修复分支及官方原始产物调查，不是所有实验的新终态认证。02:35仅刷新所列Slurm状态；历史更新数/资源按各自采样时间解释。CT新代码修复不迁移到旧训练结果；本轮未新增mAP",
...
def md_text(payload: dict[str, Any]) -> str:
    ...
    lines += [
        ...
        "## 监督器与动态状态",
        "",
        f"远端 N16R4 监督器恢复目录：`{REMOTE_ROOT}`，每60秒轮询；本地heartbeat每30分钟刷新本表并用中文详细通知用户，即使无变化。当前回执状态：`{payload['remote_supervisor'].get('status')}`，距生成`{payload['remote_supervisor'].get('receipt_age_seconds', '未知')}`秒；dispatcher：`{payload['remote_supervisor'].get('dispatcher_status', '未知')}`，mode：`{payload['remote_supervisor'].get('dispatcher_mode', '未知')}`。旧进程9月7日16:48因磁盘配额退出，日志保留；新PID1914589于21:32恢复轮询。超过180秒标STALE。dispatcher不自动续提当前修复队列。本次CT坐标admission1278007通过后，显式提交新G0/G1正式训练1278011/1278012，均已开始epoch0；未取消旧作业。",
        "",
    ...
    lines += [
        "",
        f"结果规则：{payload['result_policy']}。历史性能保留原身份；本次原始68.73日志/权重核验与三项代码修复均不构成新mAP。各行明确区分已结束作业、尚待认证的新终态和仍有缺陷的实现，不从CPU反例、PRECHECK或中期训练推导最终性能。",
        "",
    ]
    return "\n".join(lines)
