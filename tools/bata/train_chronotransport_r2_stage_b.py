...
def _load_json(path: Path) -> Any:
    _, value, _, _ = load_bound_json(path, label=f"formal Stage-B JSON {path}")
    return value
...
def _load_torch_regular_file(path: Path | str, *, label: str) -> tuple[Path, Any]:
    exact, value, _, _ = load_bound_torch(path, label=label)
    return exact, value
...
def _regular_file_exists(path: Path | str, *, label: str) -> bool:
    ...
    if not path_exists_no_follow(exact, label=label):
        return False
    with open_bound_regular_file(exact, label=label):
        pass
    return True
...
def _atomic_write(path: Path, payload: bytes) -> None:
    ...
    publish_bytes_exclusive(
        path,
        payload,
        label="formal Stage-B completion artifact",
...
def _run_locked(
    ...
    *,
    ...
    audit_formal_python_runtime(
        ...
        entrypoint_relative="tools/bata/train_chronotransport_r2_stage_b.py",
    )
