#!/usr/bin/env python3
"""Same-GPU ordered-video decode-reuse replay for ZoomToken K100 and R1.
...
in one Slurm allocation, using a counterbalanced ABBA+BAAB order.  The only
changed execution component is a synchronous bounded CPU uint8 frame buffer
below the canonical per-window transforms.
"""
...
FROZEN_EVALUATOR_VECTOR = {
    "K100": {
        "average_mAP": 0.6850689034352214,
        "mAP@0.3": 0.8361032283895863,
        "mAP@0.4": 0.7975732565520736,
        "mAP@0.5": 0.71729021957962,
        "mAP@0.6": 0.6119111785629083,
        "mAP@0.7": 0.46246663409191857,
    },
    "R1": {
        "average_mAP": 0.6904214813293945,
        "mAP@0.3": 0.8436235400542165,
        "mAP@0.4": 0.799091460790171,
        "mAP@0.5": 0.7335280797130965,
        "mAP@0.6": 0.6108696090294431,
        "mAP@0.7": 0.4649947170600461,
    },
}
...
class _NumpyBatch:
    """Minimal Decord batch surface consumed by ``DecordDecode``."""

    def __init__(self, array: Any) -> None:
        self._array = array

    def asnumpy(self) -> Any:
        return self._array


@dataclass(frozen=True)
class DecodeReuseStats:
    video_name: str | None
    filename: str | None
    reader_open_count: int
    source_frame_request_count: int
    maximum_buffered_frames: int
    buffered_frames: int


class OrderedVideoDecordInit:
    """Stateful DecordInit replacement for canonical ordered evaluation.

    ``mmaction.DecordDecode`` remains unchanged and calls this object through
    ``get_batch``.  Only missing raw RGB uint8 frames are requested from the
    real reader; overlap indices are gathered from the bounded CPU buffer.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if int(num_threads) != 4:
            raise ValueError("ordered-video replay freezes Decord num_threads=4")
        ...
        self._reader_open_count = 0
        self._source_frame_request_count = 0
        self._maximum_buffered_frames = 0
    ...
    def stats(self) -> DecodeReuseStats:
        return DecodeReuseStats(
            video_name=self._video_name,
            filename=self._filename,
            reader_open_count=self._reader_open_count,
            source_frame_request_count=self._source_frame_request_count,
            maximum_buffered_frames=self._maximum_buffered_frames,
            buffered_frames=len(self._cache),
        )

    def _open(self, video_name: str, filename: str) -> None:
        self._reader = self._factory()(filename, num_threads=self.num_threads)
        self._video_name = video_name
        self._filename = filename
        self._cache.clear()
        self._requested_indices.clear()
        self._reader_open_count += 1

    def __call__(self, results: Mapping[str, Any]) -> dict[str, Any]:
        checked = dict(results)
        video_name = str(checked["video_name"])
        filename = str(checked["filename"])
        ordinal = int(checked["window_ordinal"])
        start = int(checked["feature_start_idx"])
        end = int(checked["feature_end_idx"])
        if start < 0 or end < start:
            raise ValueError("ordered-video window indices are invalid")

        new_video = video_name != self._video_name or filename != self._filename
        if not new_video and self._last_ordinal is not None:
            continuous = (
                ordinal == self._last_ordinal + 1
                and start >= int(self._last_start)
                and start <= int(self._last_end) + 1
            )
            if not continuous:
                self.reset()
                raise ValueError("canonical ordered-video population is discontinuous")
        if new_video:
            self.reset()
            self._open(video_name, filename)

        if self._reader is None:
            raise RuntimeError("ordered-video reader was not initialized")
        self._last_ordinal = ordinal
        self._last_start = start
        self._last_end = end
        checked["video_reader"] = self
        checked["total_frames"] = int(len(self._reader))
        checked["avg_fps"] = float(self._reader.get_avg_fps())
        return checked

    def get_batch(self, frame_indices: Sequence[int] | Any) -> _NumpyBatch:
        import numpy as np

        if self._reader is None:
            raise RuntimeError("ordered-video reader is not active")
        flattened = np.asarray(frame_indices, dtype=np.int64).reshape(-1)
        if flattened.size == 0:
            raise ValueError("ordered-video decode received no frame indices")
        if int(flattened.min()) < 0 or int(flattened.max()) >= len(self._reader):
            raise IndexError("ordered-video frame index leaves the current video")

        minimum_required = int(flattened.min())
        for index in tuple(self._cache):
            if index < minimum_required:
                del self._cache[index]
        ordered_unique = list(dict.fromkeys(int(index) for index in flattened.tolist()))
        missing = [index for index in ordered_unique if index not in self._cache]
        duplicated_request = self._requested_indices.intersection(missing)
        if duplicated_request:
            raise RuntimeError(f"source frames requested twice: {sorted(duplicated_request)}")
        if missing:
            decoded = np.asarray(self._reader.get_batch(missing).asnumpy())
            if decoded.shape[0] != len(missing) or decoded.dtype != np.uint8:
                raise ValueError("Decord returned a non-uint8 or mis-sized frame batch")
            for index, frame in zip(missing, decoded):
                self._cache[index] = np.array(frame, dtype=np.uint8, copy=True)
            self._requested_indices.update(missing)
            self._source_frame_request_count += len(missing)

        assembled = np.stack([self._cache[int(index)] for index in flattened], axis=0)
        self._maximum_buffered_frames = max(self._maximum_buffered_frames, len(self._cache))
        return _NumpyBatch(assembled)


def patch_ordered_video_pipeline(
    dataset: Any, *, reader_factory: Callable[..., Any] | None = None
) -> OrderedVideoDecordInit:
    """Replace exactly one DecordInit while preserving every other transform."""

    transforms = getattr(getattr(dataset, "pipeline", None), "transforms", None)
    if not isinstance(transforms, list):
        raise TypeError("dataset pipeline does not expose an ordered transform list")
    init_indices = [
        index
        for index, transform in enumerate(transforms)
        if type(transform).__name__ == "DecordInit"
    ]
    decode_indices = [
        index
        for index, transform in enumerate(transforms)
        if type(transform).__name__ == "DecordDecode"
    ]
    if len(init_indices) != 1 or len(decode_indices) != 1:
        raise ValueError("ordered-video replay requires one DecordInit and one DecordDecode")
    if init_indices[0] >= decode_indices[0]:
        raise ValueError("DecordInit must precede DecordDecode")
    original = transforms[init_indices[0]]
    replacement = OrderedVideoDecordInit(
        num_threads=int(getattr(original, "num_threads", 4)),
        reader_factory=reader_factory,
    )
    transforms[init_indices[0]] = replacement
    return replacement


class _FrameIndexTap:
    """Expose exact LoadFrames output to the parity-only metadata path."""

    def __call__(self, results: Mapping[str, Any]) -> dict[str, Any]:
        import numpy as np

        checked = dict(results)
        indices = np.asarray(checked["frame_inds"])
        checked["frame_indices_dtype"] = str(indices.dtype)
        checked["frame_indices_shape"] = [int(value) for value in indices.shape]
        checked["frame_indices_sha256"] = hashlib.sha256(
            indices.tobytes(order="C")
        ).hexdigest()
        return checked


def patch_frame_index_parity_tap(dataset: Any) -> None:
    """Insert an input-only tap in the precheck without changing transforms."""
    ...
    load_indices = [
        ...
        if type(transform).__name__ == "LoadFrames"
    ]
    decode_indices = [
        index for index, transform in enumerate(transforms)
        if type(transform).__name__ == "DecordDecode"
    ...
    collect_indices = [
        index for index, transform in enumerate(transforms)
        if type(transform).__name__ == "Collect"
    ]
    if len(load_indices) != 1 or len(decode_indices) != 1 or len(collect_indices) != 1:
        raise ValueError("frame-index parity tap requires LoadFrames, DecordDecode and Collect")
    if decode_indices[0] != load_indices[0] + 1:
        raise ValueError("LoadFrames must immediately precede DecordDecode")
    transforms.insert(decode_indices[0], _FrameIndexTap())
    collect = transforms[collect_indices[0] + 1]
    for key in (
        "frame_indices_dtype",
        "frame_indices_shape",
        "frame_indices_sha256",
    ):
        if key not in collect.meta_keys:
            collect.meta_keys.append(key)
...
def _git_identity(root: Path, expected_commit: str) -> None:
    ...
    if head != expected_commit or dirty:
        raise RuntimeError("ordered-video replay requires its exact clean execution commit")
    ...
    if not remote_contains:
        raise RuntimeError("ordered-video replay requires an execution commit present on a remote ref")
...
def _full_population_input_parity(cfg: Any) -> dict[str, Any]:
    """Compare legacy and rolling paths over all 792 pre-H2D batches."""

    import torch
    from opentad.datasets import build_dataloader, build_dataset

    legacy_dataset = build_dataset(copy.deepcopy(cfg.dataset.test))
    rolling_dataset = build_dataset(copy.deepcopy(cfg.dataset.test))
    patch_frame_index_parity_tap(legacy_dataset)
    patch_frame_index_parity_tap(rolling_dataset)
    decode_reuse = patch_ordered_video_pipeline(rolling_dataset)
    loader_kwargs = dict(
        rank=0,
        world_size=1,
        shuffle=False,
    ...
    legacy_loader = build_dataloader(legacy_dataset, **loader_kwargs)
    rolling_loader = build_dataloader(rolling_dataset, **loader_kwargs)
    if len(legacy_loader) != EXPECTED_WINDOW_COUNT or len(rolling_loader) != EXPECTED_WINDOW_COUNT:
        raise ValueError("input-parity loaders changed the frozen 792-window population")

    legacy_digest = hashlib.sha256()
    rolling_digest = hashlib.sha256()
    frame_index_digest = hashlib.sha256()
    compared = 0
    for ordinal, (legacy, rolling) in enumerate(zip(legacy_loader, rolling_loader)):
        if set(legacy) != set(rolling):
            raise ValueError(f"pre-H2D batch keys differ at window {ordinal}")
        for key in ("inputs", "masks"):
            left, right = legacy.get(key), rolling.get(key)
            if not isinstance(left, torch.Tensor) or not isinstance(right, torch.Tensor):
                raise TypeError(f"pre-H2D {key} is not a tensor at window {ordinal}")
            if left.dtype != right.dtype or tuple(left.shape) != tuple(right.shape):
                raise ValueError(f"pre-H2D {key} contract differs at window {ordinal}")
            if not torch.equal(left, right):
                raise ValueError(f"pre-H2D {key} values differ at window {ordinal}")
            legacy_digest.update(left.contiguous().numpy().tobytes())
            rolling_digest.update(right.contiguous().numpy().tobytes())
        legacy_meta = _plain_mapping(legacy.get("metas"))
        rolling_meta = _plain_mapping(rolling.get("metas"))
        if legacy_meta != rolling_meta:
            raise ValueError(f"pre-H2D metadata differs at window {ordinal}")
        frame_identity = legacy_meta[0].get("frame_indices_sha256")
        if not isinstance(frame_identity, str) or len(frame_identity) != 64:
            raise ValueError(f"frame-index identity is missing at window {ordinal}")
        frame_index_digest.update(frame_identity.encode("ascii"))
        ...
        legacy_digest.update(encoded_meta)
        rolling_digest.update(encoded_meta)
        compared += 1

    if compared != EXPECTED_WINDOW_COUNT:
        raise ValueError("input-parity scan did not cover all 792 windows")
    stats = decode_reuse.stats()
    if stats.reader_open_count != EXPECTED_VIDEO_COUNT:
        raise ValueError("rolling decode did not open exactly one reader per validation video")
    if legacy_digest.hexdigest() != rolling_digest.hexdigest():
        raise RuntimeError("pre-H2D aggregate identities differ despite per-window parity")
    return {
        "status": "FULL_POPULATION_EXACT_PARITY",
        "video_count": EXPECTED_VIDEO_COUNT,
        "window_count": compared,
        "pre_h2d_sha256": legacy_digest.hexdigest(),
        "frame_indices_sha256": frame_index_digest.hexdigest(),
        "reader_open_count": stats.reader_open_count,
        "source_frame_request_count": stats.source_frame_request_count,
        "maximum_buffered_frames": stats.maximum_buffered_frames,
        "buffer_dtype": "uint8",
        "cache_location": "cpu",
...
def precheck(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if args.result_root.exists():
        raise FileExistsError(f"formal ordered-video result root already exists: {args.result_root}")
    ...
    if not str(args.result_root.resolve()).startswith(str(REMOTE_BOUNDARY.resolve()) + os.sep):
        raise ValueError("ordered-video result root leaves the shared remote write boundary")
    ...
    with tempfile.TemporaryDirectory(prefix="zoomtoken-ordered-reuse-precheck-") as temporary:
    ...
    parity_cfg = Config.fromfile(str(ROOT / ARM_SPECS["K100"]["config"]))
    _bind_test_config(parity_cfg, args)
    input_parity = _full_population_input_parity(parity_cfg)
    return {
        ...
        "ordered_video_input_parity": input_parity,
...
def _assert_metric_parity(arm: str, metrics: Mapping[str, float]) -> dict[str, Any]:
    ...
    frozen_raw = FROZEN_EVALUATOR_VECTOR[arm]
    if set(metrics) != set(frozen_raw):
        raise RuntimeError(f"{arm} evaluator vector keys differ from the frozen raw vector")
    metric_receipts = {}
    for key in ACCURACY_PARITY_METRIC_KEYS:
        ...
        exact_reference = float(frozen_raw[key])
        if raw_fraction != exact_reference:
            raise RuntimeError(
                f"{arm} evaluator raw vector differs at {key}: "
                f"{raw_fraction!r} != {exact_reference!r}"
            )
        ...
        metric_receipts[key] = {
            ...
            "frozen_evaluator_raw_fraction": exact_reference,
    ...
    return {
        "status": "EXACT_RAW_EVALUATOR_VECTOR_MATCH",
...
def _sample_identity(cpu_batch: Mapping[str, Any], ordinal: int) -> dict[str, Any]:
    ...
    if not isinstance(metas, list) or len(metas) != 1:
        raise ValueError("ordered-video profile requires batch-one metadata")
    meta = metas[0]
...
def _profile_one_pass(
    *,
    ...
    decode_reuse = patch_ordered_video_pipeline(dataset)
    ...
    if len(loader) != EXPECTED_WINDOW_COUNT:
        raise ValueError("ordered-video profile loader changed the frozen population")
    ...
    synchronize()
    decode_reuse.reset(clear_counters=True)
    iterator = iter(loader)
    ...
    if final_window is None or audit_receipt is None:
        raise RuntimeError("ordered-video profile pass did not reach final NMS")
    reuse_stats = decode_reuse.stats()
    if reuse_stats.reader_open_count != EXPECTED_VIDEO_COUNT:
        raise RuntimeError("measured pass did not open exactly one reader per validation video")
    ...
    receipt = {
        ...
        "ordered_video_decode_reuse": {
            **reuse_stats.__dict__,
            "buffer_dtype": "uint8",
            "cache_location": "cpu",
            "asynchronous_prefetch": False,
            "cross_pass_state": False,
            "decode_inside_timed_next_batch": True,
        },
    }
...
def _gpu_identity(torch: Any) -> dict[str, Any]:
    ...
    if not selector or "," in selector or torch.cuda.device_count() != 1:
        raise RuntimeError("ordered-video profile requires exactly one Slurm-visible GPU")
...
def run_power_sidecar(args: argparse.Namespace) -> int:
    ...
    if len(allocated) != 6 or args.sidecar_cpu not in allocated:
        raise ValueError("power sidecar requires one CPU from the six-CPU allocation")
...
def _persist_acquisition_checkpoint(
    *,
    ...
    state = {
        "schema_version": "zoomtoken_ordered_video_reuse_r1_k100_cost_v001",
...
def _classify_complete_replay(
    ...
    allocated_ratio = float(r1["peak_gpu_allocated_mb"]) / float(
        k100["peak_gpu_allocated_mb"]
    )
    reserved_ratio = float(r1["peak_gpu_reserved_mb"]) / float(
        k100["peak_gpu_reserved_mb"]
    )
    ...
    if prediction_hash_variation:
        raise RuntimeError("within-arm prediction identity is incomplete")
    all_gates_pass = (
        p50_ratio <= 0.95
        and energy_ratio <= 0.95
        and allocated_ratio <= 1.05
        and reserved_ratio <= 1.05
    )
    decision = (
        "SURVIVE_ORDERED_VIDEO_REUSE_DEPLOYMENT_PENDING_FRESH_PRO"
        if all_gates_pass
        else "STOP_R1_CURRENT_CONTIGUOUS_SUPPORT_AS_SINGLE_GPU_EFFICIENCY_ROUTE"
    )
    return {
        "decision": decision,
        "end_to_end_p50_ratio": p50_ratio,
        "complete_pass_gross_energy_ratio": energy_ratio,
        "peak_gpu_allocated_memory_ratio": allocated_ratio,
        "peak_gpu_reserved_memory_ratio": reserved_ratio,
        ...
        "accuracy_role": "identity_diagnostic_not_a_cost_gate",
        "prediction_hash_variation_diagnostic": prediction_hash_variation,
        "rule": {
            "survive": "R1/K100 median4 wall-time<=0.95, gross energy<=0.95, allocated memory<=1.05, reserved memory<=1.05",
            "stop": "any one of the four frozen ratios fails",
        },
    }
...
def profile(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if not os.environ.get("SLURM_JOB_ID") or int(os.environ.get("SLURM_CPUS_PER_TASK", -1)) != 6:
        raise RuntimeError("formal ordered-video profile requires one Slurm GPU and six CPUs")
    if (int(os.environ.get("LOCAL_RANK", -1)), int(os.environ.get("RANK", -1)), int(os.environ.get("WORLD_SIZE", -1))) != (0, 0, 1):
        raise RuntimeError("formal ordered-video profile requires torchrun world-size one")
    ...
    if len(allocated) != 6:
        raise RuntimeError("formal ordered-video profile requires exactly six allocated CPUs")
    ...
    profile_payload = {
        "schema_version": "zoomtoken_ordered_video_reuse_r1_k100_fullstack_v001",
