...
S2_TRAINING_RUNTIME_PRECHECK_SCHEMA = (
    "continuous_roi_s2_training_runtime_precheck_v2"
)
...
def validate_full_model_gate(
    ...
    if gate.get("checkpoint_sha256") != load_protocol()["data"][
    ...
    source_hashes = gate.get("code_provenance", {}).get(
    ...
    implementation_hash = implementation_copy.pop(
        "implementation_audit_sha256", None
    )
    if (
        ...
        != load_protocol()["declared_protocol_sha256"]
        or set(implementation_audit.get("config_hashes", {}))
        != set(S2_FAMILIES)
        or set(implementation_audit.get("pipeline_audits", {}))
        != set(S2_FAMILIES)
        or implementation_audit.get(
            "detector_model_surface_matches_reference"
        )
        is not True
        or implementation_audit.get("post_processing_matches_reference")
        is not True
        ...
        or optimizer_audit.get("every_requires_grad_parameter_exactly_once")
        is not True
        ...
        raise ValueError(
            "Continuous-RoI S2 full-model Gate evidence is incomplete"
        )
    return gate
    ...
    *,
    ...
    binding_invariants = (
        ...
        and all(
            ...
            and int(cell.get("successful_updates", -1))
            == S2_SUCCESSFUL_UPDATES
            and int(cell.get("updates_per_epoch", -1))
    ...
    if family_invariants:
        for family, (policy, dense_shape) in expected_sample_policies.items():
            ...
            if (
                ...
                or audit.get("train_batches_per_epoch")
                != S2_UPDATES_PER_EPOCH
                or not isinstance(
                    audit.get("gate_window_identity_sha256"), str
                )
                ...
                break
            if dense_shape is not None and real_batch.get(
                "input_shapes", {}
            ).get("dense") != dense_shape:
                family_invariants = False
                break
    ...
    if (
        ...
        or not isinstance(
            precheck.get("development_gate_window_identity_sha256"), str
        )
...
def validate_external_training_inputs(
    *,
    ...
    file_hashes = {
        ...
        "development_annotation_sha256": sha256_file(
...
def _source_config_path(
    family: str, *, repository_root: str | Path = ROOT
) -> Path:
...
def bind_training_config(
    *,
    ...
    gate = validate_full_model_gate(
        full_model_gate_path, expected_commit=code_commit
    )
    ...
    if training_runtime_precheck_path is not None:
        ...
        raise ValueError(
            "formal Continuous-RoI S2 binding requires a runtime precheck"
        )
    ...
    if runtime_authorization_path is not None:
        ...
        if (
            ...
            raise ValueError(
                "runtime authorization belongs to another base experiment"
            )
    ...
    expected_work_dir = (
        Path(canonical_experiment_root)
        / family.lower()
        / f"seed{seed}"
    ).resolve()
    if work_dir != expected_work_dir:
        raise ValueError(
            "formal Continuous-RoI S2 work_dir must equal "
            f"{expected_work_dir}"
        )
    ...
    binding = {
        ...
        "manifest_file_sha256": external["file_hashes"][
            "base_manifest_file_sha256"
        ],
        "manifest_semantic_sha256": manifest["manifest_sha256"],
        "development_annotation_path": str(
            Path(development_annotation_path).resolve()
        ),
        ...
        "development_video_inventory_sha256": development_inventory[
            "inventory_sha256"
        ],
        "fit_split_sha256": manifest["split_hashes"]["fit"],
        "gate_split_sha256": manifest["split_hashes"]["gate"],
...
def validate_bound_training_config(cfg: Config, *, seed: int) -> dict[str, Any]:
    ...
    if not isinstance(binding, Mapping):
        raise ValueError(
            "Continuous-RoI S2 source configs are not directly trainable"
        )
...
def build_checkpoint_metadata(
    ...
    *,
    ...
    metadata = {
        ...
        "development_annotation_sha256": binding[
            "development_annotation_sha256"
        ],
        ...
        "pretrained_checkpoint_sha256": binding[
            "pretrained_checkpoint_sha256"
        ],
        "full_model_gate_file_sha256": binding[
            "full_model_gate_file_sha256"
        ],
        ...
        "runtime_authorization_sha256": binding[
            "runtime_authorization_sha256"
        ],
...
def build_training_completion(
    *,
    ...
    if (
        ...
        raise ValueError(
            "Continuous-RoI S2 final checkpoint lacks its bound EMA state"
        )
    del checkpoint
    report = {
