...
def parse_annotation(args):
    ...
    train_path = os.path.join(args.csv_dir, "EPIC_100_train.csv")
    val_path = os.path.join(args.csv_dir, "EPIC_100_validation.csv")
    test_path = os.path.join(args.csv_dir, "EPIC_100_test_timestamps.csv")
                ...
                for _, row in filter_anno.iterrows():
                    ...
                    annotations.append(
                        dict(
                            ...
                            verb_label=f"id_{row['verb_class']:03d}",
                            verb_label_name=row["verb"],
                            noun_label=f"id_{row['noun_class']:03d}",
                            noun_label_name=row["noun"],
                        )
                    )
    ...
    json_path = os.path.join(args.save_path, "epic_kitchens_full.json")
    with open(json_path, "w") as f:
        json.dump(out, f)
    print(f"The full annotation is saved at {json_path}")

    # parse only verb and noun annotations
    def save_subset(database, subset_name):
        sub_database = {}
        for video_id in database.keys():
            annotation_verb = []
            if database[video_id]["subset"] != "test":
                for anno in database[video_id]["annotations"]:
                    annotation_verb.append(
                        dict(
                            segment=anno["segment"],
                            label=anno[subset_name + "_label"],
                            label_name=anno[subset_name + "_label_name"],
                        )
                    )

                sub_database[video_id] = dict(
                    subset=database[video_id]["subset"],
                    duration=database[video_id]["duration"],
                    annotations=annotation_verb,
                )
            else:
                sub_database[video_id] = dict(
                    subset=database[video_id]["subset"],
        ...
        out = dict(version=f"epic-kitchens-100-{subset_name}", database=sub_database)
        json_path = os.path.join(args.save_path, f"epic_kitchens_{subset_name}.json")
        with open(json_path, "w") as f:
            json.dump(out, f)

        print(f"The {subset_name} annotation is saved at {json_path}")

    save_subset(database, "verb")
    save_subset(database, "noun")
...
if __name__ == "__main__":
    ...
    # python tools/prepare_data/convert_epic_kitchens_anno.py data/epic_kitchens-100/annotation data/epic_kitchens-100/annotations
