...
- **唯一下一任务：** Pro 冻结 `ZOOMTOKEN-GRIDFUSE32-L6-GATED-v001`：保持 R1 K64、全部 8 个 temporal tubelet 和每层 dense Adapter；blocks 6–11 对每 tubelet 固定相邻 pair 做 `64→32` 融合，在 N256 上执行完整 Q/K/V/MLP，再把 block residual 广播回 N512。先执行 G0 六-block segment 门（p50 speedup `>=1.35x`，peak allocated/reserved ratio 均 `<=1.05`）；只有 G0 全通过才运行唯一 seed-42 G1，只有 G1 全通过才运行 matched full-stack G2。任何前门失败都直接形成终态并返回 fresh Pro。
