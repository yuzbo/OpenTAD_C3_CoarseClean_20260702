...
- **当前唯一候选输入：** 用户手工转交的 Pro 材料给出 `PIVOT / KEEP`，提出 `ZoomToken-RACER24`：BPNS K64 支持、blocks `{4,6,8,10}` 的 per-tubelet 24/64 selected-Q/full-KV、parameter-free residual completion、dense carrier 与全 Adapter。仓库中尚无对应 Project conversation/nonce/附件/提交/terminal receipt，材料还错误地把已终态的 job `1258299` 写成运行中，并错误声称 base 已有 selected-Q/full-KV helper。因此当前只把它摄取为 provenance-warning 下的 Iteration-0 Builder 规划输入；没有代码实现、microbenchmark、训练或成本证据，也没有新的北京时间 deadline。
