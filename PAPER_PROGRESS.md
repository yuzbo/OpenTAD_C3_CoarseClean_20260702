...
- **当前唯一候选输入：** 用户手工转交的 Pro 材料给出 `PIVOT / KEEP`，提出 `ZoomToken-RACER24`：BPNS K64 支持、blocks `{4,6,8,10}` 的 per-tubelet 24/64 selected-Q/full-KV、parameter-free residual completion、dense carrier 与全 Adapter。材料仍缺 Project conversation/nonce/附件/Oracle receipt，并含两项已拒绝的错误事实，因此 provenance 保持 `USER_MANUAL_TRANSFER_NOT_BROWSER_AUDITED`。用户随后明确授权 Iteration-0；clean candidate `5ebaa74f…` 已完成 Builder、两轮确定性修正、fresh Critic `PASS`、N16R4 `16 passed` 与 result-blind Evaluator `PRE_RUN_READY`。唯一冻结单卡微基准 job `1262068` 已提交；终态前没有速度、显存、训练或 full-stack 成本证据。
