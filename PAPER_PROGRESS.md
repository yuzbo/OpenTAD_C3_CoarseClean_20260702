...
- **唯一当前任务：** `ZOOMTOKEN-R1-DEPTH-SPARSITY-READONLY-FOUR-ARM-FULLSTACK-PARETO-CLOSURE-v001`。最新实现为 [GitHub exact commit `b82441c1aa2663069033d394794298d5c723bbb6`](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/b82441c1aa2663069033d394794298d5c723bbb6)，local/N16R4 focused tests 为 `22 passed, 1 skipped / 23 passed`，fresh Critic `PASS`、fresh result-blind Evaluator `PRE_RUN_READY`，precheck job `1262119` 已 `COMPLETED 0:0 / PRECHECK_READY`。唯一 formal job `1262120` 已按 `1 GPU / 5 CPU / 16h` 和 16-pass Williams order 提交，submission `1/1`、无 replacement；终态前不读取 partial 指标。不训练、不改模型，只对 R1/FULL64、DSR6-KV、MOD32-KV、DROP32 epoch-59 EMA 做完整 decode→Soft-NMS 成本闭环；任何候选必须同时达到 p50/energy ratio `<=0.95` 且 allocated/reserved ratio `<=1.05`。完整终态或 blocker 后强制 fresh Pro，并在 prompt 中绑定 repository、branch 与 exact commit URL。
