...
- **唯一下一任务：** `ZOOMTOKEN-R1-DEPTH-SPARSITY-READONLY-FOUR-ARM-FULLSTACK-PARETO-CLOSURE-v001`。不训练、不改模型，只对现有 R1/FULL64、DSR6-KV、MOD32-KV、DROP32 epoch-59 EMA checkpoint 在同一 GPU、完整 decode→Soft-NMS 链路上做 16-pass Williams-order 成本闭环；每臂以四个完整 pass 的 p50 与 total joules 中位数为主估计。任何候选必须同时达到 p50/energy ratio `<=0.95` 且 allocated/reserved ratio `<=1.05`。唯一 formal job `1/1`、无 replacement，完整终态或 blocker 后强制 fresh Pro。
