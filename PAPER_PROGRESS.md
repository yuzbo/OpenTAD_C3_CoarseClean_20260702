...
- **K100-TAR50 终态：** 唯一 formal job `1261680` 已在 `g0087` 终态 `FAILED 1:0`：epoch 0 首个成功 optimizer update 前，通用 successful-update hook 因 strict A-MoD backbone 不是 GeoRoute 而抛出 `ValueError`。没有 checkpoint、official prediction/vector、短动作、边界或成本证据，因此这是 `ENGINEERING_OR_PROTOCOL_BLOCKER`，不是固定半更新方法的准确率负结果；正式提交 `1/1` 已用尽。终态同时确认 full-800 K/V、per-tubelet K50 文案与实际 global selected-400 K/V 实现冲突。
- **RACER24 Iteration-0 终态：** clean candidate `5ebaa74f…` 完成 Builder、确定性修正、fresh Critic `PASS`、N16R4 focused tests 与 result-blind Evaluator `PRE_RUN_READY`。唯一有效单卡微基准 job `1262068` 完整测得：RACER24/dense 的 p50 speedup 仅 `0.24964x`，peak allocated/reserved memory ratio 为 `1.98884/1.84615`，全部未通过 `>=1.08x` 与 `<=1.05` 冻结门。fresh exact-Project Pro 已裁决 `PIVOT / STOP_RACER24_ITERATION0 / KEEP`；该结果是在 pre-push deployment deviation 披露下的 decision-grade negative，不是 claim-grade，不重跑、不训练、不救援，也不外推为整个 selected-Q/full-KV/completion/CPTC 家族失败。
- **GridFuse32-L6 终态与最新 Pro：** latest implementation 为 [GitHub exact commit `dde46aee17f10bf793e5407055fc7b3416d93205`](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/dde46aee17f10bf793e5407055fc7b3416d93205)。job `1262108` 的 dense/candidate p50 为 `178.500099/314.885696 ms`，speedup `0.56687268x`，allocated/reserved ratio 均为 `1.0`。fresh exact-Project `GPT-5.6 Pro` 已裁决 `PIVOT / KEEP`：这是 `PROTOCOL_COMPLETE_VALID_G0_NEGATIVE`，但 exact config 保留旧 task/failure 元数据，故为 decision-grade、非 claim-grade。exact GridFuse32-L6 永久停止，不修复、不重跑、不开 G1/G2。
- **最新四臂终态：** `ZOOMTOKEN-R1-DEPTH-SPARSITY-READONLY-FOUR-ARM-FULLSTACK-PARETO-CLOSURE-v001` 的唯一 job `1262120` 已 `COMPLETED 0:0`。最新实现为 [GitHub exact commit `b82441c1aa2663069033d394794298d5c723bbb6`](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/b82441c1aa2663069033d394794298d5c723bbb6)，完整 16-pass、211/792 population、12,672 cost rows、2,527,501 power samples、prediction/evaluator、短动作、边界、profile 与 terminal receipt 均通过独立完整性核验。相对 A/R1-FULL64，B/DSR6-KV、C/MOD32-KV、D/DROP32 的 p50 比值为 `1.112325/1.110211/1.102807`，gross-energy 比值为 `1.102719/1.099873/1.064409`；无 arm 通过冻结联合门。终态为 `STOP_R1_FIXED_DEPTH_SPARSITY_FOURPOINT_AS_CURRENT_EFFICIENCY_ROUTE`。这是四个固定点在单 RTX 4090、完整 THUMOS14 validation 成本协议下的有效负结果；fresh Pro 前不授权 successor。
- **最新 Pro 与唯一下一任务：** fresh exact-Project conversation `6a95f0ff-f7f4-83ea-ba00-18ea97d18da7` 以 browser-visible `GPT-5.6 Pro`、十个 attachment-only 文件、一次科研提交和零 follow-up 返回 `PIVOT / KEEP`。Pro 接受 ordered-video reuse 的预注册停止边界，确认其是 `PASS_STRONG / VALID_WITH_DISCLOSED_SCOPE_LIMITS / VALID_NEGATIVE_FOR_STANDALONE_SINGLE_GPU_FULL_STACK_LATENCY_ROUTE`；BPNS-R1 不再作为当前 standalone efficiency 路线。唯一任务冻结为 `ZOOMTOKEN-CONTINUOUS-ROI-S2-V2.2-REFERENCE-PROTOCOL-CORRIGENDUM-AND-KNOWN-ANSWER-CLOSURE-v001`：从 `9e774eee…` 只修改四个协议/验证文件，用 CPU-only known-answer 闭合 shared physical center、deterministic Sobol、equal privilege、no-GT 和 exact-nine/129-window 身份；任务不授权训练、raw inference、metric、cost、S3 或 official test。
- **Continuous-RoI S2 v2.2 终态：** final clean/pushed candidate 为 [GitHub exact commit `10aed28659a08fa703def278fc0f5f1422dcad89`](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/10aed28659a08fa703def278fc0f5f1422dcad89)，N16R4/Linux `12 passed`、fresh Critic `PASS`、fresh result-blind Evaluator `PRE_RUN_READY`。唯一 CPU closure 精确重现 17-candidate Sobol manifest 与 129-window population，但 exact-nine 的九个 `epoch_59.pth` 及九个 metadata sidecar 均已不在冻结 campaign root，因此受控终态为 `STOP_CONTINUOUS_ROI_S2_REFERENCE_ROUTE_BEFORE_INFERENCE`。没有训练、推理、metric、cost、performance access 或 official-test opening；这是 reference-artifact availability stop，不是 Continuous-RoI 科学负结果，下一动作只能是 fresh Pro。
- **最新 Pro 与当前唯一任务：** fresh exact-Project conversation `6a961c52-1b5c-83e9-b04c-6b0bda18892f` 以 browser-visible `GPT-5.6 Pro`、13 个 attachment-only logical files、一次科研提交和零 follow-up 返回 `PIVOT / KEEP`。Pro 接受 exact-byte `0/18` 的永久历史路线关闭，但维持 `NO_MODEL_SCIENTIFIC_RESULT`。唯一任务冻结为 `ZOOMTOKEN-CONTINUOUS-ROI-S2-V3-FRESH-3X3-MATCHED-TRAIN-REFERENCE-AND-COST-FALSIFIER-v001`：从 `10aed286…` 新建 D160/G96/U128 × seeds 4407/4408/4409 的完整九单元 60-epoch 矩阵；全部 strict-finalize 后才做冻结 reference，且只有 `S_CR=true && H=true` 才允许一次 matched full-stack cost。最终继续门为 `S_CR && H && F`；official test、旧 artifact 复用、sweep、replacement、rescue 和 second campaign 均禁止。
- **2026-09-01 完整数据硬约束与当前暂停：** 用户现要求所有正式 baseline/candidate 都覆盖完整官方训练 population 与训练计划、同一个完整官方 validation/evaluation population，并在 Pro 预注册的一次性无泄漏协议下覆盖同一个完整 official test population。该要求与上述旧 `fit160/gate40 + 129 ordered windows + official-test closed` 协议发生实质冲突，因此旧 S2-v3 部署已标记 `HELD_FOR_FULL_DATA_PROTOCOL_REVISION`。目前只有 Builder plan 和本地未提交 WIP；没有 candidate commit、正式提交、训练、验证、测试、metric 或 cost 结果。新的 `CURRENT_RESEARCH_STATE-v020`、`MODEL_EXPERIMENT_HISTORY-v015` 与一次性 fresh Pro 请求已准备并进入中央浏览器串行队列；lease/Source 确认/Pro 裁决前不执行旧实验。
- **主目标进一步简化：** 用户不接受准确率、延迟、显存/计算量三者的联合硬门。待 fresh Pro 必须只选一个二维主问题：`检测性能↔计算量` 或 `检测性能↔峰值显存`；latency 降为系统披露/瓶颈诊断，energy 默认辅助。该要求在旧 Sources mutation 前到达，因此 v020/v015 lease 已 attempt-0 退回、零浏览器接触/零上传；将由合并后的 v021/v016 与 request-v002 替代，Codex 未预选资源轴或阈值。
- **最新单轴 Pro 裁决与唯一任务：** fresh exact-Project conversation [`6a9641aa-a8f8-83ea-a5e7-b64da4daffc2`](https://chatgpt.com/g/g-p-6a79701398bc8191a9ef61db6302b24b-zoomtoken/c/6a9641aa-a8f8-83ea-a5e7-b64da4daffc2) 以 browser-picker verified `Pro`、一个 attachment-only 请求、一次科研提交和零 follow-up 返回 `REVISE / KEEP`。唯一主轴确定为完整数据上的 `检测性能↔实际执行推理计算量`，峰值显存不作为第二主轴，latency/throughput/memory/energy 仅披露。唯一任务 `ZOOMTOKEN-CONTINUOUS-ROI-S2-V3-FULL200-COMPUTE-PARETO-3X3-v001` 比较 D160/G96/U128-A0 × seeds 4407/4408/4409：每个 cell 完整训练 200 videos、60 epochs/6,000 successful updates，9/9 final EMA/predictions 封存后对完整 211-video/792-window population 一次性无泄漏评测；唯一资源门为完整算子 `C_exec(U128-A0)/C_exec(D160) <= 0.90`。当前只有经审计 Pro 裁决和 Builder plan，没有新候选、训练或模型结果。
- **ordered-video reuse 终态：** 最小 clean/pushed candidate 为 [GitHub exact commit `6139e793e530033e2af6d992819cbe327d5bbd86`](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/commit/6139e793e530033e2af6d992819cbe327d5bbd86)，focused tests `8 passed`，fresh re-Critic `PASS`、fresh result-blind Evaluator `PRE_RUN_READY`。precheck `1262717` 证明完整 211/792 legacy-vs-rolling pre-H2D exact parity；唯一 formal job `1262753` 已 `COMPLETED 0:0`，形成八 pass、6,336 cost rows、485,950 power rows、预测/evaluator identity、profile 与 terminal receipt。R1/K100 的 median-four p50/energy/allocated/reserved 比值为 `0.980999/0.921086/0.751260/0.689655`：能耗和显存通过，但 p50 仅改善 `1.90%`，未达到 `5%` 门。冻结结论是 `STOP_R1_CURRENT_CONTIGUOUS_SUPPORT_AS_SINGLE_GPU_EFFICIENCY_ROUTE`；这是完整 validation/single-RTX4090 上的有效负系统结果，fresh Pro 前不授权 successor。
- **证据边界：** v004 的 prediction identity、6,336 cost rows、929,889 power rows、profile、terminal receipt 和离线诊断均完整。新四臂 closure 的 16-pass 证据也完整，且所有 power coverage 通过；但它仍是 validation-only、单硬件、冻结历史 checkpoint 的只读成本回放，不是 official-test、matched-training、多种子或跨硬件证据。R1−K100 与四臂准确率/边界变化均为诊断，不能主张边界保护。v004 第 3 号 K100 pass 的约 `2.805 s` 功耗采样间隙仍需保留披露，但不影响新四臂独立终态。
...
- 更新时间：2026-08-29（BPNS-R1 v004 已停止独立效率 headline。TAR32-FKV replacement job `1261142` 已完成身份有效的 final-EMA official validation；三项主准确率门和短动作门失败、两项边界门通过，exact composition 终态为有效负结果，成本与 successor 冻结并等待 mandatory fresh Pro）。
...
TAR32 replacement job `1261142` 已形成冻结、有效的 seed-42 准确率负结果：官方未舍入 Average-mAP/mAP@0.6/mAP@0.7 为 `64.9811/57.3707/43.6691`，短动作下降 `3.3176 pp`，因此 fresh Pro 固定 `STOP_R1_TAR32_FKV_EXACT_COMPOSITION` 并禁止该组合的成本、第三次评测、重训、附加 seed 与残差补救。该结果只否定 R1/K64 与固定半更新叠加的精确组合。其后唯一 K100-TAR50 formal job `1261680` 没有形成科学结果：首个成功更新前因训练钩子与非-GeoRoute backbone 不兼容而终止，且终态审计暴露 full-K/V/per-tubelet-K50 文案与继承 strict A-MoD selected-only attention 实现不一致。正式提交上限已耗尽，当前只允许 fresh Pro 独立裁决，不自动修复、重跑或测成本。

Continuous-RoI S2 v2.2 只形成协议与 artifact-retention 证据。四文件 protocol candidate `10aed286…` 正确重现 17 个 Sobol candidate 与 129-window sanitized development population，但历史 exact-nine 的 9 个 final-EMA checkpoint 和 9 个 sidecar 均缺失。fresh Pro 随后只授权一次有限 exact-byte provenance census；clean/pushed candidate `058095cc…` 的唯一 formal scan 对 18 项的匹配结果为 `0/18`，没有候选 payload、重建、quarantine、GPU、模型 forward、prediction、metric、cost 或 official-test access，原 campaign root 前后未改变。因此冻结 `STOP_CONTINUOUS_ROI_S2_HISTORICAL_REFERENCE_ROUTE_ARTIFACTS_UNRECOVERABLE`，永久关闭历史 reference route。该终态不能进入性能结果表，也不能支持或反驳 Continuous-RoI/crop sufficiency。fresh Pro 已据此下达全新 S2-v3 3×3 matched falsifier；它不是恢复旧 checkpoint，必须用完整九单元训练、冻结 development reference 和条件成本形成第一份模型级证据。
