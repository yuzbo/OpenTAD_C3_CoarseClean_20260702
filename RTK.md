...
- computer-use 只负责创建或进入正确 Project、上传 Source 并确认远端文件名。Oracle 只负责在精确 Project URL 中创建全新对话、选择最高可验证的 Pro 模型、提交一次提示词并保存完整回复；不得依赖当前聚焦标签页、标签页序号、旧对话或 follow-up。
- Oracle 从提交前路由核验开始一直持有 profile 锁，直到完整回复、session 元数据、conversation URL 和终态报告均已保存。Pro 长时间思考本身不是失败；连续 30 分钟无新输出时，只允许对原会话进行一次只读存活检查，不得重新提交或切换 Project。
- 只有使用不同 iXBrowser profile、独立运行端点和独立会话目录时才允许并行 Pro 讨论；共享同一 profile 的多个项目必须严格串行。
