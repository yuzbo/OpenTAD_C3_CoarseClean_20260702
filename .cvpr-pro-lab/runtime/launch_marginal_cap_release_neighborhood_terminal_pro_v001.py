#!/usr/bin/env python3
"""Launch one exact-DUCA Pro adjudication for the joint-neighborhood terminal result."""
...
PROMPT = ROOT / ".cvpr-pro-lab/pro-reviews/prompts/PRO_DUCA_MARGINAL_CAP_RELEASE_NEIGHBORHOOD_TERMINAL_ADJUDICATION-v001.md"
...
RUN_DIR = ROOT / ".cvpr-pro-lab/pro-reviews/runs/duca-marginal-cap-release-neighborhood-terminal-compact-v002"
ORACLE_HOME = ROOT / ".cvpr-pro-lab/oracle/duca-marginal-cap-release-neighborhood-terminal-compact-v002"
...
def main() -> int:
    ...
    if head != LATEST_COMMIT or dirty:
        raise RuntimeError(f"latest implementation identity mismatch: head={head}, dirty={bool(dirty)}")
    ...
    manifest = {
        ...
        "started_at": now(),
        "transport_recovery_from": {
            "run": "duca-marginal-cap-release-neighborhood-terminal-v001",
            "classification": "PRE_SUBMIT_OVERSIZED_INLINE_BUNDLE",
    ...
    with contextlib.ExitStack() as stack:
        ...
        command = [
            ...
            "--wait",
            "--verbose",
            "--slug", "duca-marginal-cap-neighborhood-terminal-v002",
