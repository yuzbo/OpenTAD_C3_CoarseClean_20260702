#!/usr/bin/env python3
"""Launch one exact-DUCA Pro adjudication without restarting profile 61."""
...
PROMPT = ROOT / ".cvpr-pro-lab/pro-reviews/prompts/PRO_DUCA_MARGINAL_ORACLE_GRAY_ZONE_ADJUDICATION-v001.md"
...
RUN_DIR = ROOT / ".cvpr-pro-lab/pro-reviews/runs/duca-marginal-oracle-gray-zone-v001"
MATERIAL_DIR = RUN_DIR / "materials"
ORACLE_HOME = ROOT / ".cvpr-pro-lab/oracle/duca-marginal-oracle-gray-zone-v001"
...
LATEST_COMMIT = "f67d96fdf68a295eaa7f678f3dfc125530828889"
GITHUB = {
    "repository": "https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702",
    "branch": "https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/tree/feature/duca-marginal-budget-v1-20260830",
...
def discover_profile_cdp() -> str:
    ...
    matches = [
        ...
        if int(row.get("profile_id", -1)) == PROFILE_ID
        and row.get("debugging_address")
    ]
    if len(matches) != 1:
...
def main() -> int:
    ...
    materials = [MATERIAL_DIR / name for name in MATERIAL_NAMES]
    missing = [str(path) for path in materials if not path.is_file()]
    if missing:
        raise FileNotFoundError(f"missing Pro material files: {missing}")
    ...
    with contextlib.ExitStack() as stack:
        ...
        command = [
            ...
            "--wait",
            "--verbose",
            "--slug", "duca-marginal-gray-zone-v001",
            "--write-output", str(visible_report),
            "--prompt",
            (
                "Read every attached file completely and answer the authoritative prompt as one "
                "independent scientific adjudication. Treat the GitHub repository, branch, exact "
                f"commit {LATEST_COMMIT}, and key-file URLs in the prompt as the latest code truth. "
    ...
    report_text = (
        visible_report.read_text(encoding="utf-8", errors="replace")
        if visible_report.is_file()
        else ""
    )
