#!/usr/bin/env python3
"""Launch one exact-DUCA Pro turn for the multi-budget adaptation freeze."""
...
PROMPT = ROOT / ".cvpr-pro-lab/pro-reviews/prompts/PRO_DUCA_MULTI_BUDGET_DETECTOR_ADAPTATION_FREEZE-v001.md"
...
RUN_DIR = ROOT / ".cvpr-pro-lab/pro-reviews/runs/duca-multi-budget-detector-adaptation-freeze-v001"
ATTEMPT_DIR = RUN_DIR / "transport-recovery1"
ORACLE_HOME = ROOT / ".cvpr-pro-lab/oracle/duca-multi-budget-detector-adaptation-freeze-v001"
...
MATERIALS = (
    ...
    ROOT / "research-wiki/sources/2026-08-31-duca-multi-budget-detector-adaptation-revise.md",
)
...
def main() -> int:
    ...
    raw_log = ATTEMPT_DIR / "oracle.log"
    ...
    with contextlib.ExitStack() as stack:
        ...
        command = [
            ...
            "--wait",
            "--verbose",
            "--slug", "duca-multi-budget-adaptation-freeze-v001-recovery1",
            "--write-output", str(visible_report),
            "--prompt",
            (
                "Read every provided file completely and independently adjudicate the one new detector-adaptation question. "
                "Preserve the old stopped-route boundary, make every scientific choice yourself, and freeze the two missing "
                f"execution decisions. Preserve nonce {NONCE} verbatim in the response."
            ),
            "--file", str(PROMPT),
        ]
