#!/usr/bin/env python3
"""Launch one exact-DUCA Pro turn for the comprehensive route integration review."""
...
GEMINI_RAW_REPORT = Path(r"C:\Users\skywalker\.fastctx\jobs\j-02swjq\output.log")
PROMPT = ROOT / ".cvpr-pro-lab/pro-reviews/prompts/PRO_DUCA_COMPREHENSIVE_ROUTE_INTEGRATION-v001.md"
...
RUN_DIR = ROOT / ".cvpr-pro-lab/pro-reviews/runs/duca-comprehensive-route-integration-v001"
ORACLE_HOME = ROOT / ".cvpr-pro-lab/oracle/duca-comprehensive-route-integration-v001"
...
MATERIALS = (
    GEMINI_RAW_REPORT,
    ROOT / ".cvpr-pro-lab/pro-reviews/materials/AGY_GEMINI_DUCA_ADVISORY_REVIEW-v001.md",
...
def main() -> int:
    ...
    with contextlib.ExitStack() as stack:
        ...
        command = [
            ...
            "--heartbeat", "30",
            "--wait",
            # The prior identical local Oracle session was killed before browser
            # submission; bypass only its stale duplicate-prompt registry entry.
            "--force",
            "--verbose",
            "--slug", "duca-comprehensive-route-integration-v001",
            "--write-output", str(visible_report),
            "--prompt",
            (
                "Read every supplied file completely and inspect every cited GitHub implementation at the exact commit. "
                "Act as the independent scientific head: reconcile all DUCA routes, code, results and failures; select one "
                "publishable research path and one next task without delegating route choice to Codex. Preserve nonce "
                f"{NONCE} verbatim and end with DUCA_COMPREHENSIVE_RESEARCH_DECISION_READY."
            ),
            "--file", str(PROMPT),
        ]
    ...
    terminator_bound = report_text.rstrip().endswith("DUCA_COMPREHENSIVE_RESEARCH_DECISION_READY")
    complete = bool(
        return_code == 0
