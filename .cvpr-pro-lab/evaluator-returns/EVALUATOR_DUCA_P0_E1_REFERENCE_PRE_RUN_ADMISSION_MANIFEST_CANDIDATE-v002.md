...
queue_message_id: msg-20260812T211745Z-12c5f7671b38
...
critic_correction: CRITIC_DUCA_P0_E1_REFERENCE_PRE_RUN_ADMISSION_MANIFEST_STATIC_REVIEW-v001
supersedes: EVALUATOR_DUCA_P0_E1_REFERENCE_PRE_RUN_ADMISSION_MANIFEST_CANDIDATE-v001
...
This v002 candidate consumes exactly durable correction queue
`msg-20260812T211745Z-12c5f7671b38`. It applies only deterministic Critic
findings `P0-E1-MANIFEST-001` and `P0-E1-MANIFEST-002`. It does not authorize
E1, create an execution queue, or establish `PRE_RUN_READY`. No command was
issued and no runtime output or implementation surface was accessed.
...
ownership; E1-before-B1 role separation; one shot; first fail; no hash,
tolerance, recomputation, second solver, fallback, repair, or new fixture.
...
- Existing shared identity-gate parent, which Coordinator must separately admit:
...
The shared parent must already exist as the exact project-local directory and
must be named in the future Coordinator admission record. E1 may not create,
replace, or modify that parent. No path inference, alias, alternate root,
relative path, or recursive parent creation is admitted.

## 2. Environment bindings and denials

The later exact E1 queue must set these task-specific values without loading
modules or activating a training environment:
...
The admission value is only a binding to a separately existing Coordinator
record; it is not self-authorization. `-B` disables bytecode writes. The E1
process may read only the four literal Evaluator source/input paths and Python
...
the marker lines and Markdown fence. There is no shell interpolation. The
deliberate future-queue sentinel remains an admission blocker.

```text
argv[0]  = /data/run01/sczc063/yuzibo/conda_envs/opentad/bin/python
argv[1]  = -B
argv[2]  = -c
argv[3]  = E1_INLINE_PROGRAM-v002 defined below
...
PROJECT_ID = "g-p-6a796fef9a00819194024cf1de3bd697"
PARENT_DECISION = "PRO_P0_IDENTITY_GATE_AUTHORIZATION-v001"
MANIFEST_VERSION = "v002"
AUTHORING_QUEUE_ID = "msg-20260812T211745Z-12c5f7671b38"
...
TIE_REQUIREMENTS = {
    "G17-E2": (),
    "G17-EINF": (("E2", 2),),
    "G17-E1": (("E2_E_infinity", 2),),
    "G17-U1": (("E2_E_infinity_E1", 2),),
    "G17-PLEX": (("E2_E_infinity_E1_U1", 2),),
    "G385-X": (("E2_E_infinity_E1_U1", 2),),
}
...
def require_decimal(value, fixture_id, source_line, field):
    if not isinstance(value, str) or not value.isdecimal() or (len(value) > 1 and value[0] == "0"):
        stop("E1_REFERENCE", fixture_id, source_line, field,
             "required canonical non-negative exact-integer text is missing or malformed")

def require_nonempty_evidence(value, fixture_id, source_line, field):
    if value is None or value is False:
        stop("E1_REFERENCE", fixture_id, source_line, field, "required evidence is absent")
    if isinstance(value, (dict, list, tuple, str)) and not value:
        stop("E1_REFERENCE", fixture_id, source_line, field, "required evidence is empty")
...
def validate_objective(value, p, fixture_id, source_line, field="objective"):
    ...
    if not isinstance(interior, list) or interior != p[1:-1]:
        stop("E1_REFERENCE", fixture_id, source_line,
             ...
             "interior vector is missing, malformed, or inconsistent with exact p")
    return objective
...
def validate_full_candidate_rows(witness, expected_count, K, fixture_id, source_line):
    ...
    for candidate_index, candidate in enumerate(candidates):
        ...
        if (not isinstance(candidate_p, list) or len(candidate_p) != K or
            ...
            stop("E1_REFERENCE", fixture_id, source_line,
                 f"candidate[{candidate_index}].p", "candidate p is missing or malformed")
        if prior_p is not None and candidate_p <= prior_p:
            stop("E1_REFERENCE", fixture_id, source_line,
                 f"candidate[{candidate_index}].p", "candidate witness order is not strictly ascending")
        validate_objective(candidate.get("objective"), candidate_p, fixture_id,
                           source_line, f"candidate[{candidate_index}].objective")
        prior_p = candidate_p

def validate_ties(witness, fixture_id, source_line):
    ...
    for tie, (required_through, required_count) in zip(ties, requirements):
        tie = require_mapping(tie, fixture_id, source_line, "tie_witness")
        if tie.get("through") != required_through:
            stop("E1_REFERENCE", fixture_id, source_line, "tie_witness.through",
                 f"required frozen tie prefix {required_through}")
        tied = tie.get("candidates")
        if not isinstance(tied, list) or len(tied) != required_count:
            stop("E1_REFERENCE", fixture_id, source_line, "tie_witness.candidates",
                 f"required {required_count} tied candidate witnesses")
...
def validate_positive(result, payload, fixture_id, source_line, required_method):
    ...
    if (not isinstance(p, list) or len(p) != payload["K"] or
        ...
        stop("E1_REFERENCE", fixture_id, source_line, "p", "exact p is missing or malformed")
    feasibility = require_mapping(result.get("feasibility"), fixture_id,
                                  source_line, "feasibility")
    for name in FEASIBILITY_FIELDS:
        if name not in feasibility or feasibility[name] is None:
            stop("E1_REFERENCE", fixture_id, source_line, f"feasibility.{name}",
                 "mandatory feasibility evidence is missing")
    if feasibility["length"] != payload["K"]:
        stop("E1_REFERENCE", fixture_id, source_line, "feasibility.length", "length mismatch")
    if feasibility["endpoints"] != [0, payload["T"] - 1]:
        stop("E1_REFERENCE", fixture_id, source_line, "feasibility.endpoints", "endpoint mismatch")
    if (type(feasibility["minimum_stride"]) is not int or
            type(feasibility["maximum_stride"]) is not int or
            type(feasibility["maximum_uniform_displacement"]) is not int):
        stop("E1_REFERENCE", fixture_id, source_line, "feasibility",
             "stride/displacement evidence is malformed")
    validate_objective(result.get("objective"), p, fixture_id, source_line)
    global_evidence = require_mapping(result.get("global_optimality"), fixture_id,
                                      source_line, "global_optimality")
    if global_evidence.get("method") != required_method:
        stop("E1_REFERENCE", fixture_id, source_line, "global_optimality.method",
             f"required {required_method}")
    require_nonempty_evidence(global_evidence.get("candidate_order"), fixture_id,
                              source_line, "global_optimality.candidate_order")
    if fixture_id in EXHAUSTIVE_IDS:
        ...
        summary = require_mapping(global_evidence.get("exhaustive_summary"), fixture_id,
                                  source_line, "global_optimality.exhaustive_summary")
        if (type(summary.get("candidate_count")) is not int or
            ...
            stop("E1_REFERENCE", fixture_id, source_line,
                 ...
                 "complete candidate/minimizer counts are missing or malformed")
        require_nonempty_evidence(summary.get("winner_key"), fixture_id, source_line,
                                  "global_optimality.exhaustive_summary.winner_key")
        if fixture_id in FULL_WITNESS_COUNTS:
            witness = require_mapping(global_evidence.get("exhaustive_witness"), fixture_id,
                                      source_line, "global_optimality.exhaustive_witness")
            validate_full_candidate_rows(witness, FULL_WITNESS_COUNTS[fixture_id],
                                         payload["K"], fixture_id, source_line)
            validate_ties(witness, fixture_id, source_line)
    elif fixture_id in FULL_SCALE_IDS:
        root = require_mapping(global_evidence.get("root_optimum"), fixture_id,
                               source_line, "global_optimality.root_optimum")
        if root.get("p") != p or root.get("objective") != result.get("objective"):
            stop("E1_REFERENCE", fixture_id, source_line,
                 "global_optimality.root_optimum",
                 "root optimum is missing, malformed, or inconsistent with the result")
        require_nonempty_evidence(root.get("certificate"), fixture_id, source_line,
                                  "global_optimality.root_optimum.certificate")
    else:
        stop("E1_REFERENCE", fixture_id, source_line, "reference_method_allocation",
...
parent_failure = None
if (shared_parent != Path(EXPECTED_PARENT) or production_root.parent != shared_parent or
...
try:
    if sys.executable != EXPECTED_INTERPRETER:
        stop("E1_PRE_RUN", None, None, "interpreter", "literal interpreter mismatch")
    if str(Path.cwd()) != EXPECTED_CWD:
        stop("E1_PRE_RUN", None, None, "working_directory", "literal working-directory mismatch")
    ...
    for name, required_value in REQUIRED_ENV.items():
        if os.environ.get(name) != required_value:
            stop("E1_PRE_RUN", None, None, "environment", f"{name} mismatch")
    ...
    for path in (reference_path, fixture_path, index_path, mutation_path):
        if not path.is_file() or path.stat().st_mode & 0o222:
            stop("E1_PRE_RUN", None, None, "sealed_input", f"sealed input unavailable: {path}")
    if Q != 1048576:
    ...
    for source_line, (raw_line, descriptor) in enumerate(zip(raw_lines, ordered), start=1):
        ...
        if descriptor.get("line") != source_line or payload.get("Q") != Q:
            stop("E1_INPUT", fixture_id, source_line, "line_or_Q", "source-line or Q mismatch")
        ...
        if not isinstance(result, dict):
            stop("E1_REFERENCE", fixture_id, source_line, "result", "reference result is not an object")
        ...
        if category == "positive":
            ...
            validate_positive(result, payload, fixture_id, source_line, required_method)
        else:
    ...
    if (tuple(record["fixture_id"] for record in records) != POSITIVE_IDS + NEGATIVE_IDS or
            tuple(record["mutation_id"] for record in mutation_records) != MUTATION_IDS):
...
The token `FUTURE_EXACT_E1_EXECUTION_QUEUE_REQUIRED` is a literal unresolved
authority binding and therefore a hard admission failure under plan v002. A
future versioned manifest must replace only that constant with its exact durable
E1 execution queue ID. Until then this candidate argv must not issue.

## 4. Correction P0-E1-MANIFEST-001: complete evidence admission

`E1_REFERENCE_COMPLETE` is possible only after the inline validator has accepted
all 27 ordered fixture records and all six ordered mutation records. It performs
schema/evidence validation only; it never solves, recomputes an objective,
creates a witness, changes a fixture, or consults another implementation.

For every positive it requires exact `p`; complete feasibility fields; exact
`E2`, `E_infinity`, `E1`, `U1`, and interior position vector; candidate-order
evidence; and complete method-specific global-optimality evidence. Every
applicable `T<=385` positive requires an affirmative exhaustive-completion
record, candidate/minimizer counts, and winner key. All five `T=17` positives
require exactly 15 strictly ordered complete candidate records and their frozen
tie witnesses; `G385-X` requires exactly 383 and its frozen scalar-tie witness.
Every positive `T=767/768` requires a non-empty independently generated root
optimum consistent with the returned `p` and objective. Every negative must
contain explicit null `p`, `feasibility`, `objective`, and `global_optimality`.

The first missing, null, malformed, incomplete, out-of-order, or inconsistent
obligation becomes the sole `E1_REFERENCE_BLOCKED` first failure. No second
solver, recomputation, tolerance, fallback, fixture addition, repair, or rerun is
permitted.

## 5. Correction P0-E1-MANIFEST-002: exclusive child publication

Before E1, Coordinator must admit the literal shared parent and a read-only
PRE_RUN must establish that it already exists as the exact project-local
directory. The inline envelope repeats that condition before any write. A
missing, different, unadmitted, non-directory, or unusable parent exits
`E1_PRE_RUN_BLOCKED` before creating a path or receipt.

Only after the parent gate passes may publication call exactly
`partial_root.mkdir(mode=0o700, parents=False, exist_ok=False)`. This exclusively
creates the absent temporary sibling without creating any ancestor. After the
single receipt is written and sealed, the sibling is atomically renamed to the
absent reference output. No cleanup, overwrite, recursive creation, reuse,
append, repair, alternate root, or retry is allowed.

## 6. Phase-relative PRE_RUN and role boundary

Immediately before a future E1 command, a separately authorized read-only check
must establish simultaneously:

- the literal shared parent exists and is admitted for this exact identity;
- `REFERENCE_OUTPUT_ROOT` and its temporary sibling are absent;
...
Evaluator reference batch only. It may not traverse or read the Builder checkout,
production adapter/output, any adapter, validator, detector, model, decoder,
dataset, checkpoint, metric, Torch/CUDA, training stack, Slurm, browser, or
network program. A failure or incomplete receipt blocks B1, E2, C1, P0, and P1.

## 7. Expected receipt and first-fail contract

The only expected receipt path is the literal path in section 1. The canonical
JSON object retains schema `duca-p0-e1-reference-receipt-v001` and includes its
manifest version, exact authority/queue/phase, interpreter, cwd, full argv,
...
The first failing precondition, input byte/line, result-schema field, exhaustive
witness, tie record, root optimum, typed status, mutation code, candidate order,
write, seal, or promotion condition stops the batch. A caught runtime contract
failure emits only the completed prefix plus one first-failure record and cannot
make B1 eligible. Shared-parent failure occurs before any write. Publication
failure exits 3 and does not promote the temporary sibling.

## 8. Remaining blockers and no-execution receipt

This v002 candidate remains blocked because it is neither Coordinator
`PRE_RUN_READY` nor a phase-specific execution queue; its execution-queue ID is
still the deliberate sentinel; no immediate pre-command read-only admission,
binding, root, duplicate-run, credential, service, interpreter, or resource check
occurred; and no execution environment has been admitted.

- Corrections: `P0-E1-MANIFEST-001_APPLIED`, `P0-E1-MANIFEST-002_APPLIED`
...
`EVALUATOR_DECISION: E1_ADMISSION_MANIFEST_V002_CORRECTED_NOT_EXECUTED_NOT_PRE_RUN_READY`.
