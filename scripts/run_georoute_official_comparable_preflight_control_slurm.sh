...
fail() {
  printf '[GEOROUTE_OFFICIAL_PREFLIGHT_CONTROL][FAIL] %s\n' "$*" >&2
  exit 2
}
...
[[ -n "${SLURM_JOB_ID:-}" ]] || fail 'preflight finalizer requires Slurm'
...
python -m tools.bata.finalize_georoute_official_comparable_preflight \
  --run-root "${RUN_ROOT}" \
