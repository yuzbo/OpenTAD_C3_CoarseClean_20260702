...
EXPECTED_COST_EXECUTION_COMMIT="${GEOROUTE_COST_EXECUTION_COMMIT:-${EXPECTED_EXECUTION_COMMIT}}"
DEPLOYMENT="${GEOROUTE_DYNAMIC_FLOOR_M2_DEPLOYMENT:-${RUN_ROOT}/control/deployment.json}"
...
PYTHONNOUSERSITE=1 PYTHONDONTWRITEBYTECODE=1 \
  python tools/bata/finalize_georoute_dynamic_floor_m2.py \
    ...
    --expected-cost-execution-commit "${EXPECTED_COST_EXECUTION_COMMIT}"
