...
if [[ -n "${C3_DETECTOR_AWARE_RESPONSIBILITY_POINTS_JSONL}" ]]; then
  [[ -f "${C3_DETECTOR_AWARE_RESPONSIBILITY_POINTS_JSONL}" ]] || fail "responsibility points JSONL missing: ${C3_DETECTOR_AWARE_RESPONSIBILITY_POINTS_JSONL}"
  [[ -n "${C3_DETECTOR_AWARE_RESPONSIBILITY_MANIFEST_JSON}" ]] || fail "set C3_DETECTOR_AWARE_RESPONSIBILITY_MANIFEST_JSON with responsibility points"
  [[ -f "${C3_DETECTOR_AWARE_RESPONSIBILITY_MANIFEST_JSON}" ]] || fail "responsibility manifest missing: ${C3_DETECTOR_AWARE_RESPONSIBILITY_MANIFEST_JSON}"
else
...
export C3_DETECTOR_AWARE_TEACHER_CHECKPOINT_PATH C3_DETECTOR_AWARE_TEACHER_CONFIG_PATH C3_DETECTOR_AWARE_RESPONSIBILITY_POINTS_JSONL C3_DETECTOR_AWARE_RESPONSIBILITY_MANIFEST_JSON
...
C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_EXPORT_SUMMARY_JSON="${C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_EXPORT_SUMMARY_JSON:-${POLICY_DIR}/responsibility_utility_export.summary.json}"
C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_OUTPUT_JSONL="${C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_OUTPUT_JSONL:-${POLICY_DIR}/samples_with_responsibility_utility.jsonl}"
...
export C3_DETECTOR_AWARE_TEACHER_UTILITY_EXPORT_SUMMARY_JSON C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_EXPORT_SUMMARY_JSON C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_OUTPUT_JSONL DETECTOR_AWARE_POLICY_CHECKPOINT DUCA_STAGE2_PRECHECK_SUMMARY_JSON
...
stage2_teacher_args=()
if [[ -f "${C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_EXPORT_SUMMARY_JSON}" ]]; then
  stage2_teacher_args=(
    --stage2-responsibility-summary-json "${C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_EXPORT_SUMMARY_JSON}"
    --stage2-responsibility-output-jsonl "${C3_DETECTOR_AWARE_RESPONSIBILITY_UTILITY_OUTPUT_JSONL}"
  )
else
  stage2_teacher_args=(
