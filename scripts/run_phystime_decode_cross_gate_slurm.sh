...
[[ "${SLURM_JOB_NAME:-}" == "pt_dc_gate" ]] \
  || fail "gate Slurm job name mismatch"
[[ "$(scontrol show job -o "${SLURM_JOB_ID}")" == *"Comment=${EXPECTED_COMMENT}"* ]] \
  || fail "gate Slurm comment mismatch"
...
"${PYTHON}" -m pytest \
  ...
  tests/test_phystime_decode_cross_deployment.py \
  tests/test_phystime_fullprecision_nms_replay.py \
  tests/test_phystime_p0_fullprecision_deployment.py \
  tests/test_phystime_g1_matched_full60_deployment.py \
  -q 2>&1 | tee "${TEST_LOG}"
