...
if [ -z "${BASH_VERSION:-}" ]; then
  printf '[ZOOMTOKEN_ORDERED_REUSE][FAIL] invoke this packet with bash, never sh\n' >&2
  exit 2
fi
set -euo pipefail

fail() {
  printf '[ZOOMTOKEN_ORDERED_REUSE][FAIL] %s\n' "$*" >&2
  exit 2
}

BASE="/data/run01/sczc063/yuzibo"
EXPECTED_COMMIT="${ZOOMTOKEN_ORDERED_REUSE_EXPECTED_COMMIT:?set the reviewed clean commit}"
ROOT="${ZOOMTOKEN_ORDERED_REUSE_SOURCE_ROOT:?set the reviewed clean checkout}"
RESULT_ROOT="${ZOOMTOKEN_ORDERED_REUSE_RESULT_ROOT:?set the unique formal result root}"
...
[[ -n "${SLURM_JOB_ID:-}" ]] || fail 'ordered-video replay requires a Slurm allocation'
[[ "${SLURM_JOB_NAME:-}" == *r1reuse* ]] || \
  fail 'Slurm JobName must use the frozen r1reuse namespace'
IFS=',' read -r -a visible_gpus <<< "${CUDA_VISIBLE_DEVICES:-}"
[[ -n "${CUDA_VISIBLE_DEVICES:-}" && "${#visible_gpus[@]}" -eq 1 ]] || \
  fail 'ordered-video replay requires exactly one Slurm-visible GPU'
[[ "${SLURM_CPUS_PER_TASK:-}" == "6" ]] || \
  fail 'ordered-video replay requires --cpus-per-task=6'
...
[[ "$(basename "${RESULT_ROOT}")" == *ordered_video_reuse* ]] || \
  fail 'result root must use the ordered_video_reuse namespace'
...
if [[ "${PRECHECK_ONLY}" == "1" ]]; then
  exec python tools/bata/profile_zoomtoken_ordered_video_reuse_r1_k100_cost.py \
    precheck "${common_args[@]}"
fi
...
exec torchrun --nnodes=1 --nproc_per_node=1 \
  ...
  --rdzv_id="zoomtoken-ordered-video-reuse-${SLURM_JOB_ID}" \
