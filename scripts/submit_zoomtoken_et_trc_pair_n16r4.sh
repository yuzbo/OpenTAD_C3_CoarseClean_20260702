...
for mode in off on; do
  ...
  [[ -f "$cfg" ]] || { echo "missing config: $cfg" >&2; exit 1; }
    ...
    --output="${BASE}/slurm_logs/%x_%j.out" --error="${BASE}/slurm_logs/%x_%j.err" \
      --wrap="source /etc/profile; module load cuda/11.8; module load miniforge3/24.11; source '${BASE}/conda_envs/opentad/bin/activate'; export ETTRC_PRETRAIN='${ETTRC_PRETRAIN}'; cd '${PROJECT_DIR}'; python tools/train.py '${cfg}' --seed '${SEED}' --cfg-options model.backbone.backbone.stride_k=${STRIDE_K}"
done
