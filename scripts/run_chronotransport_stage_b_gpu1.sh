...
STEPS="${CHRONOTRANSPORT_STAGE_B_STEPS:-1}"
PRECHECK_ONLY="${PRECHECK_ONLY:-1}"
OUT_ROOT="${CHRONOTRANSPORT_OUT_ROOT:-${BASE}/chronotransport_runs/stage_b}"
OUTPUT="${CHRONOTRANSPORT_STAGE_B_OUTPUT:-${OUT_ROOT}/stage_b_step${STEPS}.pth}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-1}"
export CUDA_VISIBLE_DEVICES
export YUZIBO_ROOT="${BASE}"
export CHRONOTRANSPORT_CONFIG="${CONFIG}"
export CHRONOTRANSPORT_CHECKPOINT="${CHECKPOINT}"
export CHRONOTRANSPORT_REPLAY_SPLIT="train"

fail() {
  printf 'ChronoTransport Stage-B launcher: %s\n' "$*" >&2
  exit 2
}
...
for path in "${OUT_ROOT}" "${OUTPUT}"; do
  case "${path}" in
    /data/run01/sczc063/yuzibo|/data/run01/sczc063/yuzibo/*) ;;
    *) fail "Stage-B outputs must stay under /data/run01/sczc063/yuzibo" ;;
  esac
done
...
[[ "${STEPS}" =~ ^[1-9][0-9]*$ ]] || fail "CHRONOTRANSPORT_STAGE_B_STEPS must be positive"
...
python tools/bata/validate_chronotransport_adatad.py \
  --config "${CONFIG}" \
  --output "${OUT_ROOT}/validator_stage_b.json"
python -m pytest -p no:cacheprovider \
  tests/test_chronotransport_core.py \
  tests/test_chronotransport_pipeline.py \
  tests/test_chronotransport_opentad_replay.py -q

if [[ "${PRECHECK_ONLY}" == "1" ]]; then
  printf 'ChronoTransport Stage-B PRECHECK_ONLY=1 PASS; no training step was launched.\n'
  exit 0
fi
...
python tools/bata/train_chronotransport_stage_b.py \
  --factory tools.bata.chronotransport_opentad_factory:stage_b_factory \
  --output "${OUTPUT}" \
  --steps "${STEPS}"
