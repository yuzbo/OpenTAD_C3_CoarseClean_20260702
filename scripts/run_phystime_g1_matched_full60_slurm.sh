...
fail() {
  echo "[PhysTime G1 matched full60] ERROR: $*" >&2
  exit 1
}
...
VARIANT="${PHYSTIME_FULL_VARIANT:?PHYSTIME_FULL_VARIANT is required}"
CONFIG="${PHYSTIME_FULL_CONFIG:?PHYSTIME_FULL_CONFIG is required}"
RUN_DIR="${PHYSTIME_FULL_RUN_DIR:?PHYSTIME_FULL_RUN_DIR is required}"
...
[[ -n "${SLURM_JOB_ID:-}" ]] || fail "full60 training must run inside Slurm"
[[ -n "${CUDA_VISIBLE_DEVICES:-}" ]] || fail "Slurm did not assign a visible GPU"
[[ "${EPOCHS}" == "60" ]] || fail "matched full60 contract requires exactly 60 epochs"
[[ "${SEED}" == "42" ]] || fail "matched full60 contract requires seed 42"
[[ -f "${CONFIG}" && -f "${G1A_GATE}" && -f "${CHECKPOINT}" ]] \
...
case "${VARIANT}|$(basename "${CONFIG}")" in
  ...
  *) fail "variant/config pair is not part of the fixed full60 suite" ;;
esac
...
"${PYTHON}" - \
  "${VARIANT}" "${CONFIG}" "${CHECKPOINT}" "${G1A_GATE}" "${COMMIT}" "${TREE}" <<'PY'
import hashlib
import json
import sys
...
variant, config_path, checkpoint, gate_path, commit, tree = sys.argv[1:]
...
if gate.get("git_commit") != commit or gate.get("git_tree") != tree:
    raise SystemExit("gate does not match full60 runtime snapshot")
cfg = Config.fromfile(config_path, lazy_import=False)
config_sha = _canonical_sha256(cfg.to_dict())
observed = gate.get("variants", {}).get(variant, {}).get("canonical_config_sha256")
if observed != config_sha:
    raise SystemExit("G1a gate config differs from full60 config")
_, dataset_sha = build_dataset_manifest(cfg, gate["evaluation_ground_truth_filename"])
if dataset_sha != gate.get("dataset_manifest_sha256"):
    raise SystemExit("full60 dataset differs from G1a gate dataset")
if hashlib.sha256(Path(checkpoint).read_bytes()).hexdigest() != gate.get("checkpoint_sha256"):
    raise SystemExit("full60 checkpoint differs from G1a gate checkpoint")
PY

mkdir -p "${RUN_DIR}"
"${PYTHON}" - \
  "${VARIANT}" "${CONFIG}" "${CHECKPOINT}" "${G1A_GATE}" "${RUN_DIR}" \
  "${COMMIT}" "${TREE}" "${EPOCHS}" "${SEED}" <<'PY'
import hashlib
import json
import sys
import time
...
if "Interpolate" in post_types:
    raise SystemExit("matched full60 suite forbids feature interpolation")
gate = json.loads(Path(gate_path).read_text(encoding="utf-8"))
_, dataset_sha = build_dataset_manifest(cfg, gate["evaluation_ground_truth_filename"])
base_config_sha = _canonical_sha256(cfg.to_dict())
effective_overrides = {
    "model.backbone.custom.pretrain": str(Path(checkpoint).resolve()),
    "work_dir": str(Path(run_dir, "work_dir").resolve()),
    "scheduler.max_epoch": int(epochs),
    "workflow.end_epoch": int(epochs),
    "workflow.val_start_epoch": 40,
    "workflow.val_eval_interval": 2,
    "workflow.checkpoint_interval": int(epochs),
    "workflow.checkpoint_save_mode": "lightweight",
    "workflow.checkpoint_include_ema": True,
    "post_processing.save_dict": True,
}
cfg.merge_from_dict(effective_overrides)
manifest = {
    "schema_version": "phystime_g1_matched_full60_manifest_v1",
    ...
    "effective_config_sha256": _canonical_sha256(cfg.to_dict()),
    ...
    "g1a_gate": str(Path(gate_path).resolve()),
    "g1a_gate_sha256": hashlib.sha256(Path(gate_path).read_bytes()).hexdigest(),
...
STATUS="${PIPESTATUS[0]}"
set -e
[[ "${STATUS}" == "0" ]] || fail "full60 training failed with exit code ${STATUS}"
touch "${RUN_DIR}/FULL_TRAINING_COMPLETE"
"${PYTHON}" tools/bata/validate_phystime_g1_matched_full60_artifacts.py \
  --run-dir "${RUN_DIR}" \
  --output "${RUN_DIR}/FULL_COMPLETE.json"
"${PYTHON}" - "${RUN_DIR}" <<'PY'
import json
import sys
from pathlib import Path
...
completion = json.loads((run_dir / "FULL_COMPLETE.json").read_text(encoding="utf-8"))
if completion.get("validation_pass") is not True:
    raise SystemExit("full60 artifact validation did not pass")
...
PY
echo "[PhysTime G1 matched full60] complete variant=${VARIANT} run_dir=${RUN_DIR}"
