...
fail() {
  echo "[DUCA_RIME_PHASE2_BASELINE][FAIL] $*" >&2
  exit 1
}
...
for name in \
  ...
  DUCA_RIME_PHASE2_BASELINE_CHECKPOINT \
  DUCA_RIME_PHASE2_BASELINE_CHECKPOINT_SHA256 \
...
[[ -n "${SLURM_JOB_ID:-}" ]] || fail "Phase-2 baseline evaluation must run in Slurm"
[[ "${DUCA_RIME_EXPECTED_COMMIT}" =~ ^[0-9a-f]{40}$ ]] \
  || fail "an exact expected Git commit is required"
[[ -d "${DUCA_RIME_REPO_ROOT}/.git" ]] \
  || fail "Phase-2 baseline evaluation requires a complete Git worktree"
[[ ! -e "${DUCA_RIME_PHASE2_BASELINE_ROOT}" ]] \
  || fail "a fresh Phase-2 baseline root is required"
...
if role not in manifest["train_roles"]:
    raise SystemExit("Phase-2 baseline split role is not registered")
if budget not in {128, 192, 256, 384, 512} or budget % 16:
    raise SystemExit("Phase-2 baseline budget is outside the registered panel")
print(manifest["train_roles"][role]["block_list_path"])
print(manifest["train_roles"][role]["block_list_sha256"])
PY
)
[[ "${#split_values[@]}" == 2 ]] || fail "failed to resolve the Phase-2 split role"
check_sha256 "${split_values[0]}" "${split_values[1]}" "Phase-2 role block list"
export DUCA_RIME_PHASE2_EVAL_BLOCK_LIST="${split_values[0]}"
export DUCA_EXPECTED_COMMIT="${DUCA_RIME_EXPECTED_COMMIT}"

if [[ "${PRECHECK_ONLY:-0}" == 1 ]]; then
  python - "${DUCA_RIME_PHASE2_BASELINE_CONFIG}" <<'PY'
import sys
...
assert cfg.duca_rime_baseline_contract.phase == 2
assert cfg.duca_rime_baseline_contract.padded_to_kmax is False
PY
  echo "[DUCA_RIME_PHASE2_BASELINE] PRECHECK PASS"
  exit 0
fi
