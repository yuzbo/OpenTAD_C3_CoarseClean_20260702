...
submit() {
  ...
  sbatch --parsable --partition=gpu --gres=gpu:1 --cpus-per-task=4 --time=7-00:00:00 \
    ...
    "${dep_arg[@]}" --wrap="source /etc/profile; module load cuda/11.8; module load miniforge3/24.11; cd '${PROJECT_DIR}'; PROJECT_DIR='${PROJECT_DIR}' SEED='${SEED}' bash scripts/run_duca_ct_dp_revised_thumos_gpu.sh '${cfg}'"
}
