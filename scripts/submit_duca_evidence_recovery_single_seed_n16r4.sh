...
EXPECTED_COMMIT="${DUCA_EXPECTED_COMMIT:?DUCA_EXPECTED_COMMIT must be the full 40-character formal commit SHA}"
[[ "${EXPECTED_COMMIT}" =~ ^[0-9a-fA-F]{40}$ ]] || {
  echo "DUCA_EXPECTED_COMMIT must be a full 40-character SHA" >&2; exit 2;
}
[[ "$(git rev-parse HEAD)" == "${EXPECTED_COMMIT}" ]] || {
  echo "Evidence checkout HEAD mismatch: expected ${EXPECTED_COMMIT}, got $(git rev-parse HEAD)" >&2; exit 2;
}
...
PRIMARY_SEED="${1:-8261}"
[[ "${PRIMARY_SEED}" == "8261" ]] || {
  echo "Evidence formal matrix is frozen to primary seed 8261" >&2; exit 2;
}
...
COST_JOB_OUT=$(submit_job "Cost profiling" \
  ...
  --export=ALL,DUCA_EXPECTED_COMMIT="${EXPECTED_COMMIT}",DUCA_RUN_ROOT="${RUN_ROOT}",DUCA_SEEDS="${PRIMARY_SEED}",DUCA_REPO_ROOT="${REPO_ROOT}",YUZIBO_ROOT="${YUZIBO_ROOT}",DUCA_VIDEOMAE_PRETRAIN="${DUCA_VIDEOMAE_PRETRAIN}",DUCA_H65_LEDGER_ROOT="${DUCA_H65_LEDGER_ROOT}",DUCA_H65_TRAIN_LEDGER_PATH="${DUCA_H65_TRAIN_LEDGER_PATH}",DUCA_H65_VAL_LEDGER_PATH="${DUCA_H65_VAL_LEDGER_PATH}",DUCA_H65_TEST_LEDGER_PATH="${DUCA_H65_TEST_LEDGER_PATH}" \
...
if STATS_JOB_OUT="$(sbatch --parsable \
  ...
  --output="${RUN_ROOT}/slurm_logs/%x_%j.out" \
  --error="${RUN_ROOT}/slurm_logs/%x_%j.err" \
  --export=ALL,DUCA_EXPECTED_COMMIT="${EXPECTED_COMMIT}",DUCA_RUN_ROOT="${RUN_ROOT}",DUCA_SEEDS="${PRIMARY_SEED}",DUCA_REPO_ROOT="${REPO_ROOT}",YUZIBO_ROOT="${YUZIBO_ROOT}",DUCA_VIDEOMAE_PRETRAIN="${DUCA_VIDEOMAE_PRETRAIN}",DUCA_H65_LEDGER_ROOT="${DUCA_H65_LEDGER_ROOT}",DUCA_H65_TRAIN_LEDGER_PATH="${DUCA_H65_TRAIN_LEDGER_PATH}",DUCA_H65_VAL_LEDGER_PATH="${DUCA_H65_VAL_LEDGER_PATH}",DUCA_H65_TEST_LEDGER_PATH="${DUCA_H65_TEST_LEDGER_PATH}",DUCA_H65_LEDGER_SOURCE="${DUCA_H65_LEDGER_SOURCE}",DUCA_H65_LEDGER_CONFIG_HASH="${DUCA_H65_LEDGER_CONFIG_HASH}" \
