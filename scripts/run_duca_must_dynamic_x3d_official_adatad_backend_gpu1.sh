...
fail() {
  echo "[DUCA_MUST_DYNAMIC_X3D_BACKEND][FAIL] $*" >&2
  exit 1
}
...
export CONFIG="${CONFIG:-configs/adatad/thumos/duca_must_dynamic_x3d_official_adatad_backend_full_train.py}"
export VALIDATOR="${VALIDATOR:-tools/bata/validate_duca_must_dynamic_x3d_official_adatad_backend.py}"
export RUN_TAG="${RUN_TAG:-duca_must_dynamic_x3d_official_adatad_backend_$(date +%Y%m%d_%H%M%S_%z)}"
