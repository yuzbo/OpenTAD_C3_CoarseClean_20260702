...
fail() {
  printf '[GEOROUTE_ESTIMATOR_PILOT][FAIL] %s\n' "$*" >&2
  exit 2
}
...
ARM="${GEOROUTE_PILOT_ARM:?set GEOROUTE_PILOT_ARM}"
EXPECTED_COMMIT="${GEOROUTE_EXPECTED_COMMIT:?set GEOROUTE_EXPECTED_COMMIT}"

[[ -n "${SLURM_JOB_ID:-}" ]] || fail 'pilot stage requires Slurm'
if [[ "${GEOROUTE_INNER_STEP:-0}" != "1" && "${SLURM_GPUS_ON_NODE:-1}" != "1" ]]; then
  ...
  exec srun --exact --ntasks=1 --gpus=1 --cpus-per-task=5 --mem=96000M \
    bash "${ROOT}/scripts/run_georoute_estimator_pilot_stage_slurm.sh"
fi
[[ -n "${CUDA_VISIBLE_DEVICES:-}" && "${CUDA_VISIBLE_DEVICES}" != *,* ]] || \
  fail 'pilot stage requires one Slurm-visible GPU and logical cuda:0'
...
python -m tools.bata.georoute_estimator_pilot_stage_runner \
  --arm "${ARM}" \
  --run-root "${RUN_ROOT}" \
