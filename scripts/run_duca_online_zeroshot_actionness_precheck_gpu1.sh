...
fail() {
  echo "[DUCA_ONLINE_ZEROSHOT_PRECHECK][FAIL] $*" >&2
  exit 1
}
...
CONFIG="${CONFIG:-configs/adatad/thumos/duca_online_zeroshot_actionness_precheck.py}"
VALIDATOR="${VALIDATOR:-tools/bata/validate_duca_online_adatad_precheck.py}"
RUN_TAG="${RUN_TAG:-duca_online_zeroshot_actionness_precheck_gpu1_$(date +%Y%m%d_%H%M%S_%z)}"
RUN_ID="${RUN_ID:-0}"
SEED="${SEED:-0}"
MASTER_PORT="${MASTER_PORT:-30243}"
...
if [[ "${PRECHECK_ONLY}" == "1" ]]; then
  echo "[DUCA_ONLINE_ZEROSHOT_PRECHECK] PRECHECK_ONLY complete"
  exit 0
fi
...
WORK_DIR="${WORK_DIR:-exps/thumos/adatad/duca_online_zeroshot_actionness_precheck/${RUN_TAG}}"
