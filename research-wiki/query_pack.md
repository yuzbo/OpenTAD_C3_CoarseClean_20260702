...
## Current Active Route: GeoRoute-AdaTAD (2026-07-23)
...
- Status: `tested_p0_mechanical_only`. Commit
  [`4a9358d`](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702/tree/4a9358d1fba4bde9aa7693a94f7e4dfc95d31ecc)
  has passed dense, ROI score-function, and hybrid real-CUDA one-step gates.
  They establish interface, exact-K, gradient connectivity and one-heavy-
  backbone accounting only. They establish no mAP, latency, energy, official
  test, A-MoD result, or paper claim.
...
- Deployment is currently quota-held, not model-held. P0 replacement Gate
  `1181172` passed the real uint8 180x320 path. P1 roots `1181007` and
  `1181177` are immutable scheduler diagnostics, not evidence: the first hit
  `AssocMaxSubmitJobLimit`; the second admitted leaves but rejected its
  selector as the 17th job. The dispatcher now preflights
  `active + leaves + selector`; submit only when the account has at most seven
  active jobs. P2/P3 remain absent and result-gated.
