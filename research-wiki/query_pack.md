...
当前事实：PhysTime-TAD 2.0 的 feature-geometry 核心代码已实现。PhysTime-AdaTAD 1.0 已完成 raw-video transform、三份 matched K384 配置、same-index validator、one-step 梯度证明、真实 gate 工具及 gate-dependent 三头启动器。真实 gate `1158528` 因非登录 shell 无 `module`、`1158546` 因 submission 覆盖 Slurm GPU mask、`1158556` 因 imgaug RNG 未统一，均在模型构建前失败且依赖训练未启动；三个具体问题均已修复并加入回归测试，仍需新 commit 重跑。不得声称已有真实 gate 或 mAP。
