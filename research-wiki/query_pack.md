...
G1a native-J192 matched control 已实现并通过远端新旧相关回归 `116 passed`：Q0=192、QΣ=378，不做 J192→Q0=384 lift；两臂共享官方 ActionFormer 模型，只改变统一秒轴张量。扩展回归修复了 physical center 污染 candidate mask，并加入逐层 padding isolation、真实 test evaluator、完整内容数据指纹、checkpoint 反序列化与 evaluator 独立重算。正式 dataset 消费的 411 个 THUMOS14 视频预审计显示最大相对 FPS 偏差约 1.12%、帧数偏差为 0；test 根目录额外 2 个未引用 MP4 被显式登记。首个 clean gate `1161304` 因旧范围把这两个文件误当 evaluator 样本而 fail-closed，pilot 未启动；修复后真实目录范围 precheck 与 `116 passed` 已完成，等待新快照重排。当前证据仍为 `tested`。原始 AdaTAD interpolation 只允许在后续 G1b 作为双臂共享中性 query lift，不能计作新增观测。
