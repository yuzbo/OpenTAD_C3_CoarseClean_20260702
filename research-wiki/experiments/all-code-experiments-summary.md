...
> 更新时间：2026-09-03 01:13（本地汇总快照）<br>
> 远程仓库：[OpenTAD_C3_CoarseClean_20260702](https://github.com/yuzbo/OpenTAD_C3_CoarseClean_20260702)<br>
> 当前工作分支：`codex/zoomtoken-ba-fdr-k16-fullmatrix-v001`；本地 HEAD：`6f6d81f912449b6739e2ad25ffbcdac722e7cd9e`
...
- 共享计算目录：`/data/run01/sczc063/yuzibo/projects/bafdr_k16_fullmatrix_efe69d2e_compute`
- watcher 日志：`/data/run01/sczc063/yuzibo/slurm_logs/bafdr_fullmatrix_submit_watch_efe69d2e.log`
...
## 2026-09-03 监督快照（N16R4）

- BA-FDR 修复后的远端 worktree 为 `/data/run01/sczc063/yuzibo/projects/bafdr_k16_fullmatrix_5dba75c7`，tag `efe69d2e`，run root `/data/run01/sczc063/yuzibo/projects/bafdr_k16_fullmatrix_efe69d2e_compute`；minute watcher PID `2626313`，日志 `/data/run01/sczc063/yuzibo/slurm_logs/bafdr_fullmatrix_submit_watch_efe69d2e.log`。预检 1266403 通过，但训练仍处于早期运行阶段，未产生正式指标。
- BA-FDR 的 1266401 因单 GPU 启动两进程失败，1266402 因第二个不支持的 `window_overlap_ratio` 参数失败；两者都是部署/接口证据，不能作为模型结果。旧 1266325–1266330 及其 descendants 已取消或失败，均排除。
- 其他远端状态：S2-v3 1264404、D2S 1264405 均 `FAILED`，历史参考产物不可恢复且没有最终 mAP/cost；PA-TAD 1263586 因 dirty candidate checkout 被正式门禁拒绝；CT-DP 1265777/1265778 仍 RUNNING、1265779/1265780 依赖排队；ET-TRC 1266218/1266219 已 COMPLETED，但日志明确是 `max_train_iters=100` 的 smoke 训练，没有正式 receipt/指标；DUCA supervisor 仍因 Unified 未实现而 `BLOCKED`，独立 DUCA recovery array 1266397–1266400 正在运行，尚无终态指标。
- 因此本轮可核验结论仍是 `NO_VALID_RESULTS`：当前没有任何新提交路线同时具备完整官方训练/评测人口、冻结协议和终态 receipt/metrics，可用于新的 mAP、延迟或能耗论文主张；运行态、smoke、预检和失败日志只作为工程/负证据保留。
