...
- 2026-09-01：面向高精度时序动作检测的连续时空自适应与双相 B-AMoD 计算体系（CT-DP-BAMoD）完成首轮系统实现。包含三大核心组件：
  ...
  3. `opentad/models/bricks/scale_adaptive_conv1d.py` 中的 `ContinuousTimeScaleAdaptiveConv1d`：根据局部采样物理步长 $\Delta t_i = t_{i+1} - t_i$ 动态自适应调制 1D 卷积核采样偏移，彻底消除非均匀选帧引起的 FPN 感受野漂移与物理几何速度畸变。
  4. 新增配置 `configs/adatad/thumos/duca_ct_dual_phase_bamod_thumos.py` 与全面测试套件 `tests/test_ct_dual_phase_bamod.py`。
