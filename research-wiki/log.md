...
- 2026-07-20：P0 正式 DAG 全部完成：gate `1174688`、selected online/EMA `1174689/1174690`、physical online/EMA `1174691/1174692`、suite `1174693` 均 `COMPLETED 0:0`，最终 suite `validation_pass=true`。EMA fullprecision selected/physical 为 `41.2830/57.6087%`，差值 `+16.3257`；舍入对四臂 Avg-mAP 影响仅 `-0.0366` 到 `+0.0338` 点，非法 proposal 与过滤效应均为 0。physical 主效应不由旧舍入解释；fullprecision 短动作 recall@0.7 相对 selected 提升 `+31.59` 点。P0 状态更新为 `tested`；下一任务先做无训练的冻结 decode cross-replay，通过后才做 Q192 UU/UP/PU/PP 严格匹配训练，无需再为 P0 发起 Pro 讨论。
- 2026-07-20：补记 P0 权重来源诊断：fullprecision physical-EMA 相对 physical-online 的 Avg-mAP 为 `+0.0531` 点，但 mAP@0.7 为 `-0.7190` 点。该差异不改变 physical 相对 selected 的主效应，但后续实验必须预先固定并同时报告 online/EMA，禁止逐指标挑选。
- 2026-07-20：实现无训练的冻结解码交叉回放：固定 full60 selected/physical epoch-59 online/EMA 四个 checkpoint 条件，每个条件只捕获一次原始分类/回归张量，再在同一张量上执行 uniform/physical 两种 CPU float32 解码。第一轮独立审计发现 native 分支曾用捕获的生产 proposal 覆盖重建结果，已删除该自证旁路并加入扰动反例测试。
- 2026-07-20：第二轮独立逻辑与部署审计完成。新增提交前全内容 preflight、P0 direct 结果锚定、P0 推理语义哈希等价、四条件共享观测/时间轴与 online/EMA state 门禁、唯一 Slurm DAG token、幂等提交接管、真实依赖/日志/sacct 快照、原子状态文件和扩展致命日志扫描。当前状态严格为 `implemented`，正式 mAP 为 `NA`；远端门禁失败则停止并发起 Pro 严审，门禁通过才提交正式四条件 replay。
- 2026-07-20：第三轮独立复审发现 `window_sequence_sha256` 含原生坐标模式，却被误纳入四条件共享观测契约，会使 selected/physical 真实 gate 必然误失败。已拆为共享 `observation_contract` 与轴特有 `axis_window_contract`：跨 arm 允许且要求轴特有窗口哈希不同，同一 arm online/EMA 必须一致；新增行为回归测试，状态仍为 `implemented`，正式 mAP 仍为 NA。
- 2026-07-20：第四轮独立部署复审发现临时 `flock` 不能永久阻止同 token 跨运行目录接管，以及 `sbatch` 响应丢失后作业延迟可见可能触发重复提交。新增可单测的原子全局/本地 owner 双 manifest，永久绑定 `token/run_root/commit/tree`；同 token 换目录在队列查询和失败回收前拒绝。进一步复审指出有限轮询后重投仍存在超预算延迟可见窗口，最终状态机改为 `sbatch` 前持久化提交意图，任何模糊结果未被唯一查询到时保留 ambiguous 状态并退出，后续恢复只查询不重投；回归覆盖预算内接管、超预算阻断和记账可见后的恢复。状态仍为 `implemented`，远端 gate 尚未提交。
- 2026-07-20：部署复审继续发现 resolved/fatal 文件若不参与后续决策，暂时不可见时仍可能重投。最终修复把 resolved Job ID 设为永久恢复约束，把 fatal 设为 token 级永久禁用；suite 强制核对 gate、四 replay、自身共六个 resolved marker 与 `jobs.tsv` 的 token/comment/Job ID，并拒绝任何 ambiguous/fatal 文件。新增 resolved 不可见零 `sbatch` 与 fatal 重启零 `sbatch` 行为测试；远端 Linux gate 通过前仍不部署正式 replay。
- 2026-07-20：冻结解码交叉回放最终独立部署复审裁决为 `DEPLOY`，P0/P1 为 0。该裁决仅解除 clean commit/snapshot 与真实 Linux/PyTorch gate 的部署禁令；正式状态仍为 `implemented`、mAP 为 NA，native/P0 精确等价未通过前不启动四路 replay。
- 2026-07-20：首次 decode-cross CPU preflight 在创建任何 Slurm job 前 fail-closed：直接执行 `tools/bata/preflight_phystime_decode_cross.py` 时仓库根未进入 `PYTHONPATH`，触发 `ModuleNotFoundError: tools`。失败 run root 为 `phystime_decode_cross_d7314aa_20260720_151100_0800_15d8d812f5c2470e90da59da6e0b3e3f`，无 jobs.tsv/Slurm ID，不构成实验。修复在 submit/gate/replay/suite 四脚本 `cd` 精确快照后统一导出该快照为 `PYTHONPATH`，并新增部署脚本绑定测试；需以新 commit/tree 和新 run root 重新部署。
- 2026-07-20：`e94dcd8` 远端式回归证明 direct preflight import 已通过；随后 5 个 Linux 提交状态机测试因夹具内硬编码系统 `python` 而失败，集群系统解释器不支持 f-string。生产 submitter 使用固定 OpenTAD Python，未受该夹具问题影响；此时仍未调用 submitter/创建 Slurm job。夹具改为显式继承运行 pytest 的 `sys.executable`，需以新 commit 重跑远端式回归。
