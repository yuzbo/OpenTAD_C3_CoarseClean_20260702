...
| 当前完整实验 | 新作业 | 开始时间（UTC+8）与资源 | 预检与结果状态 |
| --- | --- | --- | --- |
| 全局低分辨率加动态局部片段的双分辨率检测（D2S，9单元） | `1276621`，`RUNNING` | 2026-09-07 17:50:33，`g0067`，2 GPU / 8 CPU | PRECHECK `1276607` 完成 `0:0`，17项测试通过；无完整准确率结果 |
| 局部细节仅注入低层金字塔的检测（PA-TAD，9单元） | `1276622`，`RUNNING` | 2026-09-07 17:50:33，`g0067`，2 GPU / 8 CPU | PRECHECK `1276608` 完成 `0:0`，11项测试通过；无完整准确率结果 |

容量恢复时观测约 510 GiB 可用，提交前检查至少 160 GiB 余量。两项预检均通过两卡
物理跳过前向及三臂完整算子追踪；运行代码未变。当前实验目录：

- D2S：`/data/run01/sczc063/yuzibo/projects/d2s_tad_full200_compute_21aa2945_formal_r5`
- PA-TAD：`/data/run01/sczc063/yuzibo/projects/patad_full200_compute_21aa2945_formal_r4`

本日此前失败记录：

| 实验名称 | 作业与终态（UTC+8） | 失败位置与已完成记录 | 实验结果 |
| --- | --- | --- | --- |
| 全局低分辨率加动态局部片段的双分辨率检测（D2S，9单元） | `1274666`，`FAILED 120:0`，16:55:31 | D160/4408 写训练日志时磁盘配额耗尽；仅 D160/4407 有训练终态记录 | 候选臂未开始，无完整准确率结果；失败目录保留 |
| 局部细节仅注入低层金字塔的检测（PA-TAD，9单元） | `1274667`，`FAILED 1:0`，16:49:12 | 创建 D160/4409 目录时磁盘配额耗尽；D160/4407、4408 有训练终态记录 | 候选臂未开始，无完整准确率结果；失败目录保留 |
...
观测可用 9.8 GiB；当时未重提。两套新矩阵仅 checkpoint 预计约需 143 GiB。
后续容量恢复和新预检、部署状态见本节首表；研究结论见 `query_pack.md`。
...
| :---: | :--- | :--- | :--- | :--- | :---: | :---: | :--- |
| **1** | **Phase 1: S2-v3 物理多视图基线矩阵** | `ZOOMTOKEN-CONTINUOUS-ROI-S2-V3-FULL200-COMPUTE-PARETO-3X3-v001` | • `D160` (4407, 4408, 4409)<br>• `G96` (4407, 4408, 4409)<br>• `U128-A0` (4407, 4408, 4409)<br>*(共 9 个 Cells)* | 全局 96 + 局部中心 128 原生源裁剪；单 VideoMAE 共享；零参数固定均值融合。<br>`ρ_C`: **UNVERIFIED** (待算子级对账) | **`1263644`** | 🟢 **RUNNING**<br>(Node `g0052`) | `/data/run01/sczc063/yuzibo/projects/continuous_roi_s2_v3_full200_compute`<br>`slurm_1263644.log` |
| **2** | **Phase 2: D2S-TAD 动态双速变焦矩阵** | `ZOOMTOKEN-D2S-TAD-FULL200-COMPUTE-PARETO-3X3-v001` | • `D160` (4407, 4408, 4409)<br>• `G96` (4407, 4408, 4409)<br>• `D2S-U128-B128` (4407, 4408, 4409)<br>*(共 9 个 Cells)* | 一阶时序语义变化率 $\Delta(t)$ 动态触发（仅由全局 Scout 特征计算，严格前因果）；无参掩码注入。<br>`ρ_C`: **UNVERIFIED** (待突发率实测) | **`1263645`** | 🟢 **RUNNING**<br>(Node `g0052`) | `/data/run01/sczc063/yuzibo/projects/d2s_tad_full200_compute`<br>`slurm_1263645.log` |
