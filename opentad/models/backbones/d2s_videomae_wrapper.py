...
class D2STemporalZoomBackboneWrapper(BackboneWrapper):
    ...
    4. Computes residual R and returns feature bundle for asymmetric pyramid projection.
    """
    ...
    def __init__(self, cfg):
        ...
        # Center crop canonical box for local_size
    ...
    def _encode_global_view(self, global_view: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.pre_processing_pipeline is not None:
            frames = self.pre_processing_pipeline(frames)
        ...
        if self.post_processing_pipeline is not None:
            features = self.post_processing_pipeline(features)

        B = global_view.shape[0]
        channels = features.shape[1]
        G_dense = features  # [B, C, 384]
        G_chunk = (
            features.reshape(B, channels, self.total_chunks, self.tubelets_per_chunk)
            .mean(dim=-1)
            .transpose(1, 2)
        )  # [B, 48, C]
    ...
    def _route_chunks(self, G_chunk: torch.Tensor) -> Tuple[torch.Tensor, Dict[str, torch.Tensor]]:
        ...
        sorted_indices = torch.argsort(repr_shift, dim=-1, descending=True)
    ...
    def forward(self, frames, masks=None):
        ...
        if global_view is None:
            raise ValueError("D2S requires global view tensor")
        ...
        if source_view is not None:
            ...
            G_sel_list = [G_4d[b, :, selected_indices[b], :] for b in range(B)]
            G_sel = torch.stack(G_sel_list, dim=0).flatten(start_dim=2)
            P_G_sel = self.proj_global(G_sel)

            R_sel = self.gamma * (P_L_sel - P_G_sel)

            # Scatter into dense grid
            R_dense = torch.zeros_like(G)
            R_dense_4d = R_dense.reshape(B, C, self.total_chunks, self.tubelets_per_chunk)
            R_sel_4d = R_sel.reshape(B, C, self.burst_chunks, self.tubelets_per_chunk)
            for b in range(B):
                R_dense_4d[b, :, selected_indices[b], :] = R_sel_4d[b]
            R = R_dense_4d.flatten(start_dim=2)
        else:
