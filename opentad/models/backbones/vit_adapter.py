...
class VisionTransformerAdapter(BaseModule):
    ...
    def forward(
        ...
        h //= self.patch_size
        w //= self.patch_size
        if (pjst_pair_scale is None) != (pjst_pair_valid is None):
            raise ValueError("PJST pair scale and validity metadata must be provided together")
        if pjst_pair_scale is not None:
            ...
            nonuniform = ((pjst_pair_scale != 1) & pjst_pair_valid).any(dim=1)
            if bool(nonuniform.any()):
                rows = nonuniform.nonzero(as_tuple=False).flatten()
                selected = pairs[rows]
                scales = pjst_pair_scale[rows]
                valid = pjst_pair_valid[rows, None, :, None, None]
                m = (selected[:, :, :, 0] + selected[:, :, :, 1]).float() * 0.5
                v = (selected[:, :, :, 1] - selected[:, :, :, 0]).float() * scales[:, None, :, None, None] * 0.5
                out = torch.stack((m - v, m + v), dim=3).to(dtype=x.dtype)
                updated = torch.where(valid, out, selected)
                pairs = pairs.clone()
                pairs[rows] = updated
            y = pairs.reshape_as(x)
            x = y
