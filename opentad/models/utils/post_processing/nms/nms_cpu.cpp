...
Tensor softnms_1d_cpu(Tensor segs, Tensor scores, Tensor dets, float iou_threshold,
                      float sigma, float min_score, int method, float t1, float t2) {
  if (segs.numel() == 0) {
    return at::empty({0}, segs.options().dtype(at::kLong));
  }
  ...
  for (int64_t i = 0; i < nsegs; i++) {
    ...
    while (pos < nsegs) {
      ...
      if (method == 0) {
        ...
        // improved gaussian, proposed in BMN, helpful for recall
        if (ovr >= (t1 + t2 * iarea)) weight = std::exp(-(ovr * ovr) / sigma);
      }
      sc[pos] *= weight;
...
Tensor softnms_1d(Tensor segs, Tensor scores, Tensor dets, float iou_threshold,
  ...
  return softnms_1d_cpu(segs, scores, dets, iou_threshold, sigma, min_score, method, t1, t2);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
  ...
  m.def(
    ...
    py::arg("sigma"), py::arg("min_score"), py::arg("method"), py::arg("t1"), py::arg("t2")
  );
}
