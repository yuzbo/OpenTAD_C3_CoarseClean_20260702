...
def apply_sliding_window_nms(result_dict, post_cfg, *, return_audit=False):
    ...
    if (
        _post_cfg_get(post_cfg, "sliding_window", False) is not True
        or _post_cfg_get(post_cfg, "nms", None) is None
    ):
        audit["nms_applied"] = False
        audit["aggregate"]["input_detections"] = sum(
            len(detections) for detections in result_dict.values()
        )
        audit["aggregate"]["kept_for_nms"] = audit["aggregate"][
            "input_detections"
        ]
        audit["aggregate"]["post_nms_detections"] = audit["aggregate"][
            "input_detections"
        ]
        if return_audit:
    ...
    for video_name, detections in result_dict.items():
        ...
        if policy["round_before_cross_window_nms"]:
            ...
            rounded_score_values = [
                round(value, policy["score_round_digits"])
        ...
        (
            effective_valid_detections,
            effective_validation,
        ) = _validate_video_detections(
            video_name,
            effective_detections,
            min_duration=policy["proposal_min_duration"],
        )
    ...
    merged = {}
    nms_cfg = _post_cfg_get(post_cfg, "nms", None)
...
def _build_proposal_audit(policy):
    ...
    return {
        ...
        "aggregate": {
            ...
            "effective_invalid_reason_counts": {
                key: 0 for key in invalid_keys
            },
        },
        "videos": {},
    }
...
def _combine_validation_audits(
    *,
    ...
    return {
        ...
        "effective_input_detections": effective_validation[
            "input_detections"
        ],
