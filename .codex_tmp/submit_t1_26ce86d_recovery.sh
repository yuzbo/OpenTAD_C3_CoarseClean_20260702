...
RUN_ROOT="${ROOT_BASE}/duca_t1_26ce86d_recovery_$(date +%Y%m%d_%H%M%S)"
...
for variant in "${variants[@]}"; do
  ...
  job="$(sbatch --parsable \
    ...
    "${STAGE}/recover_t1_26ce86d.sbatch")"
  printf '%s\t%s\n' "${variant}" "${job}" | tee -a "${RUN_ROOT}/jobs/jobs.tsv"
done

cat > "${RUN_ROOT}/deployment_manifest.txt" <<EOF
task=offline_tad_t1_p0_recovery
code_commit=26ce86d7810e8f7c0568dc045bb1db7240c66de2
code_snapshot=/data/run01/sczc063/yuzibo/projects/opentad_duca_t1_26ce86d_20260723
