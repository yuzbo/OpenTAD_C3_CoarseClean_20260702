...
& $scp @scpArgs '.\.codex_tmp\diagnose_sparse_mobile_failures.sh' "${hostName}:${stageDir}/"
if ($LASTEXITCODE -ne 0) { throw 'Failed to upload failure diagnostic.' }

& $ssh @sshArgs "bash '$stageDir/diagnose_sparse_mobile_failures.sh'"
if ($LASTEXITCODE -ne 0) { throw 'Remote failure diagnostic failed.' }
