...
VARIANT_SPECS = {
    "gas_vt_fixed_384": dict(
        ...
        ledger_name="gas_vt_fixed_384",
        source="learned_paction_gas_vt_policy_checkpoint",
        config_hash="c3_gas_vt_fixed384_no_uniform_v1",
    ),
    "gas_vt_fixed_768": dict(
        target_len=768,
        require_selected_count=768,
        strategy="gas_vt_fixed_768",
        ledger_name="gas_vt_fixed_768",
        source="learned_paction_gas_vt_policy_checkpoint",
        config_hash="c3_gas_vt_fixed768_no_uniform_v1",
    ),
    "gas_vt_dynamic": dict(
        target_len=768,
        require_selected_count=None,
        strategy="gas_vt_dynamic",
        ledger_name="gas_vt_dynamic",
        source="learned_paction_gas_vt_policy_checkpoint",
        config_hash="c3_gas_vt_dynamic_no_uniform_v1",
    ),
}


gas_vt_ledger_variant = os.environ.get("C3_GAS_VT_LEDGER_VARIANT", "gas_vt_fixed_384")
if gas_vt_ledger_variant not in VARIANT_SPECS:
    raise ValueError(f"unknown C3_GAS_VT_LEDGER_VARIANT={gas_vt_ledger_variant}")
_variant = VARIANT_SPECS[gas_vt_ledger_variant]
...
learned_ledger_root = os.environ.get(
    "C3_GAS_VT_LEDGER_ROOT",
    os.path.join(yuzibo_root, "projects/c3_lowres_action_probe/gas_vt_ledgers"),
)
...
c3_gas_vt_adatad_pretrain_path = os.environ.get(
    "C3_GAS_VT_ADATAD_PRETRAIN_PATH",
...
train_ledger_path = os.environ.get(
    "C3_GAS_VT_TRAIN_LEDGER_PATH",
    os.path.join(learned_ledger_root, "train", f"value_transport_ledger_{_ledger_name}.jsonl"),
)
val_ledger_path = os.environ.get(
    "C3_GAS_VT_VAL_LEDGER_PATH",
    os.path.join(learned_ledger_root, "val", f"value_transport_ledger_{_ledger_name}.jsonl"),
)
test_ledger_path = os.environ.get(
    "C3_GAS_VT_TEST_LEDGER_PATH",
    os.path.join(learned_ledger_root, "test", f"value_transport_ledger_{_ledger_name}.jsonl"),
)
c3_value_transport_source = os.environ.get("C3_GAS_VT_LEDGER_SOURCE", _variant["source"])
c3_value_transport_config_hash = os.environ.get("C3_GAS_VT_LEDGER_CONFIG_HASH", _variant["config_hash"])

window_size = int(_variant["target_len"])
dense_window_size = 768
scale_factor = 1
chunk_num = window_size * scale_factor // 16
gas_vt_ledger_strategy = _variant["strategy"]
gas_vt_require_selected_count = _variant["require_selected_count"]

experiment_scope = dict(
    route="C3_MAINLINE_OPTIMIZATION",
    route_variant="C3_GAS_VT_STRICT_LEDGER",
    stage="gas_vt_ledger_original_adatad_full_train",
    selector_source="learned_paction_gas_vt_policy_checkpoint",
    selection_strategy=gas_vt_ledger_strategy,
    gas_vt_ledger_variant=gas_vt_ledger_variant,
    ...
    oracle_shell_semantics="strict_learned_gas_vt_value_transport_ledger_not_random_online_oracle_shell",
...
def c3_gas_vt_loadframes(ledger_path):
    return dict(
        type="LoadFrames",
        num_clips=1,
        ...
        bata_value_transport_require_selected_count=gas_vt_require_selected_count,
...
dataset = dict(
    train=dict(
        ...
        pipeline=[
            ...
            c3_gas_vt_loadframes(train_ledger_path),
            ...
            dict(type="Collect", inputs="imgs", keys=["masks", "gt_segments", "gt_labels"], meta_keys=c3_gas_vt_meta_keys),
        ],
    ),
    val=dict(
        type="ThumosSlidingDataset",
        ...
        pipeline=[
            dict(type="PrepareVideoInfo", format="mp4"),
            dict(type="mmaction.DecordInit", num_threads=4),
            c3_gas_vt_loadframes(val_ledger_path),
            dict(type="mmaction.DecordDecode"),
    ...
    test=dict(
        ...
        pipeline=[
            dict(type="PrepareVideoInfo", format="mp4"),
            dict(type="mmaction.DecordInit", num_threads=4),
            c3_gas_vt_loadframes(test_ledger_path),
            dict(type="mmaction.DecordDecode"),
            dict(type="mmaction.Resize", scale=(-1, 160)),
            dict(type="mmaction.CenterCrop", crop_size=160),
            dict(type="mmaction.FormatShape", input_format="NCTHW"),
            dict(type="ConvertToTensor", keys=["imgs"]),
            dict(type="Collect", inputs="imgs", keys=["masks"], meta_keys=c3_gas_vt_meta_keys),
        ],
    ),
)
...
model = dict(
    backbone=dict(
        ...
        custom=dict(
            pretrain=c3_gas_vt_adatad_pretrain_path,
...
work_dir = f"exps/thumos/adatad/c3_gas_vt_ledger_original_adatad_full_train/{gas_vt_ledger_variant}"
