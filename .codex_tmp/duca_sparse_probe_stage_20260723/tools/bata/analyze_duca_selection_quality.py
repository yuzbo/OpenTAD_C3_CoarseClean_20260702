...
def _global_structured_positions(
    scores: Sequence[float],
    *,
    budget: int,
    max_unselected_hole: int,
) -> list[int]:
    logits = torch.tensor([float(score) for score in scores], dtype=torch.float32)
    effective_budget = min(max(0, int(budget)), int(logits.numel()))
    ...
    positions = [int(position) for position in decoded.selected_positions[0].tolist()]
    if positions != sorted(set(positions)) or len(positions) != effective_budget:
        raise RuntimeError("global structured DP violated ordered exact-K selection")
    if _max_unselected_hole(len(scores), positions) > int(max_unselected_hole):
        raise RuntimeError("global structured DP violated the max-unselected-hole contract")
    return positions
...
def analyze_record(record: Mapping[str, Any], *, random_seed: int = 0) -> dict[str, Any]:
    ...
    methods = {
        ...
        "pure_delta_same_feasible_dp": pure_delta_structured,
        # Compatibility alias for older analysis consumers. This is no longer
        # ordinary top-k; both keys are generated by the same global DP.
        "pure_delta_topk_diagnostic": pure_delta_structured,
