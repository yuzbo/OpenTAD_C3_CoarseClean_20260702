...
# Matched PJST-D1 control arm.  This arm reuses the identical Stage-1 learned
# H65 nonuniform selector acquisition (policy_alpha pinned to 1.0, never the
# exact-uniform fallback) and differs from the ON arm ONLY in
# ``model.backbone.custom.pjst_derivative_only`` and ``work_dir``.  All
...
model = dict(
    ...
    backbone=dict(custom=dict(pjst_derivative_only=False)),
)
