_base_ = ["./duca_local_residual_fixed384_official60_base.py"]


duca_transition_only_contract = dict(
    route="DUCA_LOCAL_RESIDUAL_R0_NO_FEEDBACK_FIXED384_OFFICIAL60",
    optimization_variant="bounded_residual_without_detector_feedback",
    detector_gradient_updates="none",
)


model = dict(
    frame_selector=dict(
        local_cell_residual_scale=0.25,
        detector_gradient_mode="none",
        inference_policy_alpha=1.0,
        loss_weight_schedule=dict(
            detector_gradient=dict(start=0.0, end=0.0),
        ),
    ),
)


work_dir = "exps/thumos/adatad/duca_local_residual_r0_no_feedback_fixed384_official60"
