...
selector = dict(type="PCOTMRASPreBackboneFrameSelector", selection_strategy="semantic_indirect", target_len=384, dense_window_size=768, dynamic_k_min=192, dynamic_k_max=384, dynamic_k_threshold=0.5, dynamic_k_step=32, boundary_radius=1, reader=dict(type="PCOTMRASCoarseActionnessFrameScout", in_dim=4, hidden_dim=32, temporal_layers=1, temporal_kernel_size=3))
...
arms = {
 ...
 "actionness_only_fixed_k_control": dict(mode="actionness_only_fixed_k_control", selector=dict(selector, dynamic_k_min=384, dynamic_k_max=384), **shared),
 "actionness_boundary_fixed_k": dict(mode="actionness_boundary_fixed_k", selector=dict(selector, dynamic_k_min=384, dynamic_k_max=384), **shared),
