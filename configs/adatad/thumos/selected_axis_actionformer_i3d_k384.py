...
append_timestamp_channels = bool(int(os.environ.get("PHYSTIME_APPEND_TIMESTAMP_CHANNELS", "0")))
input_channels = 2052 if append_timestamp_channels else 2048
worker_count = int(os.environ.get("PHYSTIME_NUM_WORKERS", "4"))
batch_size = int(os.environ.get("PHYSTIME_BATCH_SIZE", "2"))


def selected_pipeline(training, with_gt):
    ...
    pipeline.extend(
        [
            ...
            dict(
                type="BuildSelectedAxisFeatureBaseline",
                append_timestamp_channels=append_timestamp_channels,
            ),
...
dataset = dict(
    train=dict(
        ...
        pipeline=selected_pipeline(training=True, with_gt=True),
    ),
    val=dict(
        ann_file=annotation_path,
        class_map=class_map,
        data_path=feature_path,
        block_list=block_list,
        pipeline=selected_pipeline(training=False, with_gt=True),
    ),
    test=dict(
        ann_file=annotation_path,
        class_map=class_map,
        data_path=feature_path,
        block_list=block_list,
        pipeline=selected_pipeline(training=False, with_gt=False),
    ),
)
...
work_dir = os.environ.get("PHYSTIME_WORK_DIR", "exps/thumos/actionformer/selected_axis_i3d_k384")
