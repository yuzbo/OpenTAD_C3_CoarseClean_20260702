...
2026-09-09解释补充：下文“只改变allocation”仅指模型配置的acquisition_policy开关；实际同时切换采样评分来源和ST路径，不是单独四相配额效应。初版A Phase说明明确采用动作性logits/导数，本点与当前实现一致，不能只凭未融合center_scores判接线错误。见[43号记录](43_HEARTBEAT_EVIDENCE_20260909_0950.md)。五项训练和独立终态现均已完成；以下保留部署时历史状态，不改写原作业身份。

核查时间：2026-09-08 17:27:59 CST。本次是用户交互任务，不是补记心跳。

18:25 后续进展：五项均已返回第5轮全测试，ET-TRC又完成第10轮，真实最佳EMA已核验。
最新分数见[30_TEST_GUIDED_EVAL5_PROGRESS_20260908_1825.md](30_TEST_GUIDED_EVAL5_PROGRESS_20260908_1825.md)。以下保留部署当时状态。
