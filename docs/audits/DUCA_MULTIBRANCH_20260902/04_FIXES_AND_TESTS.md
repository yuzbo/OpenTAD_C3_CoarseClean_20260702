...
The first CT-DP protocol-precheck command (`j-uftgal`) referenced a generator absent from the exact checkout and failed before execution. The corrected precheck compiled the actual CT-DP modules and gate script successfully. ET-TRC's exact-SHA diagnostic PRECHECK also passed; its GPU gate remains queued behind the account submission limit.
