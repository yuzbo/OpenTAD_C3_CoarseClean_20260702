...
最新运行与成绩见[44号心跳记录](44_HEARTBEAT_EVIDENCE_20260909_1035.md)：10:38五项新训练及独立终态均完成，成绩/receipt/stat未变，最高修复版64.2720%。Computer Use初始化及重置后复试仍报kernel assets路径不存在，未进入ixBrowser/Pro。43号四相解释、其他路线缺项和历史数值失败继续保留；实际触发间隔45分31秒，不冒称30分钟准点。以下按时间保留历史裁决，不能把旧观察当当前状态。
...
本轮22:15触发，首次SSH被关闭后按相同配置恢复；实际采样22:21:55-22:22:24，补读日志22:23:56。
五项仍RUNNING。H65第30轮均匀/相位关/四相开60.7207%/60.2283%/52.6773%，三份新best实际EMA通过。
ET新增ON第45轮55.2871%、OFF第40轮62.4262%都未超过此前best，正确保留ON第40轮55.3617%、
OFF第35轮62.5093%，复用未变权重认证。尚无新终态或调参尝试。

H65更新审计3000/3100/3200一致，AMP skip累计3/9/6；相位关epoch29 batch74连续4次重放后恢复，
最新成功3100、耗尽0。已读上下文，列为数值波动而非运行失败，不热改或重启健康作业。

另一任务96e5b3a5历史匹配PRECHECK1280306完成，原日志28tests和三配置真实GPU三batch通过。
signed1280314与control/fulltia链1280316于22:01:16开训；链中当前仅见control，fulltia尚未见正式启动。
保留30+60轮终态主结果身份，不混入本任务strict60测试选best，也不重复提交已通过预检。
四个相关修复分支仍本地clean/未见GitHub发布，其余缺失机制和独立评测不变。

GPU7/200未分配，账户11R/4P，盘143.1GiB。分钟回执新鲜但仍plan/BLOCKED。本轮无新增Slurm提交、
...
## 2026-09-09 03:23 心跳：五项完整训练结束

03:24-03:26实际核查，H65三项与ET两项均COMPLETED，全部60份五轮周期测试收据齐全。
H65终轮均匀/相位关/四相开63.8596%/64.2632%/57.4857%；最佳63.9646%(50)/64.2632%(60)/57.5081%(55)。
三份H65终态实际EMA有限、global成功更新6000；参数Adam步数分布另列，不称所有参数都6000。
ET最佳ON/OFF55.3617%/62.5093%，终轮54.5976%/61.7587%未变，复用实际权重认证。
新独立terminal receipt仍缺；测试集指导选模与官方原版复现分开，不因训练结束宣称方法有效。
35号已发现的历史90轮1280314/1280316数值失败仍在，原任务待修；未接管、热改或重提。
CT/Evidence旧成绩及BAFDR/Unified剩余缺项不变；本轮没有调参、模型修改、Slurm新提交或Pro咨询。
资源55/200未分配、账户1R/2P均其他项目，盘121.83GiB。详见[36号记录](36_HEARTBEAT_EVIDENCE_20260909_0323.md)。

## 2026-09-09 04:56 心跳：独立终态评测推进

05:04实时核查五项完整训练及60份全测试，最佳和终轮成绩不变，既有负结果与缺项保留。
本轮仅修ET独立评测入口：edfbd7e580bbd903db543e1be67169eff6d9125f已推送，
绑定9a346f0d两份原epoch59 EMA、原eval5协议与211视频/792窗口；opentad/configs无差异。
本地及远端exact/clean各34 tests通过，双GPU终态推理PRECHECK1281042_0/1已COMPLETED(0:0)。
在预检通过后，1281049_0(OFF)/1(ON)于05:13:04开始完整独立评测；05:21已推理226/184批，
没有新fatal或独立metrics，不提前升级结果资格。未重新训练或调参，未改旧源码/权重。
H65独立评测、其他路线机制缺项、历史90轮数值失败及原始AdaTAD来源/独立复评仍未解决。
详见[38号记录](38_HEARTBEAT_EVIDENCE_20260909_0456.md)，所有旧日志和历史BAFDR只读边界保留。
