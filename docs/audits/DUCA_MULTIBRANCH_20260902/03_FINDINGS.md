...
1. **H65-Pro: conditional only.** Static validator passes (`28` unique jobs and strict-60 identities) and focused tests pass where Windows permits them (`31 passed`, `7 skipped`). No N16R4 CUDA, physical-coordinate reduction, strict60/6000 terminal receipt, Slurm concurrency, or measured end-to-end cost receipt exists.
...
3. **Evidence: parity-only.** The required C0 must compare indices, physical positions, features, logits, losses, decode and final predictions at exact SHA. No valid remote CUDA/resume/terminal EMA/mAP receipt is present.
4. **CT-DP-BAMoD: geometry-only at frozen SHA.** The frozen G0/G1 factorization did not match the declared G0=Dual-Phase+CT-Tubelet and G1=G0+B-AMoD contract. Correction branch `codex/duca-ctdp-admission-fix-20260902` commit `d62cab763c8e0478e73c6c47a4c185db45164dda` restores that factorization in `duca_ctdp_geometry_g0.py` lines 3-22, `g1.py` lines 3-16, and `g2.py` lines 1-3. It still needs real geometry, gradient and batch/D DP admission.
5. **BAFDR: screen-only.** The static protocol suite passes (`11 passed`), but the terminal D160 epoch59 EMA Teacher identity and disjoint selection split receipt are absent. The official 211-video held-out split must not open the 21-cell matrix.
6. **ET-TRC: checkpoint/DDP-only.** The launcher contract is present, but no verified VideoMAE checkpoint, one-GPU load receipt, or real two-GPU global-batch-2 DDP/resume receipt exists.
