...
The second smoke (`1154956`) passed the dtype boundary and exposed a DDP static-graph mismatch: the stable warmup intentionally left `center_head` unused, while the learned structured policy used it after the schedule transition. The stable reference scores now retain a zero-valued graph dependency on learned scores. This preserves an exactly stable hard input and exactly zero warmup gradient, while keeping the DDP parameter-use set constant across curriculum phases.
