...
- Current one-step joint gradient proof:
  `tools/bata/run_duca_jct_one_step_grad_proof.py`, emitted as
  `duca_jct_one_step_grad_proof.json` under the suite run root.
...
- `run_duca_jct_one_step_grad_proof.py`: generated by the suite focused job
  before full runs; it loads the fixed and DUCA-MUST official configs, runs a
  scaled one-step backward/optimizer update, and records nonzero gradients for
...
readiness, one-step joint gradient proof readiness, result artifacts, and
missing-result gates in
