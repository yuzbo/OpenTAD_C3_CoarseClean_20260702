...
当前已有选择、预算、曲线及 Pareto 绘图框架；以下新增干预数据的生产器尚未实现。这里只交付规格，不生成带科学数值的示意结果。原 117 个诊断不能因本文件而记为完成。B 的空洞/Receiver×Router 与 C 的相似性/计划切换四张补充图见[B/C/LITE 计划](bc-lite-comparison-plan.md#四张新增图生产器尚待实现)，共用本文件的产物身份和解释规则。
