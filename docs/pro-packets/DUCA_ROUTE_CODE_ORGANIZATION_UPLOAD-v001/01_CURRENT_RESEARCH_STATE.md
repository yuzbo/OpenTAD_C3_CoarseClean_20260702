...
当前唯一优先科学动作是：由 Pro 直接检查同步后的代码库存、各实验的 clean revision、已有正负结果与证据缺口，独立决定值得继续的论文问题，并指定一个最小权威代码主线。裁决必须明确哪些实现继续进入候选、哪些只保留为历史证据、哪些重复或混杂表面需要移出候选，以及唯一下一项可证伪任务。Codex 不在该裁决前自行选择或合并 H65、PJST-D1、UVT、Fovea/Query-Bridge、连续片段或动态预算实现。

PJST-D1 的 10,000 次整视频配对自助法仍是未完成证据，但不自动成为下一动作。Pro 需先判断该区间是否仍会改变路线决策；若会，再以现有密封预测做最小统计恢复，禁止重训或改变模型、选择器、数据、NMS、评估器和统计量。若不会，Pro 应说明现有点估计与其他负证据如何支持其下一任务。
