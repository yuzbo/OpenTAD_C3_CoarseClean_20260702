...
def test_duca_jct_one_step_grad_proof_writes_joint_training_artifact(tmp_path: Path) -> None:
    ...
    env.update(
        {
            "DUCA_ONLINE_DENSE_WINDOW_SIZE": "24",
            "DUCA_ONLINE_BUDGET": "16",
            "DUCA_MUST_DENSE_WINDOW_SIZE": "24",
            ...
            "DUCA_JCT_PROOF_TEMPORAL_LEN": "24",
    ...
    assert payload["fixed384"]["proof_uses_precheck_head"] is False
    assert payload["fixed384"]["coarse_probe_grad_sum"] > 0.0
    assert payload["fixed384"]["selector_encoder_grad_sum"] > 0.0
    assert payload["fixed384"]["proof_schedule_phase"] == "joint_detection_selection"
    ...
    assert payload["duca_must"]["proof_uses_precheck_head"] is False
    ...
    assert payload["duca_must"]["after_optimizer_step_summary"]["updated"] is True
