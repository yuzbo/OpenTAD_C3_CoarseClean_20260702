...
def test_registry_inherits_child_configuration_and_retains_every_job(tmp_path):
    ...
    assert result["counts"] == dict(train=612, evaluate=612, benchmark=204, diagnostic=117)
    assert len(result["jobs"]) == 1545
...
def test_split_keeps_all_official_training_videos_and_rejects_changed_annotations(tmp_path):
    ...
    assert len(split["training"]) == 200 and split["validation"] == ["heldout"]
    assert not set(split["training"]) & set(split["validation"])
    assert json.loads((tmp_path / "protocol/annotations.json").read_text())["database"] == database
...
def test_trainable_checkpoint_reconstructs_exact_model_and_ema(tmp_path):
    import copy
    from geosparse_ext.runtime import ModelEMA, save_checkpoint, restore_mutable_state
    ...
    assert "0.weight" in saved["state_dict"] and saved["completed_epochs"] == 6
...
def test_host_assignment_applies_to_training_and_its_artifacts(tmp_path):
    ...
    assert script.index("#SBATCH --mem-per-gpu=120G") < script.index("source /etc/profile")
...
def test_per_configuration_certification_and_failed_rerun_withdrawal(tmp_path):
    ...
    row = dict(status="PASS", job_id=checked["job_id"], optimizer_updates=1, ema_update="PASS", evaluation_pipeline_check=dict(status="PASS"))
