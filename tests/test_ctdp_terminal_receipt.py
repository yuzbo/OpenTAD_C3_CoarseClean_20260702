...
def test_coordinate_evaluator_rejects_pre_remap_checkpoint(arm):
    config = Path(str(CONFIG).replace("g0.py", f"{arm}.py"))
    saved = checkpoint()
    saved["experiment_metadata"]["config_path"] = str(config)
    assert TRAINING_COMMIT == "fe1c53db1b2a7e467a6af2eccfe8b7636f667a81"
    assert checkpoint_counts(saved, config)["successful_optimizer_updates"] == 6000
    saved["experiment_metadata"]["git_commit"] = "78cde1be1cb8b3acc7d750afc92ea740f2a03d06"
    with pytest.raises(ValueError, match="identity"):
        checkpoint_counts(saved, config)
