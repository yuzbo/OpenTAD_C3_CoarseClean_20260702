...
def test_truetime_selected_dense_roundtrip_preserves_fractional_positions():
    time_map = TrueTimeMap(
        selected_positions=[0, 2, 5, 9],
        dense_len=10,
        valid_len=10,
    )
    ...
    assert torch.allclose(roundtrip, selected_axis, atol=1.0e-5)
    assert time_map.selected_axis_name == "selected_axis_index"
    assert time_map.true_time_axis_name == "true_time_dense_index"


def test_truetime_identity_map_preserves_half_open_right_edge():
    time_map = TrueTimeMap(
        selected_positions=range(8),
        dense_len=8,
        valid_len=8,
    )
...
def test_truetime_sparse_map_roundtrip_preserves_head_and_tail_boundaries():
    time_map = TrueTimeMap(
    ...
    assert torch.allclose(roundtrip, segments, atol=1.0e-6)
    assert selected[0, 0].item() == -1.0
    assert selected[-1, 1].item() == 4.0


def test_truetime_segment_remap_is_explicit_about_coordinate_spaces():
    time_map = TrueTimeMap(
        selected_positions=[0, 2, 5, 9],
        dense_len=10,
        valid_len=10,
...
def test_prediction_inverse_map_records_selected_axis_source():
    time_map = TrueTimeMap(
    ...
    assert torch.allclose(
        mapped["segments"],
        torch.tensor([[1.0, 6.0], [3.0, 7.0]]),
    )
    assert torch.equal(mapped["scores"], predictions["scores"])


def test_metadata_selected_axis_remap_preserves_ordering():
    meta = {
    ...
    selected_segments = torch.tensor(
        [[0.0, 1.0], [1.25, 2.5], [2.5, 3.0]]
    )

    time_map = truetime_map_from_metadata(meta)
    true_segments = remap_selected_axis_segments_to_true_time(
        selected_segments,
        meta,
    )
    ...
    assert torch.allclose(
        true_segments,
        torch.tensor([[1.0, 4.0], [5.0, 8.5], [8.5, 9.0]]),
    )
    assert torch.all(true_segments[1:, 0] >= true_segments[:-1, 0])
    assert time_map.selected_len == 4


def test_metadata_selected_axis_remap_fails_closed_when_mapping_missing():
    meta = {
        "detector_prediction_inverse_map_required": True,
        "truetime_dense_len": 10,
    ...
    with pytest.raises(
        ValueError,
        match="selected_axis_to_true_time_dense_index",
    ):
        remap_selected_axis_segments_to_true_time(
