...
def base(route, **kw):
    d = dict(route=route, axis="ST", selector="hybrid", budget_mode="fixed",
             ...
             query_length=384, heavy_context="parent16",
             tia_scope="source_native", cost_target="traced_macs",
             roi_mode="none", source_resolution=224,
...
def compile_all():
        ...
        for dataset in datasets:
            for seed in seeds:
                signature = dict(model=cfg, dataset=dataset, seed=seed,
                                 protocol_version="geosparse-v1.0", epochs=60)
                ...
                if sig not in train_by_sig:
                    ...
                    train_by_sig[sig] = dict(
                      ...
                      protocol_version="geosparse-v1.0", is_mock=False)
                else:
    ...
    for route, label in [("DENSE","official_dense"),("A","A_full"),
