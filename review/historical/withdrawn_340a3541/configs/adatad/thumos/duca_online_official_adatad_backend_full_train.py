...
duca_loss_schedule_warmup_fraction = _env_float("DUCA_LOSS_SCHEDULE_WARMUP_FRACTION", 0.05)
duca_loss_schedule_transition_fraction = _env_float("DUCA_LOSS_SCHEDULE_TRANSITION_FRACTION", 0.15)
...
duca_online_main_contract = dict(
    ...
    detector_stack="official_OpenTAD_AdaTAD_VideoMAE-S_ActionFormerHead_plus_DUCA_prebackbone_plugin_plus_runtime_C3_coarse_probe",
    ...
    boundary_utility_proxy_target_kind="instance_normalized_start_end_context_proxy",
    ...
    max_gap_encoded_in_structured_policy=True,
    ...
    physical_grid_extension_active=False,
    loss_assignment_formula_changed=False,
    ...
    legacy_changes_fields_scope="head_config_and_assignment_formula_only",
...
model = dict(
    frame_selector=dict(
        ...
        max_radius=0,
        acquisition_policy="global_structured_topk",
        structured_temperature=_env_float("DUCA_STRUCTURED_TEMPERATURE", 0.7),
        ...
        loss_weight_schedule=dict(
            ...
            start=dict(start=_env_float("DUCA_LOSS_START_ENDPOINT_START", 0.50), end=0.50),
            end=dict(start=_env_float("DUCA_LOSS_END_ENDPOINT_START", 0.50), end=0.50),
            context=dict(start=_env_float("DUCA_LOSS_CONTEXT_START", 0.10), end=0.10),
