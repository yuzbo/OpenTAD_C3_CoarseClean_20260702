...
def resolved_model_protocol(model, input_shape):
    ...
    return dict(route=geo.route, input_shape=list(input_shape), input_positions=input_shape[-3],
                query_length=geo.query_length, native_tubelet=2 if source is None else source.tubelet_size,
...
def resolve_opentad_config(job, bindings, annotation_file):
    ...
    cfg = Config.fromfile(str(root / "configs/adatad/thumos/e2e_thumos_videomae_s_768x1_160_adapter.py"))
    model = job["model"]
    ...
    for split, data in cfg.dataset.items():
        ...
        for transform in data.pipeline:
            ...
            if transform["type"] == "mmaction.ImgAug":
                # Its unrecorded affine operations invalidate original-image ROI
                # coordinates. The common amended recipe uses tracked crop/flip
                # and ColorJitter for every route, including the dense control.
                continue
            if transform["type"] == "mmaction.Resize":
                transform["scale"] = (224, 224) if transform.get("keep_ratio") is False else (-1, 256 if split == "train" else 224)
