...
class TrainableEMA:
    """Same 0.999 EMA; frozen recognition parameters are constant and shared."""
    def __init__(self, model):
        self.names = mutable_state_names(model)
    ...
    def update(self, model):
        for name, value in model.state_dict().items():
            ...
            if value.is_floating_point():
                self.shadow[name].mul_(.999).add_(value, alpha=.001)
            else:
...
def mutable_state_names(model):
    names = {n for n, p in model.named_parameters() if p.requires_grad} | {n for n, _ in model.named_buffers()}
    return names & set(model.state_dict())


def restore_mutable_state(model, checkpoint, key):
    """The model must already contain the provenance-checked recognition base."""
    if checkpoint.get("format") != "geosparse_trainable_v1":
        raise ValueError("checkpoint is not the registered trainable-state format")
    values = checkpoint[key]
    expected = mutable_state_names(model)
...
def optimizer_for(model, cfg):
    groups = model.get_optim_groups(cfg)
...
def save_checkpoint(path, model, ema, optimizer, scheduler, scaler, epoch, provenance, updates):
    names = mutable_state_names(model)
    state = dict(format="geosparse_trainable_v1", epoch=epoch, completed_epochs=epoch + 1,
