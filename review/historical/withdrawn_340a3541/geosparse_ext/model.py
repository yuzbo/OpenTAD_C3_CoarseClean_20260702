...
class GeoSparseModel(nn.Module):
    ...
    def forward(self, batch, epoch=0, forced_plan=None, generators=None, execution_rng=None):
        ...
        if self.route == "COARSE":
            ...
            reference_tokens = 8 * self.encoder.source.grid_size[0] * self.encoder.source.grid_size[1]
        ...
        if self.route in {"B", "COARSE"}:
            ...
            query = self.regular_tia(query.reshape(-1, 8, 256), 1, 1,
                                     temporal_token_mask=query_valid.reshape(-1, 8)).reshape(b, self.query_length, 256)
