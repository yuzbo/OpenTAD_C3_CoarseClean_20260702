...
def _metadata_scalar(value, *, key: str):
    if torch.is_tensor(value):
        if int(value.numel()) != 1:
            raise ValueError(
                f"{key} must be scalar, got tensor shape {tuple(value.shape)}"
            )
        return value.detach().cpu().reshape(-1)[0].item()
    if isinstance(value, (list, tuple)):
        if len(value) != 1:
            raise ValueError(
                f"{key} must be scalar, got sequence length {len(value)}"
            )
        return value[0]
    return value
...
def _metadata_int(
    ...
    *,
    ...
    if default is None:
        raise ValueError(
            f"metadata missing required integer field; expected one of {keys!r}"
        )
    return int(default)
...
class TrueTimeMap:
    """Map detector selected-axis coordinates to original dense-time coordinates."""
    ...
    def _interpolation_knots(self):
        ...
        if float(true_time[0].item()) > 0.0:
            selected_axis = torch.cat(
                (selected_axis.new_tensor([-1.0]), selected_axis)
            )
        ...
        if float(true_time[-1].item()) < float(self.valid_len):
            selected_axis = torch.cat(
                (
                    selected_axis,
                    selected_axis.new_tensor([float(self.selected_len)]),
                )
            )
            true_time = torch.cat(
                (true_time, true_time.new_tensor([float(self.valid_len)]))
            )
        return selected_axis, true_time

    @staticmethod
    def _piecewise_linear(values, source_knots, target_knots):
        clipped = values.clamp(
            float(source_knots[0].item()),
            float(source_knots[-1].item()),
        )
        right = torch.searchsorted(source_knots, clipped, right=False)
        right = torch.clamp(
            right,
            min=1,
            max=int(source_knots.numel()) - 1,
        )
        left = right - 1
        left_source = source_knots[left]
        right_source = source_knots[right]
        fraction = (clipped - left_source) / (
            right_source - left_source
        ).clamp_min(1.0e-6)
        return (
            target_knots[left]
            + (target_knots[right] - target_knots[left]) * fraction
        )

    def selected_to_true(self, values):
        values_t = _as_float_tensor(
            values,
            device=self.selected_positions.device,
        )
    ...
    def remap_segments(
        ...
        *,
        ...
        if source_coordinate_space == target_coordinate_space:
            return _as_float_tensor(
                segments,
                device=self.selected_positions.device,
            ).clone()
        if (
            source_coordinate_space == self.selected_axis_name
            and target_coordinate_space == self.true_time_axis_name
        ):
            return self.selected_to_true(segments)
        if (
            source_coordinate_space == self.true_time_axis_name
            and target_coordinate_space == self.selected_axis_name
        ):
            return self.true_to_selected(segments)
        raise ValueError(
            "unsupported segment remap: "
            f"{source_coordinate_space!r} -> {target_coordinate_space!r}; "
            f"expected {self.selected_axis_name!r} or "
            f"{self.true_time_axis_name!r}"
        )


def truetime_map_from_metadata(
    metadata: Mapping,
    *,
    require_inverse_map: bool | None = None,
) -> TrueTimeMap:
    ...
    if positions is None:
        ...
        if remap_required:
            raise ValueError(
                f"{message} while detector_prediction_inverse_map_required is true"
            )
    ...
    if positions_t.ndim != 1 or int(positions_t.numel()) == 0:
        raise ValueError(
            "selected_axis_to_true_time_dense_index must be a non-empty 1D sequence"
        )
    ...
    if (
        ...
        and _metadata_int_value(
            selected_count,
            key="irregular_selected_count",
        )
        != int(positions_t.numel())
    ):
        raise ValueError(
            "irregular_selected_count must match "
            "selected_axis_to_true_time_dense_index length"
        )
    if (
        selected_valid_len is not None
        and _metadata_int_value(
            selected_valid_len,
            key="irregular_selected_valid_len",
        )
        != int(positions_t.numel())
    ):
        raise ValueError(
            "irregular_selected_valid_len must match "
            "selected_axis_to_true_time_dense_index length"
        )

    return TrueTimeMap(
        selected_positions=positions_t,
        dense_len=dense_len,
        valid_len=dense_valid_len,
    )
...
def inverse_map_prediction_segments(
    predictions: Mapping,
    time_map: TrueTimeMap,
    *,
    segment_key: str = "segments",
):
    source_space = predictions.get("coordinate_space")
    if source_space != time_map.selected_axis_name:
        raise ValueError(
            "prediction inverse-map requires detector outputs in selected-axis "
            f"coordinates; got {source_space!r}"
        )
    ...
    mapped["true_time_map"] = {
        "selected_positions": [
            int(item)
            for item in time_map.selected_positions.long().tolist()
        ],
