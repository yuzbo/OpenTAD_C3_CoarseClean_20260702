...
class BAFDRAsymmetricProjection(Conv1DTransformerProj):
    """BA-FDR Asymmetric Feature Pyramid Projection.
    ...
    def forward(
        ...
        # L2..L5 are strictly derived from global branch (Bitwise invariant to R)
...
@PROJECTIONS.register_module()
class BAFDRLateProjection(Conv1DTransformerProj):
    """BA-FDR Late Injection Projection (Ablation Control Arm BAFDR-K16-LATE).
    
    Feeds fused feature Z = G + R into standard single-stream pyramid across all levels L0..L5.
    """
    ...
    def forward(
        ...
        if isinstance(x, dict):
            feat = x.get("fused_features", x.get("feats"))
        else:
            feat = x
        return super().forward(feat, mask)
