...
def detector(route, estimator="pg_acquisition"):
    ...
    return GeoSparseDetector(
        ...
        geosparse=config,
        source_config=dict(img_size=32, embed_dims=16, depth=2, num_heads=2, drop_path_rate=0.,
...
def test_real_optimizer_ema_and_restore_keep_fractional_loss_normalizer():
    from geosparse_ext.runtime import TrainableEMA, restore_mutable_state
    model = detector("B").train()
    ...
    optimizer.step()
    ema.update(model)
    expected = 10. * .999 + float(model.rpn_head.loss_normalizer) * .001
    torch.testing.assert_close(ema.shadow["rpn_head.loss_normalizer"], torch.tensor(expected))
    restored = detector("B")
    restore_mutable_state(restored, dict(format="geosparse_trainable_v1", state_dict_ema=ema.state_dict()), "state_dict_ema")
    torch.testing.assert_close(restored.rpn_head.loss_normalizer, torch.tensor(expected))
