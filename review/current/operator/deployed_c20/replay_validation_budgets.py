...
def main():
    ...
    if not os.environ.get("SLURM_JOB_ID") or not step_gpu.isdigit() or not os.environ.get("CUDA_VISIBLE_DEVICES", "").isdigit():
        raise RuntimeError("diagnostic requires the existing allocation's single mapped GPU")
    if binding["cluster"] == "N16R4" and (step_gpu != "1" or os.environ.get("CUDA_VISIBLE_DEVICES") != "0"):
