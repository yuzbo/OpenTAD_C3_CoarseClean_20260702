"""Run the first supplementary seed0 cohort after deployed main methods.

The production model source and primary coordinators are unchanged. This
external launcher adds only a lower Slurm priority and start dependencies.
"""
import argparse
import json
import os
...
def primary_state(bindings):
    ...
    for path in bindings['primary_binding_paths']:
        primary = load(path)
        states = load(Path(primary['work_root']) / 'slurm_state.json')['jobs']
...
def scheduled_script(script, primary_ids):
    directives = ['#SBATCH --nice=10000']
    if primary_ids:
...
def precheck(args, bindings, queue):
    ...
    for jid in bindings['assigned_training_ids']:
        ...
        script = script.replace('#SBATCH --time=24:00:00', '#SBATCH --time=00:15:00')
        script = scheduled_script(script, primary_ids)
        ...
        argv = ['sbatch', '--parsable', '--job-name=gs-secondary-check-' + jid, '--output=' + str(out / 'slurm-%j.out'), '--error=' + str(out / 'slurm-%j.err')]
        if nodes:
...
def main():
    ...
    lock = open(control / 'secondary_supervisor.lock', 'a+')
    ...
    def worker(job_file, output, bindings_file, current):
        ...
        return scheduled_script(original_worker(job_file, output, bindings_file, current), primary_ids)
    ...
    while True:
        ...
        save(control / 'secondary_progress.json', dict(updated_at=time.time(), pid=os.getpid(), host=socket.gethostname(), precheck_status=status, states={jid:state['jobs'][jid]['status'] for jid in bindings['assigned_training_ids']}))
        time.sleep(60)
