...
def main():
    ...
    for cluster, target in [('N16R4','source'),('A100','destination')]:
        filename='bindings.dynamic.'+cluster+'.json'
    ...
    if (HERE/'FULL_METHODS_FIRST.zh.md').exists():
        page+='<section><b>当前优先级：完整方法先行</b><p>A/B/C各保留固定50%与完整动态预算版本，共六项seed0主实验；官方AdaTAD基线继续。动态版本开启预算选择、任务价值PG和在线收益校准。B-full（全量Heavy证据对照）、额外稠密对照、其他预算档位、模块消融和新增消融实现暂缓。<a href="../../official_adatad_audit/FULL_METHODS_FIRST.zh.md">查看本轮执行修订</a>。</p></section>'
