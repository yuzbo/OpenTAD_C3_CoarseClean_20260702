...
for jid in b['assigned_training_ids']:
 p=root/'runs'/jid
 rows[jid]=dict(queue=state['jobs'].get(jid),progress=read(p/'progress.json'),best=read(p/'best.json'),result=read(p/'result.json'),failure=read(p/'failure.json'),precheck_submission=pre.get(jid),precheck=read(root.parent.parent/'prechecks'/jid/'gpu_precheck.json'))
...
print(json.dumps(dict(cluster=b['cluster'],source_commit=b['source_commit'],runs=rows,official=official,slurm=slurm,coordinator=read(control/'coordinator.json'))))
'''
...
def main():
    ...
    labels = {'tr-c43d3e9cad58':'A-ST 0.5','tr-be89cd6cb2ca':'B-ST 0.5','tr-a45686a40afa':'C 0.5',
              'tr-c2d93c4c3979':'B-full','tr-bacdbb058eca':'原版结构＋统一训练器的稠密对照'}
    rows=[]
